...
import { RelationPoint, Edge as RWEdge } from "../lib/layout";
...
const getEdgeTooltipContent = (
  ...
  return `
    <div>
      <div><b>Back Pressure:</b> ${stats.backPressure != null ? `${(stats.backPressure * 100).toFixed(2)}%` : 'N/A'}</div>
      <div><b>Recv Throughput:</b> ${stats.recvThroughput != null ? `${stats.recvThroughput.toFixed(2)} rows/s` : 'N/A'}</div>
      <div><b>Send Throughput:</b> ${stats.sendThroughput != null ? `${stats.sendThroughput.toFixed(2)} rows/s` : 'N/A'}</div>
    </div>
  `;
};
...
const ReactFlowRelationGraph = ({
      ...
      if (node.parentIds && node.parentIds.length > 0) {
        node.parentIds.forEach((parentId: string) => {
          edges.push({
            ...
            source: parentId,
            target: node.id,
            style: getEdgeStyle(parentId, node.id, channelStats, 
              selectedId === parentId || selectedId === node.id),
            data: {
              tooltipContent: getEdgeTooltipContent(parentId, node.id, channelStats)
            },
          });
        });
      }
  ...
  const showSubDag = useCallback((nodeId: string) => {
    if (!nodeId || nodeId === '') {
      setFilteredNodes([]);
      return;
    }

    const getRelatedNodes = (nodes: Node[], edges: Edge[], nodeId: string) => {
  ...
  const displayedEdges = useMemo(() => {
    ...
    return edges.filter((edge: Edge) => 
      filteredNodes.includes(edge.source) && filteredNodes.includes(edge.target)
    );
  }, [edges, filteredNodes]);
