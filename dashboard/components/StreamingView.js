...
export default function StreamingView(props) {
  ...
  const [mvTableIdToSingleViewActorList, setMvTableIdToSingleViewActorList] =
    useState(null)
  const [mvTableIdToChainViewActorList, setMvTableIdToChainViewActorList] =
    useState(null)
  ...
  const resizeCanvas = () => {
    if (canvasOuterBox.current) {
      getEngine() &&
        getEngine().resize(
          canvasOuterBox.current.clientWidth,
          canvasOuterBox.current.clientHeight
        )
    }
  }
  ...
  useEffect(() => {
    if (canvasRef.current && showFullGraph) {
      ...
      mvTableIdToSingleViewActorList ||
        setMvTableIdToSingleViewActorList(
          getView().getMvTableIdToSingleViewActorList()
        )
      mvTableIdToChainViewActorList ||
        setMvTableIdToChainViewActorList(
          getView().getMvTableIdToChainViewActorList()
        )
      return () => {
      ...
      <SvgBoxCover style={{ right: "10px", top: "10px", width: "500px" }}>
        <PopupBox
          style={{
            ...
            height:
              canvasOuterBox && canvasOuterBox.current
                ? canvasOuterBox.current.clientHeight - 100
                : 500,
          }}
        >
          <PopupBoxHeader>
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      width: 10,
                      borderRadius: 5,
                      backgroundColor: computeNodeAddrToSideColor(
                        x.host.host + ":" + x.host.port
                      ),
                    }}
                  />
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          <FormControl sx={{ m: 1, width: 300 }}>
            ...
            <Autocomplete
              ...
              renderInput={(param) => (
                <TextField {...param} label="Materialized View" />
              )}
            />
          </FormControl>
        </div>
