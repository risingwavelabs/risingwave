...
export default function HeapProfiling() {
  ...
  const [selectedProfileList, setSelectedProfileList] = useState<FileList | undefined>()
  ...
  function updateSelectedProfileList() {
    ...
    if (profileType === "Auto") {
      setSelectedProfileList({dir: profileList.dirAuto, name: profileList.nameAuto})
    } else if (profileType === "Manually") {
      setSelectedProfileList({dir: profileList.dirManually, name: profileList.nameManually})
    } else {
  ...
  async function getProfileList(computeNodes: WorkerNode[] | undefined, computeNodeId: number | undefined) {
    if (computeNodes && computeNodeId && computeNodes.length > 0) {
        try {
  ...
  async function analyzeHeapFile() {
    if (selectedProfileList === undefined || analyzeTargetFileName === undefined) {
      console.log(`selectedProfileList: ${selectedProfileList}, analyzeTargetFileName: ${analyzeTargetFileName}`)
      return
    }

    let analyzeFilePath = path.join(selectedProfileList.dir, analyzeTargetFileName)
