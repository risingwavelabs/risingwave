...
public class FlinkSubQueryRemoveRule extends RelOptRule {
  ...
  private Optional<RexNode> apply(
      RexCall subQueryCall, RelBuilder relBuilder, SubQueryDecorrelator.Result decorrelate) {
  ...
  private List<RexNode> fields(RelBuilder builder, int fieldCount) {
    ...
    IntStream.range(0, fieldCount).forEachOrdered(i -> projects.add(builder.field(i)));
    return projects;
  }
   ...
   *
   * <p>e.g. SELECT * FROM l WHERE a + 1 IN (SELECT c FROM r) We will add projection as SEMI join
   * left input, the added projection will pass along fields from the input, and add `a + 1` as new
   * field.
   */
  private Pair<List<RexNode>, Optional<RexNode>> handleSubQueryOperands(
      RexSubQuery subQuery, Optional<RexNode> joinCondition, RelBuilder relBuilder) {
  ...
  private boolean hasCorrelatedExpressions(List<RexNode> nodes) {
    var relShuttle =
        new RelShuttleImpl() {
          final RexVisitor<Void> corVarFinder =
              new RexVisitorImpl<Void>(true) {
