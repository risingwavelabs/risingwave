 ...
 *
 * <p>This class is adapted from Flink's source code.
 */
public class SubQueryDecorrelator extends RelShuttleImpl {
  ...
  public static Result decorrelateQuery(RelNode rootRel) {
    var maxCnfNodeCount = Integer.MAX_VALUE;
  ...
  private static void analyzeCorConditions(
    ...
    final RexNode cnf = PlannerUtils.toCnf(rexBuilder, maxCnfNodeCount, condition);
  ...
  private static RexNode adjustInputRefs(
      final RexNode c, final Map<Integer, Integer> mapOldToNewIndex, final RelDataType rowType) {
    return c.accept(
        new RexShuttle() {
          @Override
  ...
  public static class SubQueryRelDecorrelator implements ReflectiveVisitor {
    ...
    public Frame decorrelateRel(LogicalProject rel) {
      ...
      RelNode newProject =
          RelOptUtil.createProject(
              newInput,
              projects.stream().map(Pair::getKey).collect(Collectors.toList()),
              projects.stream().map(Pair::getValue).collect(Collectors.toList()),
              false);
     ...
     *
     * <p>Rewrite logic: 1. Permute the group by keys to the front. 2. If the input of an aggregate
     * produces correlated variables, add them to the group list. 3. Change aggCalls to reference
     * the new project.
     *
    ...
    public Frame decorrelateRel(LogicalAggregate rel) {
      ...
      for (AggregateCall oldAggCall : oldAggCalls) {
        ...
        final int filterArg =
            oldAggCall.filterArg < 0 ? oldAggCall.filterArg : combinedMap.get(oldAggCall.filterArg);
     ...
     * Rewrite LogicalJoin.
     *
     * <p>Rewrite logic: 1. rewrite join condition. 2. map output positions and produce corVars if
     * any.
     *
     * @param rel Join
     */
  ...
  private static class CorelMapBuilder extends RelShuttleImpl {
    ...
    /**
     * check whether the predicate on join has unsupported correlation condition. e.g. SELECT * FROM
     * l WHERE a IN (SELECT c FROM r WHERE l.b IN (SELECT e FROM t))
     */
    ...
    /**
     * check whether a node has some input which have correlation condition. e.g. SELECT * FROM l
     * WHERE EXISTS (SELECT * FROM r LEFT JOIN (SELECT * FROM t WHERE t.j=l.b) t1 ON r.f=t1.k) the
     * above sql can not be converted to semi-join plan, because the right input of Left-Join has
     * the correlation condition(t.j=l.b).
     */
    private void checkCorConditionOfInput(final RelNode input) {
      final RelShuttleImpl shuttle =
          new RelShuttleImpl() {
    ...
    /**
     * check whether a SetOp has some children node which have correlation condition. e.g. SELECT a
     * FROM l WHERE b IN (SELECT r1.e FROM r1 WHERE l.a = r1.d UNION SELECT r2.i FROM r2)
     */
  ...
  public static class Result {
    private final com.google.common.collect.ImmutableMap<RexSubQuery, Pair<RelNode, RexNode>>
        subQueryMap;
