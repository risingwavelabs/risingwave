...
public class ParseSqlNodeTest {
  ...
  public void testCreateStream() {
    SqlNode node =
        SqlParser.createCalciteStatement("create stream s (v1 int not null) ROW FORMAT json");

    var writer = new SqlPrettyWriter();
    node.unparse(writer, 0, 0);
    assertEquals(
        writer.toSqlString().getSql(),
        "CREATE STREAM IF NOT EXISTS \"s\" (\"v1\" INTEGER NOT NULL) WITH () ROW FORMAT json");
  }
}
