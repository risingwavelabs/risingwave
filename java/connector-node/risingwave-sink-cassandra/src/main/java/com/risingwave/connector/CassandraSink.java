...
public class CassandraSink extends SinkWriterBase {
  ...
  public CassandraSink(TableSchema tableSchema, CassandraConfig config) {
    ...
    if (hostPort.length != 2) {
      throw new IllegalArgumentException("Invalid cassandraURL: expected `host:port`, got " + url);
    }
    // check connection
    ...
    if (config.getType().equals("upsert")) {
      ...
      this.stmtMap.put(
          "update", session.prepare(createUpdateStatement(config.getTable(), tableSchema)));

      // prepare the delete statement
      this.stmtMap.put(
          "delete", session.prepare(createDeleteStatement(config.getTable(), tableSchema)));
    }
  }
    ...
    while (rows.hasNext()) {
      ...
      switch (op) {
        ...
        case DELETE:
          break;
        default:
          throw Status.INTERNAL.withDescription("Unknown operation: " + op).asRuntimeException();
      }
      tryCommit();
    }
  }
  ...
  private String createInsertStatement(String tableName, TableSchema tableSchema) {
    ...
    return String.format(
        "INSERT INTO %s (%s) VALUES (%s)", tableName, columnNamesString, placeholdersString);
  }
            ...
            .mapToObj(
                (index) ->
                    CassandraUtil.convertRow(
                        row.get(index),
                        schema.getColumnDescs().get(index).getDataType().getTypeName()))
            .toArray());
  }
  ...
  private BoundStatement bindDeleteStatement(PreparedStatement stmt, SinkRow row) {
    ...
    return stmt.bind(
        getTableSchema().getPrimaryKeys().stream()
            .map(key -> CassandraUtil.convertRow(schema.getFromRow(key, row), columnDescs.get(key)))
            .toArray());
  }
  ...
  private BoundStatement bindUpdateInsertStatement(
    ...
    for (int i = 0; i < numNonKeys; i++) {
      ...
      values[i] =
          CassandraUtil.convertRow(schema.getFromRow(name, insertRow), columnDescs.get(name));
    }
    ...
    for (int i = 0; i < numKeys; i++) {
      ...
      values[numNonKeys + i] =
          CassandraUtil.convertRow(schema.getFromRow(name, updateRow), columnDescs.get(name));
    }
