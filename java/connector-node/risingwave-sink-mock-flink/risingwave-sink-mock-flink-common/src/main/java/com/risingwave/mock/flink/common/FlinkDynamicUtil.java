...
public class FlinkDynamicUtil {
  ...
  public static List<Column> getFlinkColumnsFromSchema(TableSchema tableSchema) {
    ...
    for (ColumnDesc columnDesc : tableSchema.getColumnDescs()) {
      columns.add(
          Column.physical(
              columnDesc.getName(), getCorrespondingFlinkType(columnDesc.getDataType())));
    }
    return columns;
  }

  public static void validateSchemaWithCatalog(
      TableSchema tableSchema, FlinkDynamicAdapterConfig config, CatalogFactory catalogFactory)
    ...
    Map<String, DataType> flinkColumnMap =
        flinkColumns.stream().collect(Collectors.toMap(e -> e.getName(), e -> e.getDataType()));
    for (ColumnDesc columnDesc : rwColumnDescs) {
      if (!flinkColumnMap.containsKey(columnDesc.getName())) {
        throw Status.FAILED_PRECONDITION
            .withDescription(
                String.format("Don't match in the name, rw is %s", columnDesc.getName()))
            .asRuntimeException();
      }
    ...
    try {
      ...
      for (int i = 0; i < flinkColumns.size(); i++) {
        ...
        if (unresolvedColumn instanceof Schema.UnresolvedPhysicalColumn) {
          Schema.UnresolvedPhysicalColumn c1 = (Schema.UnresolvedPhysicalColumn) unresolvedColumn;
