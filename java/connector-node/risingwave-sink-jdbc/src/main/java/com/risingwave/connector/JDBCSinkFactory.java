...
public class JDBCSinkFactory implements SinkFactory {
  ...
  public void validate(
    ...
    try (Connection conn = DriverManager.getConnection(jdbcUrl);
        ResultSet tableNamesResultSet = conn.getMetaData().getTables(null, schemaName, "%", null);
        ResultSet columnResultSet =
            conn.getMetaData().getColumns(null, schemaName, tableName, null);
        ResultSet pkResultSet = conn.getMetaData().getPrimaryKeys(null, schemaName, tableName); ) {
    ...
    for (String sinkColumn : tableSchema.getColumnNames()) {
      if (!jdbcColumns.contains(sinkColumn)) {
        ...
        throw Status.FAILED_PRECONDITION
            .withDescription("table schema does not match, column not found: " + sinkColumn)
            .asRuntimeException();
      }
    }
