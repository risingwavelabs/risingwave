...
public interface JdbcDialect {
   ...
   * string will be used as a {@link java.sql.PreparedStatement}. Fields in the statement must be in
   * the same order as the {@code fieldNames} parameter.
   *
   * <p>If the dialect does not support native upsert statements, the writer will fallback to {@code
   * SELECT} + {@code UPDATE}/{@code INSERT} which may have poor performance.
   *
  ...
  default String getDeleteStatement(SchemaTableName schemaTableName, List<String> conditionFields) {
    ...
    return "DELETE FROM " + getNormalizedTableName(schemaTableName) + " WHERE " + conditionClause;
  }
