...
public class JDBCSink implements SinkWriter {
  ...
  public JDBCSink(JDBCSinkConfig config, TableSchema tableSchema) {
    ...
    try {
      ...
      this.pkColumnNames = getPkColumnNames(conn, config.getTableName(), config.getSchemaName());
      ...
      throw Status.INTERNAL
          .withDescription(String.format(ERROR_REPORT_TEMPLATE, e.getSQLState(), e.getMessage()))
          .asRuntimeException();
    }
  }
  ...
  private static Map<String, Integer> getColumnTypeMapping(
    ...
    try {
      ResultSet columnResultSet = conn.getMetaData().getColumns(null, schemaName, tableName, null);
    ...
    while (true) {
      try {
        ...
        if (++retryCount > maxRetryCount) {
          throw Status.INTERNAL
              .withDescription("Failed to execute JDBC statements and exceeded max retry times")
              .withCause(e)
              .asRuntimeException();
        }
        ...
        try {
          ...
          LOG.error("Failed to create a new JDBC connection, retried {} times", retryCount, ex);
          throw io.grpc.Status.INTERNAL
              .withDescription(
                  String.format(ERROR_REPORT_TEMPLATE, ex.getSQLState(), ex.getMessage()))
              .asRuntimeException();
        }
      }
    }
  ...
  /**
   * A wrapper class for JDBC prepared statements We create this object one time and reuse it across
   * multiple batches if only the JDBC connection is valid.
   */
  class JdbcStatements implements AutoCloseable {
    ...
    public JdbcStatements(Connection conn) throws SQLException {
      this.conn = conn;
      var schemaTableName =
          jdbcDialect.createSchemaTableName(config.getSchemaName(), config.getTableName());
      ...
      if (config.isUpsertSink()) {
        ...
        this.deleteStatement = conn.prepareStatement(deleteSql, Statement.RETURN_GENERATED_KEYS);
      } else {
        var insertSql =
            jdbcDialect.getInsertIntoStatement(
                schemaTableName, List.of(tableSchema.getColumnNames()));
        this.insertStatement = conn.prepareStatement(insertSql, Statement.RETURN_GENERATED_KEYS);
      }
    }
    ...
    public void prepareDelete(SinkRow row) {
      ...
      if (pkColumnNames.isEmpty()) {
        throw Status.INTERNAL
            .withDescription("downstream jdbc table should have primary key to handle DELETE event")
            .asRuntimeException();
      }
      try {
  ...
  public Optional<ConnectorServiceProto.SinkMetadata> barrier(boolean isCheckpoint) {
    if (updateFlag) {
      throw Status.FAILED_PRECONDITION
          .withDescription("expected UPDATE_INSERT to complete an UPDATE operation, got `sync`")
          .asRuntimeException();
    }
