...
// import java.util.Calendar; // Use java.time.* instead
 ...
 *
 * <p>A few of the {@link AbstractRowsEventDataDeserializer MySQL Binlog Client row deserializers}
 * convert MySQL raw row data into {@link java.sql.Date}, {@link java.sql.Time}, and {@link
 * java.sql.Timestamp} values using {@link Calendar} instances (and thus implicitly use the local
 * timezone to calculate the milliseconds past epoch. Rather than perform this conversion, we prefer
 * to convert the raw MySQL row values directly into {@link LocalDate}, {@link LocalTime}, {@link
 * LocalDateTime}, and {@link OffsetDateTime}.
 *
 * <p>Unfortunately, all of the methods used to deserialize individual values are defined on the
 * {@link AbstractRowsEventDataDeserializer} base class, and inherited by the {@link
 * DeleteRowsEventDataDeserializer}, {@link UpdateRowsEventDataDeserializer}, and {@link
 * WriteRowsEventDataDeserializer} subclasses. Since we can't provide a new base class, the simplest
 * way to override these methods is to subclass each of these 3 subclasses and override the methods
 * on all 3 classes. It's ugly, but it works.
 *
 * <p>See the <a href="https://dev.mysql.com/doc/refman/5.0/en/datetime.html">MySQL Date Time</a>
 * documentation.
 *
...
public class RowDeserializers {
    ...
    public static class DeleteRowsDeserializer extends DeleteRowsEventDataDeserializer {
        ...
        protected Serializable deserializeDate(ByteArrayInputStream inputStream)
                throws IOException {
            return RowDeserializers.deserializeDate(
                    inputStream, eventProcessingFailureHandlingMode);
        }

        @Override
        protected Serializable deserializeDatetime(ByteArrayInputStream inputStream)
                throws IOException {
            return RowDeserializers.deserializeDatetime(
                    inputStream, eventProcessingFailureHandlingMode);
        }
        ...
        protected Serializable deserializeDatetimeV2(int meta, ByteArrayInputStream inputStream)
                throws IOException {
            return RowDeserializers.deserializeDatetimeV2(
                    meta, inputStream, eventProcessingFailureHandlingMode);
        }
     ...
     * href="http://dev.mysql.com/doc/refman/8.2/en/date-and-time-types.html">zero values</a> for
     ...
     * @throws IOException if there is an error reading from the binlog event data
     */
    protected static Serializable deserializeDate(
    ...
    protected static Serializable deserializeTimeV2(int meta, ByteArrayInputStream inputStream)
        ...
        boolean isNegative = bitSlice(time, 0, 1, payloadBits) == 0;
     ...
     * @throws IOException if there is an error reading from the binlog event data
     */
    protected static Serializable deserializeDatetime(
            ByteArrayInputStream inputStream,
            EventProcessingFailureHandlingMode eventProcessingFailureHandlingMode)
            throws IOException {
     ...
     * @throws IOException if there is an error reading from the binlog event data
     */
    protected static Serializable deserializeDatetimeV2(
            int meta,
            ByteArrayInputStream inputStream,
            EventProcessingFailureHandlingMode eventProcessingFailureHandlingMode)
            throws IOException {
    ...
    private static Serializable handleException(
            EventProcessingFailureHandlingMode eventProcessingFailureHandlingMode,
            String columnType,
            Exception e,
        ...
        if (eventProcessingFailureHandlingMode == EventProcessingFailureHandlingMode.FAIL) {
            LOGGER.error(
                    "Error while deserializing binlog data of {}: {}", columnType, e.getMessage());
            throw new DebeziumException(
                    "Error while deserializing binlog data of "
                            + columnType
                            + ": "
                            + e.getMessage());
