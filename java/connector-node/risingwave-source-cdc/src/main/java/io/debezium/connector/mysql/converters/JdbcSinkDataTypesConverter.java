 ...
 * that data types be handled consistently across both phases for JDBC sink connectors to create the
 * sink tables properly that adhere to the data provided in both phases.
 *
 * <p>This converter specific makes the following changes: - {@code BOOLEAN} columns always emitted
 * as INT16 schema types, true=1 and false=0. - {@code REAL} columns always emitted as FLOAT64
 * schema types. - String-based columns always emitted with "__debezium.source.column.character_set"
 * parameter.
 *
...
public class JdbcSinkDataTypesConverter
    ...
    public void configure(Properties props) {
        ...
        if (!Strings.isNullOrBlank(stringSelectorConfig)) {
            selectorString =
                    Predicates.includes(
                            stringSelectorConfig.trim(), x -> x.dataCollection() + "." + x.name());
        }
