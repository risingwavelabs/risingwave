...
public class MariaDbReadOnlyIncrementalSnapshotContext<T>
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(MariaDbReadOnlyIncrementalSnapshotContext.class);
    ...
    protected static <U> IncrementalSnapshotContext<U> init(
            MariaDbReadOnlyIncrementalSnapshotContext<U> context, Map<String, ?> offsets) {
    ...
    public static <U> MariaDbReadOnlyIncrementalSnapshotContext<U> load(
            Map<String, ?> offsets, boolean useCatalogBeforeSchema) {
        MariaDbReadOnlyIncrementalSnapshotContext<U> context =
                new MariaDbReadOnlyIncrementalSnapshotContext<>(useCatalogBeforeSchema);
    ...
    public boolean reachedHighWatermark(String currentGtid) {
        ...
        if (streamSet != null) {
            long maxSequenceId =
                    streamSet.stream().mapToLong(MariaDbGtid::getSequence).max().getAsLong();
