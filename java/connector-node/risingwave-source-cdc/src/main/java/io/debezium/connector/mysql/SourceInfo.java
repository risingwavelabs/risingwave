...
/**
 * Information about the source of information, which includes the position in the source binary log
 * we have previously processed.
 *
 * <p>The {@link MySqlPartition#getSourcePartition() source partition} information describes the
 * database whose log is being consumed. Typically, the database is identified by the host address
 * port number of the MySQL server and the name of the database. Here's a JSON-like representation
 * of an example database:
 *
 * <pre>
 * {
 ...
 * </pre>
 *
 * <p>The offset includes the {@link #binlogFilename() binlog filename}, the {@link
 * #binlogPosition() position of the first event} in the binlog, the {@link
 * MySqlOffsetContext#eventsToSkipUponRestart() number of events to skip}, and the {@link
 * MySqlOffsetContext#rowsToSkipUponRestart() number of rows to also skip}.
 *
 * <p>Here's a JSON-like representation of an example offset:
 *
 * <pre>
 * {
 *     "server_id": 112233,
 ...
 * </pre>
 *
 * <p>The "{@code gtids}" field only appears in offsets produced when GTIDs are enabled. The "{@code
 ...
 * (since Jan 1, 1970) of the MySQL event; the message {@link Envelope envelopes} also have a
 * timestamp, but that timestamp is the <em>milliseconds</em> since since Jan 1, 1970.
 *
 * <p>Each change event envelope also includes the {@link #struct() source} struct that contains
 ...
 * restart from either just after the most recently completed transaction or the beginning of the
 * most recently started transaction (whichever appears later in the binlog).
 *
 * <p>Here's a JSON-like representation of the source for the metadata for an event that corresponds
...
public final class SourceInfo extends BaseSourceInfo {
     ...
     *
     * @return the name of the binary log file; null if it has not been {@link
     *     MySqlOffsetContext#setBinlogStartPoint(String, long) set}
     */
     ...
     *
     * @return the position within the binary log file; null if it has not been {@link
     *     MySqlOffsetContext#setBinlogStartPoint(String, long) set}
     */
    ...
    /**
     * Returns a string representation of the table(s) affected by the current event. Will only
     * represent more than a single table for events in the user-facing schema history topic for
     * certain types of DDL. Will be {@code null} for DDL events not applying to tables (CREATE
     * DATABASE etc.).
     */
    String table() {
    ...
    public static Document createDocumentFromOffset(Map<String, ?> offset) {
        ...
        // all of the offset keys represent int, long, or string types, so we don't need to worry
        // about references
