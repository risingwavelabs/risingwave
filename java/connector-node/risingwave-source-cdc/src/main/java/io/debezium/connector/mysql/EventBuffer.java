 ...
 * decide if the last event in transaction is either {@code ROLLBACK} or {@code COMMIT}. The
 ...
 *
 * <p>The reason for the buffering is that the binlog contains rolled back transactions in some
 * cases. E.g. that's the case when a temporary table is dropped (see DBZ-390). For rolled back
 * transactions we may not propagate any of the contained events, hence the buffering is applied.
 *
 * <p>The transaction start is identified by a {@code BEGIN} event. Transaction is ended either by
 * {@code COMMIT} event or by {@code XID} an event.
 *
 * <p>If there are more events that can fit to the buffer then:
 *
 * <ul>
 *   <li>Binlog position is recorded for the first event not fitting into the buffer
 *   <li>Binlog position is recorded for the commit event
 *   <li>Buffer content is sent to the final handler
 *   <li>Binlog position is rewound and all events between the above recorded positions are sent to
 *       the final handler
 * </ul>
 *
...
class EventBuffer {
    ...
    public void add(MySqlPartition partition, MySqlOffsetContext offsetContext, Event event) {
        ...
        if (isReplayingEventsBeyondBufferCapacity()) {
            streamingChangeEventSource.handleEvent(
                    partition, offsetContext, changeEventSourceContext, event);
            return;
        }
    ...
    /**
     * Adds an event to the buffer if there is a space available. Records binlog position for the
     * first event that does not fit for later replay.
     *
     * @param event
     */
    ...
    private void switchToBufferFullMode() {
        ...
        LOGGER.info(
                "Buffer full, will need to re-read part of the transaction from binlog from {}",
                largeTxNotBufferedPosition);
        streamingChangeEventSource.getMetrics().onLargeTransaction();
        // Position for TABLE_MAP is not stored by
        // com.github.shyiko.mysql.binlog.BinaryLogClient.updateClientBinlogFilenameAndPosition(Event)
    ...
    private void beginTransaction(
        ...
        if (txStarted) {
            LOGGER.warn(
                    "New transaction started but the previous was not completed, processing the buffer");
    ...
    /**
     * Sends all events from the buffer int a final handler. For large transactions it executes
     * rewind of binlog reader back to the first event that was not stored in the buffer.
     *
    ...
    private void completeTransaction(
        ...
        for (Event e : buffer) {
            streamingChangeEventSource.handleEvent(
                    partition, offsetContext, changeEventSourceContext, e);
        }
        ...
        if (isInBufferFullMode()) {
            ...
            streamingChangeEventSource.rewindBinaryLogClient(
                    changeEventSourceContext, largeTxNotBufferedPosition);
        }
    ...
    /**
     * Cleans-up the buffer after the transaction is either thrown away or streamed into a Kafka
     * topic
     */
    private void clear() {
        buffer.clear();
