...
public abstract class AbstractHistoryRecordComparator extends HistoryRecordComparator {
     ...
     * where the offsets are given in JSON representation of the maps returned by {@link
     * MySqlOffsetContext#getOffset()}.
     *
     * <p>This logic makes a significant assumption: once a MySQL server/cluster has GTIDs enabled,
     * they will never be disabled. This is the only way to compare a position with a GTID to a
     * position without a GTID, and we conclude that any position with a GTID is *after* the
     * position without.
     *
     * <p>When both positions have GTIDs, then we compare the positions by using only the GTIDs. Of
     * course, if the GTIDs are the same, then we also look at whether they have snapshots enabled.
     *
     * @param recorded the position obtained from recorded history; never null
     * @param desired the desired position that we want to obtain, which should be after some
     *     recorded positions, at some recorded positions, and before other recorded positions;
     *     never null
     * @return {@code true} if the recorded position is at or before the desired position; or {@code
     *     false} otherwise
     */
    @Override
    @VisibleForTesting
    public boolean isPositionAtOrBefore(Document recorded, Document desired) {
        ...
        if (desiredGtidSetStr != null) {
            ...
            if (recordedGtidSetStr != null) {
                ...
                if (recordedGtidSet.equals(desiredGtidSet)) {
                    // They are exactly the same, which means the recorded position exactly matches
                    // the desired ...
                    if (!recorded.has(SourceInfo.SNAPSHOT_KEY)
                            && desired.has(SourceInfo.SNAPSHOT_KEY)) {
                        // the desired is in snapshot mode, but the recorded is not. So the recorded
                        // is *after* the desired ...
                        return false;
                    }
                    // In all other cases (even when recorded is in snapshot mode), recorded is
                    // before or at desired GTID.
                    // Now we need to compare how many events in that transaction we've already
                    // completed ...
                    int recordedEventCount =
                            recorded.getInteger(MySqlOffsetContext.EVENTS_TO_SKIP_OFFSET_KEY, 0);
                    int desiredEventCount =
                            desired.getInteger(MySqlOffsetContext.EVENTS_TO_SKIP_OFFSET_KEY, 0);
                ...
                // The GTIDs are not an exact match, so figure out if recorded is a subset of the
                // desired ...
                return recordedGtidSet.isContainedWithin(desiredGtidSet);
            }
            // The desired position did use GTIDs while the recorded did not use GTIDs. So, we
            // assume that the
            // recorded position is older since GTIDs are often enabled but rarely disabled. And if
            // they are disabled,
            // it is likely that the desired position would not include GTIDs as we would be trying
            // to read the binlog of a
            // server that no longer has GTIDs. And if they are enabled, disabled, and re-enabled,
            // per
            // https://dev.mysql.com/doc/refman/8.2/en/replication-gtids-failover.html all properly
            ...
            // The recorded has a GTID but the desired does not, so per the previous paragraph we
        ...
        BinlogFilename recordedFilename =
                BinlogFilename.of(recorded.getString(SourceInfo.BINLOG_FILENAME_OFFSET_KEY));
        BinlogFilename desiredFilename =
                BinlogFilename.of(desired.getString(SourceInfo.BINLOG_FILENAME_OFFSET_KEY));
    ...
    private static class BinlogFilename implements Comparable<BinlogFilename> {
        ...
        public static BinlogFilename of(String filename) {
            ...
            if (index == -1) {
                throw new IllegalArgumentException(
                        "Filename does not have an extension: " + filename);
            }
            ...
            try {
                ...
                throw new IllegalArgumentException(
                        "Can't parse binlog filename extension: " + filename, e);
            }
        ...
        public int compareTo(BinlogFilename other) {
            if (!baseName.equals(other.baseName)) {
                throw new IllegalArgumentException(
                        "Can't compare binlog filenames with different base names");
            }
