...
public class MySqlConnectorTask extends BaseSourceTask<MySqlPartition, MySqlOffsetContext> {
    ...
    public ChangeEventSourceCoordinator<MySqlPartition, MySqlOffsetContext> start(
        ...
        validateAndLoadSchemaHistory(
                connectorConfig,
                connection::validateLogPosition,
                previousOffsets,
                schema,
                snapshotter);
    ...
    protected Iterable<Field> getAllConfigurationFields() {
        return MySqlConnectorConfig.allFields;
    }
    ...
    private void resetOffset(
            ...
            SignalProcessor<MySqlPartition, MySqlOffsetContext> signalProcessor) {
        boolean isKafkaChannelEnabled =
                connectorConfig.getEnabledChannels().contains(KafkaSignalChannel.CHANNEL_NAME);
        if (previousOffset != null
                && isKafkaChannelEnabled
                && connectorConfig.isReadOnlyConnection()) {
            KafkaSignalChannel kafkaSignal =
                    signalProcessor.getSignalChannel(KafkaSignalChannel.class);
            Long signalOffset =
                    connectorConfig
                            .getConnectorAdapter()
                            .getReadOnlyIncrementalSnapshotSignalOffset(previousOffset);
