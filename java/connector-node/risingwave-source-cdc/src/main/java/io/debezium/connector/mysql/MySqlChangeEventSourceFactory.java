...
public class MySqlChangeEventSourceFactory
    ...
    private final MainConnectionProvidingConnectionFactory<AbstractConnectorConnection>
            connectionFactory;
    ...
    // MySQL snapshot requires buffering to modify the last record in the snapshot as sometimes it
    // is
    // impossible to detect it till the snapshot is ended. Mainly when the last snapshotted table is
    // empty.
    ...
    public MySqlChangeEventSourceFactory(
    ...
    @Override
    public SnapshotChangeEventSource<MySqlPartition, MySqlOffsetContext>
    ...
    private void modifyAndFlushLastRecord(Function<SourceRecord, SourceRecord> modify)
    ...
    @Override
    public StreamingChangeEventSource<MySqlPartition, MySqlOffsetContext>
    ...
    public Optional<
                    IncrementalSnapshotChangeEventSource<
                            MySqlPartition, ? extends DataCollectionId>>
        ...
        if (configuration.isReadOnlyConnection()) {
            ...
            throw new UnsupportedOperationException(
                    "Read only connection requires GTID_MODE to be ON");
        }
        ...
        return Optional.of(
                new SignalBasedIncrementalSnapshotChangeEventSource<>(
                        configuration,
                        connectionFactory.mainConnection(),
                        dispatcher,
                        schema,
