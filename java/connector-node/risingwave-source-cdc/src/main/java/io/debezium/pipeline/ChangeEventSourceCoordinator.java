...
public class ChangeEventSourceCoordinator<P extends Partition, O extends OffsetContext> {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(ChangeEventSourceCoordinator.class);

    /**
     * Waiting period for the polling loop to finish. Will be applied twice, once gracefully, once
     * forcefully.
     */
    ...
    public ChangeEventSourceCoordinator(
        ...
        this.blockingSnapshotExecutor =
                Threads.newSingleThreadExecutor(
                        connectorType, connectorConfig.getLogicalName(), "blocking-snapshot");
                        ...
                        try {
                            ...
                            snapshotSource =
                                    changeEventSourceFactory.getSnapshotChangeEventSource(
                                            snapshotMetrics, notificationService);
    ...
    protected void registerSignalActionsAndStartProcessor(
            SignalProcessor<P, O> signalProcessor,
        ...
        List<SignalActionProvider> actionProviders =
                StreamSupport.stream(
                                ServiceLoader.load(SignalActionProvider.class).spliterator(), false)
                ...
                .map(
                        provider ->
                                provider.createActions(
                                        dispatcher, changeEventSourceCoordinator, connectorConfig))
    ...
    public Optional<SignalProcessor<P, O>> getSignalProcessor(
            Offsets<P, O> previousOffset) { // Signal processing only work with one partition
        return previousOffset == null || previousOffset.getOffsets().size() == 1
                ? Optional.ofNullable(signalProcessor)
                : Optional.empty();
    }
    ...
    protected void executeChangeEventSources(
        ...
        getSignalProcessor(previousOffsets)
                .ifPresent(s -> s.setContext(snapshotResult.getOffset()));
    ...
    /**
     * Delays streaming execution as per the {@link CommonConnectorConfig#STREAMING_DELAY_MS}
     * parameter.
     */
    protected void delayStreamingIfNeeded(ChangeEventSourceContext context)
            throws InterruptedException {
        ...
        Metronome metronome =
                Metronome.parker(ConfigurationDefaults.RETURN_CONTROL_INTERVAL, Clock.SYSTEM);
        ...
        while (!timer.expired()) {
            ...
            LOGGER.info(
                    "The connector will wait for {}s before initiating streaming",
                    timer.remaining().getSeconds());
            metronome.pause();
        }
    }

    public void doBlockingSnapshot(
            P partition, OffsetContext offsetContext, SnapshotConfiguration snapshotConfiguration) {

        blockingSnapshotExecutor.submit(
                () -> {
                    previousLogContext.set(
                            taskContext.configureLoggingContext("streaming", partition));
                    ...
                    try {
                        ...
                        SnapshottingTask snapshottingTask =
                                snapshotSource.getBlockingSnapshottingTask(
                                        partition, (O) offsetContext, snapshotConfiguration);
                        try {
                            doSnapshot(
                                    snapshotSource,
                                    context,
    ...
    protected SnapshotResult<O> doSnapshot(
            SnapshotChangeEventSource<P, O> snapshotSource,
            ChangeEventSourceContext context,
            P partition,
            O previousOffset)
        ...
        SnapshottingTask snapshottingTask =
                snapshotSource.getSnapshottingTask(partition, previousOffset);
    ...
    protected SnapshotResult<O> doSnapshot(
        ...
        CatchUpStreamingResult catchUpStreamingResult =
                executeCatchUpStreaming(context, snapshotSource, partition, previousOffset);
    ...
    protected void streamEvents(ChangeEventSourceContext context, P partition, O offsetContext)
            throws InterruptedException {
        try {
            ...
            if (snapshotterService != null && !snapshotterService.getSnapshotter().shouldStream()) {
                LOGGER.info(
                        ...
                        snapshotterService.getSnapshotter().name());
                return;
            }
    ...
    protected void initStreamEvents(P partition, O offsetContext) throws InterruptedException {
        ...
        final Optional<IncrementalSnapshotChangeEventSource<P, ? extends DataCollectionId>>
                incrementalSnapshotChangeEventSource =
                        changeEventSourceFactory.getIncrementalSnapshotChangeEventSource(
                                offsetContext,
                                snapshotMetrics,
                                snapshotMetrics,
                                notificationService);
        ...
        if (eventDispatcher.heartbeatsEnabled()) {
            if (offsetContext == null || offsetContext.getOffset() == null || offsetContext.getOffset().isEmpty()) {
    ...
    public synchronized void stop() throws InterruptedException {
        ...
        try {
            ...
            final long shutdownWaitTimeout =
                    connectorConfig.getExecutorShutdownTimeout().toMillis();
            boolean isShutdown =
                    executor.awaitTermination(shutdownWaitTimeout, TimeUnit.MILLISECONDS);
            boolean isBlockingSnapshotShutdown =
                    blockingSnapshotExecutor.awaitTermination(
                            shutdownWaitTimeout, TimeUnit.MILLISECONDS);
            ...
            if (!isBlockingSnapshotShutdown) {
                LOGGER.warn(
                        "Coordinator didn't stop in the expected time, shutting down blocking snapshot executor now");
    ...
    public class ChangeEventSourceContextImpl implements ChangeEventSourceContext {
        ...
        public void waitStreamingPaused() throws InterruptedException {
            ...
            try {
                while (streaming) {
                    LOGGER.trace(
                            "Requested a blocking snapshot. Waiting for streaming to be paused.");
