...
public class MariaDbConnection extends AbstractConnectorConnection {
    ...
    public MariaDbConnection(
            MariaDbConnectionConfiguration connectionConfig, MySqlFieldReader fieldReader) {
    ...
    public GtidSet knownGtidSet() {
        // MariaDB does not store the executed GTID details in the SHOW MASTER STATUS output like
        // MySQL;
        // however, instead makes this information available as a variable. The GTID_BINLOG_POS
        // gives
        // the current GTID position of the binary log and can therefore be considered the
        // equivalent to
    ...
    public GtidSet purgedGtidSet() {
        // The MariaDB community mentioned we could get the purged GTID values from the
        // GTID_LIST_EVENT; however,
        // this value is only available after we connect and would require a temporary binlog
        // connection to get
                ...
                relevantAvailableServerGtidSet
                        .retainAll(
                                serverId -> {
                                    ...
                                    final MariaDbGtid compliantGtid =
                                            MariaDbGtid.parse(serverId + "-0");
