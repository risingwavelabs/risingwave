...
public abstract class AbstractConnectorConnection extends JdbcConnection {
     ...
     */
    public AbstractConnectorConnection(
            ConnectionConfiguration configuration, MySqlFieldReader fieldReader) {
    ...
    /**
     * Determine whether the current user has the named privilege. If the user has the "ALL"
     * privilege, this method will always return {@code true}.
     *
    ...
    public boolean userHasPrivileges(String grantName) {
        try {
            ...
            throw new DebeziumException(
                    "Unexpected error while connecting to database and looking at privileges for current user: ",
                    e);
        }
    }
    ...
    public String earliestBinlogFilename() {
        ...
        try {
            ...
            throw new DebeziumException(
                    "Unexpected error while connecting to the database and looking for binary logs: ",
                    e);
        }
     ...
     * @return an optional long-value that may be empty if no data is available or an exception
     *     occurred
     */
    ...
    /**
     * Determines whether the binlog format used by the database server is {@code
     * binlog_row_image='FULL'}.
     *
     * @return {@code true} if the {@code binlog_row_image} is set to {@code FULL}, {@code false}
     *     otherwise
     */
    public boolean isBinlogRowImageFull() {
        try {
            ...
            throw new DebeziumException(
                    "Unexpected error while connecting to the database and looking at BINLOG_ROW_IMAGE mode: ",
                    e);
        }
    }
     ...
     *
     * @return {@code true} if the server's {@code binlog_format} is set to {@code ROW}, {@code
     *     false} otherwise
     */
    public boolean isBinlogFormatRow() {
        try {
            ...
            throw new DebeziumException(
                    "Unexpected error while connecting to the database and looking at BINLOG_FORMAT mode: ",
                    e);
        }
    }
    ...
    /**
     * Determine whether the binlog position as set in the offset details is available on the
     * server.
     *
    ...
    public boolean isBinlogPositionAvailable(
            MySqlConnectorConfig config, String gtid, String binlogFileName) {
        if (gtid != null) {
            ...
            if (availableGtidSet.isEmpty()) {
                ...
                LOGGER.info(
                        "Connector used GTIDs previously, but server does not know of any GTIDs or they are not enabled");
                return false;
            }
            ...
            if (gtidSet.isContainedWithin(availableGtidSet)) {
                LOGGER.info(
                        "The current GTID set '{}' does not contain the GTID set '{}' required by the connector",
                        availableGtidSet,
                        gtidSet);
                ...
                final GtidSet nonPurgedGtidSetTemplate =
                        subtractGtidSet(gtidSetToReplicate, purgedGtidSet);
                LOGGER.info(
                        "GTIDs known by the server but not processed yet '{}', for replication are available only '{}'",
                        gtidSetToReplicate,
                        nonPurgedGtidSetTemplate);
            ...
            LOGGER.info(
                    "Connector last known GTIDs are '{}', but server has '{}'",
                    gtidSet,
                    availableGtidSet);
            return false;
        }
        ...
        if (!found && LOGGER.isInfoEnabled()) {
            LOGGER.info(
                    "Connector requires binlog file '{}', but server only has {}",
                    binlogFileName,
                    String.join(", ", logNames));
        } else if (found && LOGGER.isInfoEnabled()) {
            LOGGER.info(
                    "Server has the binlog file '{}' required by the connector", binlogFileName);
        }
     ...
     *
     * @return the string representation of the most recent executed GTID set or position; never
     *     null but will be empty if the server does not support or has not processed any GTID
     */
     ...
     */
    public abstract GtidSet filterGtidSet(
