...
public class MySqlReadOnlyIncrementalSnapshotChangeEventSource<T extends DataCollectionId>
    ...
    private static final String SHOW_BINARY_LOG_STMT = "SHOW BINARY LOG STATUS";
    ...
    private void getExecutedGtidSet(Consumer<MySqlGtidSet> watermark) {
        try {
            /* patch code */
            // MySQL 8.4+ requires using "SHOW BINARY LOG STATUS" instead of "SHOW MASTER STATUS"
            // to retrieve executed GTID set for watermarking.
            final String sql = getShowStatusCommand();
    ...
    private String getShowStatusCommand() {
        /* patch code */
        // Version detection and statement selection for MySQL >= 8.4.
        try {
            String version =
                    jdbcConnection.queryAndMap(
                            "SELECT VERSION()", rs -> rs.next() ? rs.getString(1) : null);
            if (version == null) {
                return SHOW_MASTER_STMT;
            }
            String[] parts = version.split("\\.");
            int major = parts.length > 0 ? Integer.parseInt(parts[0]) : 0;
            int minor = parts.length > 1 ? Integer.parseInt(parts[1]) : 0;
            return (major > 8 || (major == 8 && minor >= 4))
                    ? SHOW_BINARY_LOG_STMT
                    : SHOW_MASTER_STMT;
        } catch (SQLException | NumberFormatException ex) {
            LOGGER.warn("Failed to parse MySQL version, default to SHOW MASTER STATUS", ex);
            return SHOW_MASTER_STMT;
        }
        /* patch code */
    }
    ...
    @Override
    protected void emitWindowClose(MySqlPartition partition, OffsetContext offsetContext)
            throws InterruptedException {
        updateHighWatermark();
    ...
    public void addDataCollectionNamesToSnapshot(
            SignalPayload<MySqlPartition> signalPayload,
    ...
    public void stopSnapshot(
            ...
            List<String> dataCollectionIds) {
        {
            ...
            getContext()
                    .setSignalOffset((Long) additionalData.get(KafkaSignalChannel.CHANNEL_OFFSET));
        }
    }
