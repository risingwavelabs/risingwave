 ...
 */
public class MariaDbReadOnlyIncrementalSnapshotChangeEventSource<T extends DataCollectionId>
        extends AbstractIncrementalSnapshotChangeEventSource<MySqlPartition, T> {

    private static final Logger LOGGER =
            LoggerFactory.getLogger(MariaDbReadOnlyIncrementalSnapshotChangeEventSource.class);
    ...
    private void getExecutedGtidSet(Consumer<MariaDbGtidSet> watermark) {
        try {
    ...
    protected void emitWindowClose(MySqlPartition partition, OffsetContext offsetContext)
            throws InterruptedException {
        updateHighWatermark();
        if (getContext().serverStreamSetChanged()) {
    ...
    private MariaDbReadOnlyIncrementalSnapshotContext<T> getContext() {
        return (MariaDbReadOnlyIncrementalSnapshotContext<T>) context;
    }
}
