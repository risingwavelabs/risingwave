...
public class MySqlConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
    ...
    public enum SnapshotMode implements EnumeratedValue {
         ...
         * binlog. This should be used with care, but it is very useful when the change event
         * consumers need only the changes from the point in time the snapshot is made (and doesn't
         * care about any state or changes prior to this point).
         *
        ...
        /**
         * Perform a snapshot of only the database schemas (without data) and then begin reading the
         * binlog at the current binlog position. This can be used for recovery only if the
         * connector has existing offsets and the schema.history.internal.kafka.topic does not exist
        ...
        /**
         * Never perform a snapshot and only read the binlog. This assumes the binlog contains all
         * the history of those databases and tables that will be captured.
         */
        NEVER("never"),
    ...
    public enum SnapshotLockingMode implements EnumeratedValue {
         ...
         *
         * <p>Replaces deprecated configuration option snapshot.locking.minimal with a value of
         * false.
         */
        EXTENDED("extended"),

        /**
         * The connector holds the global read lock for just the initial portion of the snapshot
         * while the connector reads the database schemas and other metadata. The remaining work in
         * a snapshot involves selecting all rows from each table, and this can be done in a
         * consistent fashion using the REPEATABLE READ transaction even when the global read lock
         * is no longer held and while other MySQL clients are updating the database.
         *
         * <p>Replaces deprecated configuration option snapshot.locking.minimal with a value of
         * true.
         */
        ...
        /**
         * The connector holds a (Percona-specific) backup lock for just the initial portion of the
         * snapshot while the connector reads the database schemas and other metadata. This lock
         * will only block DDL and DML on non-transactional tables (MyISAM etc.). The remaining work
         * in a snapshot involves selecting all rows from each table, and this can be done in a
         * consistent fashion using the REPEATABLE READ transaction even when the global read lock
         * is no longer held and while other MySQL clients are updating the database.
         */
        ...
        /**
         * This mode will avoid using ANY table locks during the snapshot process. This mode can
         * only be used with SnapShotMode set to schema_only or schema_only_recovery.
         */
        NONE("none"),
    ...
    public enum ConnectorAdapterMode implements EnumeratedValue {
        /**
         * Expects the target database to be MySQL using the MySQL driver. This should also be used
         * if the target database is MySQL compliant but isn't MariaDB.
         */
        MYSQL("mysql") {
            @Override
    ...
    /**
     * {@link Integer#MIN_VALUE Minimum value} used for fetch size hint. See <a
     * href="https://issues.jboss.org/browse/DBZ-94">DBZ-94</a> for details.
     *
     * <p>This fetch size is not valid for MariaDB and per <a
     * href="https://jira.mariadb.org/browse/CONJ-977">CONJ-997</a> they believe that the MySQL
     * implementation for this is not according to the spec. Starting with MariaDB driver's 3.x+,
     * this value cannot be negative. See the configuration method {@link
     * #resolveDefaultFetchSize(Configuration)} for more details when MariaDB is enabled.
     */
    ...
    /**
     * Default size of the binlog buffer used for examining transactions and deciding whether to
     * propagate them or not. A size of 0 disables the buffer, all events will be passed on directly
     * as they are passed by the binlog client.
     */
    ...
    public static final Field ON_CONNECT_STATEMENTS =
            Field.create("database.initial.statements")
    ...
    public static final Field SERVER_ID =
            Field.create("database.server.id")
    ...
    public static final Field SERVER_ID_OFFSET =
            Field.create("database.server.id.offset")
                    ...
                    .withDefault(10000L)
                    .withDescription(
                            "Only relevant if parallel snapshotting is configured. During "
    ...
    public static final Field TABLES_IGNORE_BUILTIN =
            RelationalDatabaseConnectorConfig.TABLE_IGNORE_BUILTIN.withDependents(
                    DATABASE_INCLUDE_LIST_NAME);

    public static final Field JDBC_DRIVER =
            Field.create(DATABASE_CONFIG_PREFIX + "jdbc.driver")
    ...
    public static final Field JDBC_PROTOCOL =
            Field.create(DATABASE_CONFIG_PREFIX + "protocol")
    ...
    /**
     * A comma-separated list of regular expressions that match source UUIDs in the GTID set used to
     * find the binlog position in the MySQL server. Only the GTID ranges that have sources matching
     * one of these include patterns will be used. Must not be used with {@link
     * #GTID_SOURCE_EXCLUDES}.
     */
    public static final Field GTID_SOURCE_INCLUDES =
            Field.create("gtid.source.includes")
     ...
     * A comma-separated list of regular expressions that match source UUIDs in the GTID set used to
     * find the binlog position in the MySQL server. Only the GTID ranges that have sources matching
     * none of these exclude patterns will be used. Must not be used with {@link
     * #GTID_SOURCE_INCLUDES}.
     */
    public static final Field GTID_SOURCE_EXCLUDES =
            Field.create("gtid.source.excludes")
     ...
     * on MySQL servers with UUIDs matching the filters defined by the {@link #GTID_SOURCE_INCLUDES}
     * or {@link #GTID_SOURCE_EXCLUDES} configuration options, if they are specified.
     *
     * <p>Defaults to true.
     *
     * <p>When true, either {@link #GTID_SOURCE_INCLUDES} or {@link #GTID_SOURCE_EXCLUDES} must be
     * set.
     */
    public static final Field GTID_SOURCE_FILTER_DML_EVENTS =
            Field.create("gtid.source.filter.dml.events")
    ...
    public static final Field KEEP_ALIVE =
            Field.create("connect.keep.alive")
    ...
    public static final Field KEEP_ALIVE_INTERVAL_MS =
            Field.create("connect.keep.alive.interval.ms")
    ...
    public static final Field ROW_COUNT_FOR_STREAMING_RESULT_SETS =
            Field.create("min.row.count.to.stream.results")
                    ...
                    .withImportance(Importance.LOW)
                    .withDescription(
                            "The number of rows a table must contain to stream results rather than pull "
    ...
    public static final Field BUFFER_SIZE_FOR_BINLOG_READER =
            Field.create("binlog.buffer.size")
    ...
    public static final Field TOPIC_NAMING_STRATEGY =
            Field.create("topic.naming.strategy")
                    .withDisplayName("Topic naming strategy class")
                    .withType(Type.CLASS)
                    .withWidth(Width.MEDIUM)
                    .withImportance(Importance.MEDIUM)
                    .withDescription(
                            "The name of the TopicNamingStrategy class that should be used to determine the topic name "
    ...
    public static final Field SNAPSHOT_MODE =
            Field.create(SNAPSHOT_MODE_PROPERTY_NAME)
    ...
    public static final Field SNAPSHOT_LOCKING_MODE =
            Field.create(SNAPSHOT_LOCKING_MODE_PROPERTY_NAME)
    ...
    public static final Field SNAPSHOT_NEW_TABLES =
            Field.create("snapshot.new.tables")
    ...
    public static final Field CONNECTOR_ADAPTER =
            Field.create("connector.adapter")
    ...
    public static final Field SOURCE_INFO_STRUCT_MAKER =
            CommonConnectorConfig.SOURCE_INFO_STRUCT_MAKER.withDefault(
                    MySqlSourceInfoStructMaker.class.getName());
    ...
    protected static final Set<String> BUILT_IN_DB_NAMES =
            Collect.unmodifiableSet("mysql", "performance_schema", "sys", "information_schema");
    ...
    public MySqlConnectorConfig(Configuration config) {
        ...
        this.snapshotNewTables =
                SnapshotNewTables.parse(
                        snapshotNewTables,
                        MySqlConnectorConfig.SNAPSHOT_NEW_TABLES.defaultValueAsString());

        final String inconsistentSchemaFailureHandlingMode =
                config.getString(MySqlConnectorConfig.INCONSISTENT_SCHEMA_HANDLING_MODE);
        this.inconsistentSchemaFailureHandlingMode =
                EventProcessingFailureHandlingMode.parse(inconsistentSchemaFailureHandlingMode);
        ...
        this.gtidSourceFilter =
                gtidSetIncludes != null
                        ? Predicates.includesUuids(gtidSetIncludes)
                        : (gtidSetExcludes != null
                                ? Predicates.excludesUuids(gtidSetExcludes)
                                : null);

        this.storeOnlyCapturedDatabasesDdl = config.getBoolean(STORE_ONLY_CAPTURED_DATABASES_DDL);

        // This should always be last to guarantee the full configuration is passed in the
        // constructor
        this.connectorAdapter =
                ConnectorAdapterMode.parse(config.getString(CONNECTOR_ADAPTER)).getAdapter(this);
    }
    ...
    private static int validateGtidSetExcludes(
            Configuration config, Field field, ValidationOutput problems) {
        ...
        if (includes != null && excludes != null) {
            problems.accept(
                    GTID_SOURCE_EXCLUDES,
                    excludes,
                    "Included GTID source UUIDs are already specified");
            return 1;
        }
        return 0;
    }
    ...
    /**
     * Validate the new snapshot.locking.mode configuration, which replaces
     * snapshot.minimal.locking.
     *
     * <p>If minimal.locking is explicitly defined and locking.mode is NOT explicitly defined: -
     * coerce minimal.locking into the new snap.locking.mode property.
     *
     * <p>If minimal.locking is NOT explicitly defined and locking.mode IS explicitly defined: - use
     * new locking.mode property.
     *
     * <p>If BOTH minimal.locking and locking.mode ARE defined: - Throw a validation error.
     */
    private static int validateSnapshotLockingMode(
            Configuration config, Field field, ValidationOutput problems) {
        ...
        if (config.hasKey(SNAPSHOT_LOCKING_MODE.name())) {
            ...
            if (lockingModeValue == null) {
                problems.accept(
                        SNAPSHOT_LOCKING_MODE,
                        lockingModeValue,
                        "Must be a valid snapshot.locking.mode value");
                return 1;
            }
        }
     ...
     *
     * <p>If {@code adaptive} is specified, this option has the potential to cause overflow which is
     * why the option was deprecated and no longer supported for this connector.
     */
    ...
    /**
     * Get the predicate function that will return {@code true} if a GTID source is to be included,
     * or {@code false} if a GTID source is to be excluded.
     *
