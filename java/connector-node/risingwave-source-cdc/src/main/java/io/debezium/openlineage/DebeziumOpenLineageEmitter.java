 ...
 *
 * <p>This class serves as a facade for the underlying OpenLineage integration, providing static
 * methods for initializing the emitter and emitting lineage events at various points in the
 * Debezium connector lifecycle. The implementation uses a thread-safe approach to ensure proper
 * initialization across multiple threads.
 *
 * <p>The emitter will only be active if OpenLineage integration is enabled in the configuration.
...
public class DebeziumOpenLineageEmitter {
    ...
    private static final ServiceLoader<LineageEmitterFactory> lineageEmitterFactory =
            ServiceLoader.load(LineageEmitterFactory.class);
    private static final Object SERVICE_LOADER_LOCK = new Object();

    // Thread-safe map to store emitters per connector
    private static final ConcurrentHashMap<String, LineageEmitter> emitters =
     ...
     *
     * <p>This method must be called before any emission methods are used. It sets up the
    ...
    public static void init(Map<String, String> configuration, String connectorTypeName) {
        ...
        LOGGER.debug(
                "Calling init for connector {} and config {}", connectorTypeName, configuration);
        ...
        LineageEmitter emitter =
                emitters.computeIfAbsent(
                        ...
                        key -> {
                             ...
                             */
                            synchronized (
                                    SERVICE_LOADER_LOCK) { // This required for connectors with
                                // multiple tasks because the
                                // ServiceLoader is not thread-safe
                                return lineageEmitterFactory.stream()
                                        .findFirst()
    ...
    /**
     * Removes the emitter for the specified connector. Should be called when a connector is stopped
     * or destroyed.
     *
    ...
    public static ConnectorContext connectorContext(
     ...
     *
     * <p>This method is typically used for error reporting.
     *
     * @param connectorContext The connector context
     * @param state The current state of the source task
    ...
    public static void emit(
            ConnectorContext connectorContext, DebeziumTaskState state, Throwable t) {
        getEmitter(connectorContext).emit(state, List.of(), t);
    }

    /**
     * Emits a lineage inputDatasetMetadata for the given source task state and table
     * inputDatasetMetadata.
     *
     * <p>This method is typically used for emitting input dataset lineage.
     *
     * @param connectorContext The connector context
     * @param state The current state of the source task
     ...
     *
     * <p>This method provides the most detailed lineage information, including both table metadata
    ...
    private static LineageEmitter getEmitter(ConnectorContext connectorContext) {
        if (isOpenLineageDisabled(connectorContext.config())) {
    ...
    private static boolean isOpenLineageDisabled(Map<String, String> configuration) {
        return !Boolean.parseBoolean(configuration.get(OPEN_LINEAGE_INTEGRATION_ENABLED));
    }
}
