 ...
 * {@code DESCRIBE CREATE TABLE}. This custom converter allows user to handle all {@code TINYINT(1)}
 * fields as {@code BOOLEAN} or provide a set of regexes to match only subset of tables/columns.
 *
...
public class TinyIntOneToBooleanConverter
    ...
    // recommend disabling this option for mysql8 since "show create table" not showing length of
    // tinyint unsigned type,
    // and specify the columns that need to be converted to "selector" property instead of
    // converting all columns based on type.
    public static final String LENGTH_CHECKER = "length.checker";

    private static final List<String> TINYINT_FAMILY =
            Collect.arrayListOf("TINYINT", "TINYINT UNSIGNED");

    private static final Logger LOGGER =
            LoggerFactory.getLogger(TinyIntOneToBooleanConverter.class);
    ...
    public void configure(Properties props) {
        ...
        if (!Strings.isNullOrEmpty(selectorConfig)) {
            selector =
                    Predicates.includes(
                            selectorConfig.trim(), x -> x.dataCollection() + "." + x.name());
        }
    ...
    public void converterFor(
        ...
        if (!TINYINT_FAMILY.contains(field.typeName().toUpperCase())
                || (lengthChecker && field.length().orElse(-1) != 1)
                || !selector.test(field)) {
            return;
        }
