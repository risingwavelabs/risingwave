...
public class MySqlOffsetContext extends CommonOffsetContext<SourceInfo> {
    ...
    private long serverIdFromOffset = 0;
    private long tsSecFromOffset = 0;
        ...
        if (sourceInfo.getCurrentGtid() != null) {
            ...
            this.transactionId =
                    String.format(
                            ...
                            this.restartBinlogFilename,
                            this.restartBinlogPosition);
        }
    }
    ...
    public static MySqlOffsetContext initial(MySqlConnectorConfig config) {
        final MySqlOffsetContext offset =
                new MySqlOffsetContext(config, false, false, new SourceInfo(config));
    ...
    public static class Loader implements OffsetContext.Loader<MySqlOffsetContext> {
        ...
        public MySqlOffsetContext load(Map<String, ?> offset) {
            boolean snapshot =
                    Boolean.TRUE.equals(offset.get(SourceInfo.SNAPSHOT_KEY))
    ...
    public void setBinlogStartPoint(String binlogFilename, long positionOfFirstEvent) {
        ...
        if (binlogFilename != null) {
            ...
            sourceInfo.setBinlogPosition(
                    sourceInfo.getCurrentBinlogFilename(), positionOfFirstEvent);
        }
    ...
    /**
     * Set the GTID set that captures all of the GTID transactions that have been completely
     * processed.
     *
     * @param gtidSet the string representation of the GTID set; may not be null, but may be an
     *     empty string if no GTIDs have been previously processed
     */
    ...
    public void setServerIdFromOffset(long serverId) {
        this.serverIdFromOffset = serverId;
    }

    public void setTsSecFromOffset(long tsSec) {
        this.tsSecFromOffset = tsSec;
    }
    ...
    /**
     * Record that a new GTID transaction has been started and has been included in the set of GTIDs
     * known to the MySQL server.
     *
     * @param gtid the string representation of a specific GTID that has been begun; may not be null
     * @param gtidSet the string representation of GTID set that includes the newly begun GTID; may
     *     not be null
     */
    public void startGtid(String gtid, String gtidSet) {
        ...
        if (gtidSet != null && !gtidSet.trim().isEmpty()) {
            // Remove all the newline chars that exist in the GTID set string ...
            String trimmedGtidSet = gtidSet.replace("\n", "").replace("\r", "");
            // Set the GTID set that we'll use if restarting BEFORE successful completion of the
            // events in this GTID ...
            this.restartGtidSet =
                    this.currentGtidSet != null ? this.currentGtidSet : trimmedGtidSet;
    ...
    public void setEventPosition(long positionOfCurrentEvent, long eventSizeInBytes) {
        ...
        // Don't set anything else, since the row numbers are set in the offset(int,int) method
    ...
    /**
     * Get the number of rows beyond the {@link #eventsToSkipUponRestart() last completely processed
     * event} to be skipped upon restart.
     *
     ...
     * compute the Kafka Connect offset that is be included in the produced change event describing
     * the row.
     *
     * <p>This method should always be called before {@link SourceInfo#struct()}.
     *
     * @param eventRowNumber the 0-based row number within the event for which the offset is to be
     *     produced
    ...
    public String toString() {
        return "MySqlOffsetContext [sourceInfoSchema="
                + sourceInfoSchema
                + ", sourceInfo="
                + sourceInfo
                + ", snapshotCompleted="
