...
public abstract class AbstractMySqlFieldReader implements MySqlFieldReader {
    ...
    private static final Set<String> TEXT_DATATYPES =
            Collect.unmodifiableSet("CHAR", "VARCHAR", "TEXT");
    ...
    @Override
    public Object readField(ResultSet rs, int columnIndex, Column column, Table table)
            throws SQLException {
        ...
        if (column.jdbcType() == Types.DATE) {
            try {
                ...
                logger.warn(
                        "Failed to read date value: '{}'. Trying default ResultSet implementation.",
                        e.getMessage());
        ...
        if (column.jdbcType() == Types.TIMESTAMP) {
            try {
                ...
                logger.warn(
                        "Failed to read timestamp value: '{}'. Trying default ResultSet implementation.",
                        e.getMessage());
                // If our field reader failed, let's try JDBC as the last resort.
                // Workaround for DBZ-5084.
                return rs.getObject(columnIndex);
        ...
        if (hasConverterForColumn && TEXT_DATATYPES.contains(column.typeName())) {
            try {
                ...
                if (columnData != null) {
                    return columnData.getBytes(
                            connectorConfig
                                    .getConnectorAdapter()
                                    .getJavaEncodingForCharSet(column.charsetName()));
                }
    ...
    protected abstract Object readDateField(
            ResultSet rs, int columnIndex, Column column, Table table) throws SQLException;

    protected abstract Object readTimestampField(
            ResultSet rs, int columnIndex, Column column, Table table) throws SQLException;
