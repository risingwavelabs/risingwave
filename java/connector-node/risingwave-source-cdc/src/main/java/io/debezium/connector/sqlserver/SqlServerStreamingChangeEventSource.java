...
public class SqlServerStreamingChangeEventSource
        ...
        this.effectiveOffset =
                offsetContext == null
                        ? new SqlServerOffsetContext(
                                connectorConfig, TxLogPosition.NULL, null, false)
                        : offsetContext;
    }
    ...
    public void execute(
        ...
        throw new UnsupportedOperationException(
                "Currently unsupported by the SQL Server connector");
    }
            ...
            if (context.isRunning()) {
                ...
                try {
                    ...
                    for (int i = 0; i < tables.length; i++) {
                        changeTables[i] =
                                new SqlServerChangeTablePointer(
                                        ...
                                        fromLsn,
                                        toLsn,
                                        connectorConfig.getStreamingFetchSize());
                        changeTables[i].next();
                    }
                    ...
                    while (true) {
                        ...
                        // After restart for changes that were executed before the last committed
                        // offset
                        ...
                        // After restart for change that was the last committed and operations in it
                        // before the last committed offset
                        if (!changesStoppedBeingMonotonic.get()
                                && tableWithSmallestLsn
                                                .getChangePosition()
                                                .compareTo(lastProcessedPositionOnStart)
                        ...
                        if (!schemaChangeCheckpoints.isEmpty()) {
                            if (tableWithSmallestLsn
                                            .getChangePosition()
                                            .getCommitLsn()
                                            .compareTo(schemaChangeCheckpoints.peek().getStartLsn())
                                    >= 0) {
                                migrateTable(partition, schemaChangeCheckpoints, offsetContext);
                            }
                        }
                        ...
                        if (operation == SqlServerChangeRecordEmitter.OP_UPDATE_BEFORE) {
                            if (!tableWithSmallestLsn.next()
                                    || tableWithSmallestLsn.getOperation()
                                            != SqlServerChangeRecordEmitter.OP_UPDATE_AFTER) {
                        ...
                        offsetContext.event(
                                tableWithSmallestLsn.getChangeTable().getSourceTableId(),
                                resultSet
                                        .getTimestamp(resultSet.getMetaData().getColumnCount())
                                        .toInstant());
                    ...
                    Arrays.stream(changeTables)
                            .filter(Objects::nonNull)
                            .forEach(ChangeTableResultSet::close);
                }
            }
        } catch (Exception e) {
