...
/**
 * This connector adapter provides a complete implementation for MariaDB assuming that the MariaDB
...
public class MariaDbConnectorAdapter implements ConnectorAdapter {
    ...
    public AbstractConnectorConnection createConnection(Configuration configuration) {
        MariaDbConnectionConfiguration connectionConfig =
                new MariaDbConnectionConfiguration(configuration);
        return new MariaDbConnection(
                connectionConfig, new MariaDbProtocolFieldReader(connectorConfig));
    }
    ...
    public void setOffsetContextBinlogPositionAndGtidDetailsForSnapshot(
        ...
        final String showMasterStmt = getShowMasterStatusCommand(connection);
        LOGGER.debug("Using SQL command: {} for binlog position query", showMasterStmt);
        connection.query(
                showMasterStmt,
                rs -> {
                    if (rs.next()) {
    ...
    public String getRecordingQueryFromEvent(EventData eventData) {
        ...
        // During incremental snapshots, the updates made to the signal table can lead to a case
        // where
        // the query stored in the offsets mismatches with the events being dispatched.
        //
        // IncrementalSnapshotIT#updates performs a series of updates where the pk/aa columns are
        // changed
        // i.e. [1,0 to [1,2000] and the ANNOTATE_ROWS event that contains the query specifies this
        // SQL:
        // "UPDATE `schema`.`a` SET aa = aa + 2000 WHERE pk > 0 and pk <= 10".
        //
        // The problem is that signal events do not seem to record a query string in the offsets for
        // MySQL
        // but this is recorded for MariaDB, and this causes there to be a mismatch of query string
        // values
        // with behavior expected for MySQL. For now, this tests the test to pass until we can
        // better
        // understand the root-cause.
        if (!Strings.isNullOrBlank(connectorConfig.getSignalingDataCollectionId())) {
            final TableId signalDataCollectionId =
                    TableId.parse(connectorConfig.getSignalingDataCollectionId());
    ...
    public Long getReadOnlyIncrementalSnapshotSignalOffset(MySqlOffsetContext previousOffsets) {
        return ((MariaDbReadOnlyIncrementalSnapshotContext<TableId>)
    ...
    /**
     * Get the appropriate SQL command based on MySQL version. MySQL 8.4+ uses "SHOW BINARY LOG
     * STATUS", older versions use "SHOW MASTER STATUS".
     */
            ...
            if (version != null) {
                ...
                if (parts.length >= 2) {
                    try {
                        ...
                        LOGGER.warn("Failed to parse MySQL version: {}", version, e);
                    }
                }
            }
        } catch (Exception e) {
            LOGGER.warn("Failed to get MySQL version, assuming older version", e);
        }
