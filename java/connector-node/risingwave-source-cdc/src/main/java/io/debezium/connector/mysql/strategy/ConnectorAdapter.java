...
/**
 * Provides the MySQL connector with an adapter pattern to support varied configurations between
 * MySQL and MariaDB and their drivers.
 *
...
public interface ConnectorAdapter {
    ...
    <T> IncrementalSnapshotContext<T> loadIncrementalSnapshotContextFromOffset(
            Map<String, ?> offset);

    Long getReadOnlyIncrementalSnapshotSignalOffset(MySqlOffsetContext previousOffsets);

    IncrementalSnapshotChangeEventSource<MySqlPartition, ? extends DataCollectionId>
