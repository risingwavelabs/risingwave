...
public abstract class BinlogHistoryRecordComparator extends HistoryRecordComparator {
    protected static final Logger LOGGER =
            LoggerFactory.getLogger(BinlogHistoryRecordComparator.class);
     ...
     * offset context.
     *
     * <p>This logic makes a significant assumption: once a server enables GTID, they are never
     * disabled. This is the only way to compare a position with a GTID to a position without a
     * GTID, and any change with a GTID is <em>after</em> positions without.
     *
     * <p>When both positions have GTIDs, the positions are compared using the GTIDs. If the GTID
     * values are identical, then we compare whether they have snapshots enabled.
     *
     ...
     *     recorded positions at some recorded positions, and before other recorded positions; never
     *     null
    ...
    public boolean isPositionAtOrBefore(Document recorded, Document desired) {
        LOGGER.debug("recorded: " + recorded);
        ...
        if (desiredGtid != null) {
            ...
            if (recordedGtid != null) {
                ...
                if (recordedGtidSet.equals(desiredGtidSet)) {
                    ...
                    if (!isSnapshot(recorded) && isSnapshot(desired)) {
                        // The desired is in snapshot mode, but the recorded is not, so its *after*
                        // the desired
                        return false;
                    }
                    ...
                    int recordedEventCount =
                            recorded.getInteger(BinlogOffsetContext.EVENTS_TO_SKIP_OFFSET_KEY, 0);
                    int desiredEventCount =
                            desired.getInteger(BinlogOffsetContext.EVENTS_TO_SKIP_OFFSET_KEY, 0);
            ...
            // Assume that the recorded position is older since GTIDs are often enabled but rarely
            // disabled.
            // If they are disabled, it is likely that the desired position would not include GTIDs
            // as we
            // would be reading a binlog of a server that no longer has GTIDs. If they are enabled,
            // disabled,
            // and then re-enabled, per
            // https://dev.mysql.com/doc/refman/8.2/en/replication-gtids-failover.html,
            // all properly configured replicas that use GTIDs should always have the complete set
            // of GTIDs
            // copied from the primary, in which case we know that recorded not having GTID is
            // before desired.
            return true;
    ...
    protected static class BinlogFileName implements Comparable<BinlogFileName> {
        ...
        public static BinlogFileName of(String fileName) {
            ...
            try {
                ...
                throw new IllegalArgumentException(
                        "Cannot parse binlog filename extension: " + fileName, e);
            }
