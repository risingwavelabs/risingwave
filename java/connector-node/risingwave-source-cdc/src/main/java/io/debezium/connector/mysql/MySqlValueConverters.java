...
/**
 * MySQL-specific customization of the conversions from JDBC values obtained from the MySQL binlog
 * client library.
 *
 * <p>This class always uses UTC for the default time zone when converting values without timezone
 * information to values that require timezones. This is because MySQL {@code TIMESTAMP} values are
 * always <a href="https://dev.mysql.com/doc/refman/8.2/en/datetime.html">stored in UTC</a> (unlike
 * {@code DATETIME} values) and are replicated in this form. Meanwhile, the MySQL Binlog Client
 * library will {@link AbstractRowsEventDataDeserializer deserialize} these as {@link
 * java.sql.Timestamp} values that have no timezone and, therefore, are presumed to be in UTC. When
 * the column is properly marked with a {@link Types#TIMESTAMP_WITH_TIMEZONE} type, the converters
 * will need to convert that {@link java.sql.Timestamp} value into an {@link OffsetDateTime} using
 * the default time zone, which always is UTC.
 *
...
public class MySqlValueConverters extends JdbcValueConverters {
     ...
     * href="https://dev.mysql.com/doc/refman/8.2/en/two-digit-years.html">ambiguous</a> 2-digit
    ...
    protected List<String> extractEnumAndSetOptions(Column column) {
        ...
        return column.enumValues(); // TODO: Fix ANTLR dependency
    }
