...
public class MariaDbGtidSet implements GtidSet {
                    ...
                    .forEach(
                            gtid -> {
                                if (MariadbGtidSet.isMariaGtidSet(gtid)) {
                                    ...
                                    MariaDbGtidStreamId streamId =
                                            new MariaDbGtidStreamId(mariaGtid);
    ...
    public GtidSet retainAll(Predicate<String> sourceFilter) {
        ...
        Map<MariaDbGtidStreamId, MariaDbStreamSet> newSets =
                streamSets.entrySet().stream()
                ...
                streamSets.entrySet().stream()
                        .filter(
                                entry ->
                                        !entry.getValue()
                                                .isContainedWith(
                                                        theOther.forStreamId(entry.getKey())))
                        .map(
                                entry ->
    ...
    public String toString() {
        return streamSets.values().stream()
                .map(MariaDbStreamSet::toString)
    ...
    public static class MariaDbGtidStreamId implements Comparable<MariaDbGtidStreamId> {
        ...
        @Override
        public String toString() {
            return "MariaDbGtidStreamId{" + "domainId=" + domainId + ", serverId=" + serverId + '}';
        }
    }
    ...
    public static class MariaDbStreamSet extends TreeSet<MariaDbGtid> {
        ...
        public boolean isAllBefore(MariaDbStreamSet other) {
            final Long otherMinSequence =
                    other.stream().mapToLong(MariaDbGtid::getSequence).min().getAsLong();
        ...
        public MariaDbStreamSet subtract(MariaDbStreamSet other) {
            ...
            streamSet.addAll(
                    stream().filter(gtid -> !other.contains(gtid)).collect(Collectors.toSet()));
            return streamSet;
        }
