...
/**
 * A Kafka Connect source connector that creates tasks that read the MySQL binary log and generate
 * the corresponding data change events.
 *
 * <h2>Configuration</h2>
 *
 * <p>This connector is configured with the set of properties described in {@link
...
public class MySqlConnector extends RelationalBaseSourceConnector {
        ...
        try (AbstractConnectorConnection connection = adapter.createConnection(config)) {
            try {
                ...
                LOGGER.info(
                        ...
                        connection.connectionConfig().username());
    ...
    protected Map<String, ConfigValue> validateAllFields(Configuration config) {
        return config.validate(MySqlConnectorConfig.allFields);
    }
    ...
    public List<TableId> getMatchingCollections(Configuration config) {
        ...
        try (AbstractConnectorConnection connection = adapter(config).createConnection(config)) {
            ...
            for (String databaseName : databaseNames) {
                ...
                tables.addAll(
                        connection
                                .readTableNames(databaseName, null, null, new String[] {"TABLE"})
                                .stream()
                                .filter(
                                        tableId ->
                                                connectorConfig
