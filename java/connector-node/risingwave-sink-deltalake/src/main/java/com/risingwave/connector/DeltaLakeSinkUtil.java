...
class DeltaLakeSinkUtil {
  public static void checkSchema(TableSchema tableSchema, StructType schema) {
    if (schema == null) {
      throw INVALID_ARGUMENT.withDescription("Schema of delta table is null").asRuntimeException();
    }
    ...
    for (String tableColumnName : tableColumnTypes.keySet()) {
      if (!columnTypes.containsKey(tableColumnName)) {
        throw INVALID_ARGUMENT
            .withDescription(String.format("Delta table should contain column %s", tableColumnName))
            .asRuntimeException();
      }
    }
    ...
    for (Map.Entry<String, StructField> column : columnTypes.entrySet()) {
      ...
      if (tableColumnType == null) {
        if (!column.getValue().isNullable()) {
          throw INVALID_ARGUMENT
              .withDescription("Column in delta table which is not in sink should be nullable")
              .asRuntimeException();
        }
      } else {
        if (!checkType(tableColumnType, column.getValue().getDataType())) {
          throw INVALID_ARGUMENT
              .withDescription(String.format("Type of column %s is different", column.getKey()))
              .asRuntimeException();
        }
      }
    }
    ...
    switch (typeName) {
      ...
      case TIMESTAMPTZ:
        throw INVALID_ARGUMENT
            ...
            .asRuntimeException();
      default:
        throw INVALID_ARGUMENT.withDescription("unspecified type" + typeName).asRuntimeException();
    }
  }
