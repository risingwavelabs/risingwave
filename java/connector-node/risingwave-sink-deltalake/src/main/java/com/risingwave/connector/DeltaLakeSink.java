...
public class DeltaLakeSink extends SinkWriterBase {
  ...
  public void write(Iterator<SinkRow> rows) {
    if (this.parquetWriter == null) {
      this.dataFileNum += 1;
      this.parquetPath =
          new Path(this.log.getPath(), String.format("%s-%d.parquet", this.uuid, this.dataFileNum));
      try {
        ...
        case INSERT:
          ...
          for (int i = 0; i < this.sinkSchema.getFields().size(); i++) {
            ...
            if (row.get(i) instanceof LocalDateTime) {
              values = ((LocalDateTime) row.get(i)).toInstant(ZoneOffset.UTC).toEpochMilli();
            } else {
              values = row.get(i);
            }
            record.put(i, values);
    ...
    try {
      ...
      addFile =
          new AddFile(
              ...
              new HashMap<>(),
              this.parquetPath.getFileSystem(conf).getContentSummary(this.parquetPath).getLength(),
