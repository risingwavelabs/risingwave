...
public class DbzConnectorConfig {
  ...
  private static Map<String, String> extractDebeziumProperties(Map<String, String> userProperties) {
    ...
    if (source == SourceTypeE.MYSQL) {
      ...
      if (isCdcBackfill) {
        ...
        if (null != startOffset && !startOffset.isBlank()) {
          ...
          mysqlProps.setProperty(ConfigurableOffsetBackingStore.OFFSET_STATE_VALUE, startOffset);
        } else {
      ...
      var pubAutoCreate =
          Boolean.parseBoolean(userProps.getOrDefault(DbzConnectorConfig.PG_PUB_CREATE, "true"));
      if (!pubAutoCreate) {
      ...
      if (isCdcBackfill) {
        ...
        if (null != startOffset && !startOffset.isBlank()) {
          postgresProps.setProperty(ConfigurableOffsetBackingStore.OFFSET_STATE_VALUE, startOffset);
        }
