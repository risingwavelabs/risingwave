...
public class PostgresValidator extends DatabaseValidator implements AutoCloseable {
  ...
  public PostgresValidator(
    ...
    this.pubAutoCreate = userProps.get(DbzConnectorConfig.PG_PUB_CREATE).equalsIgnoreCase("true");
  ...
  public void validateDbConfig() {
    ...
    try (var stmt = jdbcConnection.prepareStatement(ValidatorUtils.getSql("postgres.slot.check"))) {
      ...
      if (res.next() && res.getString(1).equals(slotName)) {
        ...
        var stmt2 =
            jdbcConnection.prepareStatement(ValidatorUtils.getSql("postgres.slot_limit.check"));
  ...
  private void validatePrivileges() throws SQLException {
    ...
    if (this.isAwsRds) {
      ...
      try (var stmt =
          jdbcConnection.prepareStatement(ValidatorUtils.getSql("postgres.rds.role.check"))) {
      ...
      try (var stmt =
          jdbcConnection.prepareStatement(ValidatorUtils.getSql("postgres.superuser.check"))) {
        stmt.setString(1, this.user);
        var res = stmt.executeQuery();
        while (res.next()) {
  ...
  private void validatePublicationConfig(boolean isSuperUser) throws SQLException {
    ...
    if (isPartialPublicationEnabled) {
      try (var stmt =
          jdbcConnection.prepareStatement(ValidatorUtils.getSql("postgres.publication_attnames"))) {
        stmt.setString(1, schemaName);
        stmt.setString(2, tableName);
  ...
  private void validatePublicationPrivileges() throws SQLException {
    ...
    try (var stmt =
      ...
      while (res.next()) {
        if (!res.getBoolean(1)) {
          throw ValidatorUtils.invalidArgument(
              "Postgres user must have create privilege on database '" + this.dbName + "'");
        }
      }
    }
    ...
    if (!isTableOwner && null != tableOwner) {
      try (var stmt =
          jdbcConnection.prepareStatement(ValidatorUtils.getSql("postgres.users_of_group"))) {
        ...
        while (res.next()) {
          ...
          if (null != users
              && Arrays.asList(users).contains(userProps.get(DbzConnectorConfig.USER))) {
            isTableOwner = true;
            break;
          }
        }
  ...
  protected void alterPublicationIfNeeded() throws SQLException {
    ...
    String alterPublicationSql =
        String.format("ALTER PUBLICATION %s ADD TABLE %s", pubName, schemaName + "." + tableName);
    try (var stmt = jdbcConnection.createStatement()) {
