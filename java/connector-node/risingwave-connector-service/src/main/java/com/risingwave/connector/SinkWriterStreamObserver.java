  ...
  public void onNext(ConnectorServiceProto.SinkWriterStreamRequest sinkTask) {
    ...
    try {
      if (sinkTask.hasStart()) {
        if (isInitialized()) {
          throw ALREADY_EXISTS.withDescription("Sink is already initialized").asRuntimeException();
        }
        ...
        responseObserver.onNext(
            ConnectorServiceProto.SinkWriterStreamResponse.newBuilder()
                .setStart(ConnectorServiceProto.SinkWriterStreamResponse.StartResponse.newBuilder())
                .build());
      } else if (sinkTask.hasWriteBatch()) {
        ConnectorServiceProto.SinkWriterStreamRequest.WriteBatch batch = sinkTask.getWriteBatch();
        ...
        try (CloseableIterable<SinkRow> rowIter = deserializer.deserialize(batch)) {
          batchWritten =
              sink.write(new MonitoredRowIterable(rowIter, connectorName, String.valueOf(sinkId)));
        }
        ...
        if (batchWritten) {
          responseObserver.onNext(
              ConnectorServiceProto.SinkWriterStreamResponse.newBuilder()
                  .setBatch(
                      ConnectorServiceProto.SinkWriterStreamResponse.BatchWrittenResponse
                          .newBuilder()
        ...
        ConnectorServiceProto.SinkWriterStreamRequest.Barrier barrier = sinkTask.getBarrier();
        ...
        if (isCheckpoint) {
          ConnectorServiceProto.SinkWriterStreamResponse.CommitResponse.Builder builder =
              ConnectorServiceProto.SinkWriterStreamResponse.CommitResponse.newBuilder()
                  .setEpoch(currentEpoch);
