  ...
  public void onNext(ConnectorServiceProto.SinkCoordinatorStreamRequest request) {
    try {
      if (request.hasStart()) {
        ...
        TableSchema schema = TableSchema.fromProto(request.getStart().getParam().getTableSchema());
        this.coordinator =
            factory.createCoordinator(schema, request.getStart().getParam().getPropertiesMap());
        this.responseObserver.onNext(
            ConnectorServiceProto.SinkCoordinatorStreamResponse.newBuilder()
                .setStart(
                    ConnectorServiceProto.SinkCoordinatorStreamResponse.StartResponse.newBuilder()
                        .build())
                .build());
        ...
        this.responseObserver.onNext(
            ConnectorServiceProto.SinkCoordinatorStreamResponse.newBuilder()
                .setCommit(
                    ConnectorServiceProto.SinkCoordinatorStreamResponse.CommitResponse.newBuilder()
      ...
      String errString = String.format("get error when processing coordinator stream: %s", ex);
      LOG.error(errString);
