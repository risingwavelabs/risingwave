    ...
    for (int i = 0; i < colNames.length; i++) {
      ...
      switch (columnDesc.getDataType().getTypeName()) {
          ...
          switch (fieldType.getTypeName()) {
            ...
            default:
              throw io.grpc.Status.INVALID_ARGUMENT
                  .withDescription(
                      "stream_chunk: unsupported array with field type " + fieldType.getTypeName())
                  .asRuntimeException();
          }
          ret[i] =
              row -> {
        ...
        default:
          throw io.grpc.Status.INVALID_ARGUMENT
              .withDescription(
                  "stream_chunk: unsupported data type " + columnDesc.getDataType().getTypeName())
              .asRuntimeException();
      }
    }
    return ret;
  ...
  public CloseableIterable<SinkRow> deserialize(
    ...
    if (writeBatch.hasStreamChunkPayload()) {
      ...
      return new StreamChunkIterable(
          StreamChunk.fromPayload(streamChunkPayload.getBinaryData().toByteArray()), valueGetters);
    } else if (writeBatch.hasStreamChunkRefPointer()) {
      return new StreamChunkIterable(
          StreamChunk.fromRefPointer(writeBatch.getStreamChunkRefPointer()), valueGetters);
    } else {
      throw INVALID_ARGUMENT
          .withDescription("expected StreamChunkPayload, got " + writeBatch.getPayloadCase())
          .asRuntimeException();
    }
  }
