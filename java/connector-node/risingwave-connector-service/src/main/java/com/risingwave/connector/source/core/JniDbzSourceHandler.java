...
public class JniDbzSourceHandler {
  ...
  public static void runJniDbzSourceThread(byte[] getEventStreamRequestBytes, long channelPtr)
      throws Exception {
    var request = ConnectorServiceProto.GetEventStreamRequest.parseFrom(getEventStreamRequestBytes);
  ...
  public void start(long channelPtr) {
    ...
    try {
      ...
      if (!sendHandshakeMessage(runner, channelPtr, startOk)) {
        LOG.error("Failed to send handshake message to channel. sourceId={}", config.getSourceId());
        return;
      }
      ...
      while (runner.isRunning()) {
        ...
        if (!success) {
          LOG.info("Engine#{}: JNI sender broken detected, stop the engine", config.getSourceId());
          runner.stop();
          return;
        }
      }
