...
def init_iceberg_table(args, init_sqls):
    """Initialize Iceberg table with the given SQL statements."""
    if not init_sqls:
        return
    log("Initializing Iceberg table", level=LogLevel.HEADER)
    spark = get_spark(args)
    for sql in init_sqls:
        log(f"Executing SQL: {sql}", level=LogLevel.INFO, indent=2)
        spark.sql(sql)
    time.sleep(3)
...
def compare_sql(args, cmp_sqls):
    ...
    if len(cmp_sqls) != 2:
        return
    log("Comparing SQL results", level=LogLevel.HEADER)
    spark = get_spark(args)
    results = []
    for sql in cmp_sqls:
        log(f"Executing SQL: {sql}", level=LogLevel.INFO, indent=2)
        ...
        results.append(df)
    tc = unittest.TestCase()
    tc.assertEqual(len(results[0]), len(results[1]), "Row count mismatch")
    for row1, row2 in zip(results[0], results[1]):
        tc.assertEqual(row1, row2, "Row content mismatch")
...
def drop_table(args, drop_sqls):
    ...
    if not drop_sqls:
        return
    log("Dropping tables", level=LogLevel.HEADER)
    spark = get_spark(args)
