...
# TODO: compile only frontend 
k8s-frontend-node: setup-compile
	docker run --memory=13g --cpus="7" -v ${PWD}/src:/risingwave/src -v ${PWD}/docker_bin:/risingwave/bin -v ${PWD}/docker_target:/risingwave/target -v ${PWD}/k8s_workflow:/risingwave/k8s_workflow --workdir /risingwave rwk8scont /bin/bash -c "./k8s_workflow/compile.sh -p risingwave_frontend"
	exit 1
	...
	kubectl -n rwc-2-mytenant set image deployment/risingwave-frontend frontend=risingwave
...
k8s-compactor-node: setup-compile
	docker run --memory=13g --cpus="7" -v ${PWD}/src:/risingwave/src -v ${PWD}/docker_bin:/risingwave/bin -v ${PWD}/docker_target:/risingwave/target -v ${PWD}/k8s_workflow:/risingwave/k8s_workflow --workdir /risingwave rwk8scont /bin/bash -c "./k8s_workflow/compile.sh -p risingwave_compactor"
	bash -c "docker save risingwave | docker exec --privileged -i onebox-control-plane ctr --namespace=k8s.io images import --all-platforms -"
	kubectl -n rwc-2-mytenant set image deployment/risingwave-compactor compactor=risingwave
...
k8s-meta-node: setup-compile
	docker run --memory=13g --cpus="7" -v ${PWD}/src:/risingwave/src -v ${PWD}/docker_bin:/risingwave/bin -v ${PWD}/docker_target:/risingwave/target -v ${PWD}/k8s_workflow:/risingwave/k8s_workflow --workdir /risingwave rwk8scont /bin/bash -c "./k8s_workflow/compile.sh -p risingwave_meta"
	bash -c "docker save risingwave | docker exec --privileged -i onebox-control-plane ctr --namespace=k8s.io images import --all-platforms -"
	kubectl -n rwc-2-mytenant set image deployment/risingwave-meta meta=risingwave
...
k8s-compute-node: setup-compile
	exit 1
	docker run --memory=13g --cpus="7" -v ${PWD}/src:/risingwave/src -v ${PWD}/docker_bin:/risingwave/bin -v ${PWD}/docker_target:/risingwave/target -v ${PWD}/k8s_workflow:/risingwave/k8s_workflow --workdir /risingwave rwk8scont /bin/bash -c "./k8s_workflow/compile.sh -p risingwave_compute"
	bash -c "docker save risingwave | docker exec --privileged -i onebox-control-plane ctr --namespace=k8s.io images import --all-platforms -"
	kubectl -n rwc-2-mytenant set image pod/risingwave-compactor-o compute=risingwave
