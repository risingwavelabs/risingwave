...
echo "--- Test the jar file exists"
test -f /usr/local/lib/sqllogictest-jdbc-runner.jar

echo "--- e2e, jdbc, ci-3cn-1fe, streaming"
EXTERNAL_ENGINE_COMMAND_TEMPLATE="java -cp /usr/local/lib/sqllogictest-jdbc-runner.jar com.risingwave.sqllogictest.App jdbc:postgresql://{host}:{port}/{db} {user}" sqllogictest --engine=external -p 4566 -d dev './e2e_test/streaming/**/*.slt' --junit "streaming-${profile}"

echo "--- e2e, jdbc, ci-3cn-1fe, batch"
EXTERNAL_ENGINE_COMMAND_TEMPLATE="java -cp /usr/local/lib/sqllogictest-jdbc-runner.jar com.risingwave.sqllogictest.App jdbc:postgresql://{host}:{port}/{db} {user}" sqllogictest --engine=external -p 4566 -d dev './e2e_test/batch/**/*.slt' --junit "batch-${profile}"
