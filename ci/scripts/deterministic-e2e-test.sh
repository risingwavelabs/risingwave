...
seq 1 | timeout 1m parallel MADSIM_TEST_SEED={} $RUNNER './e2e_test/streaming/\*\*/\*.slt'

echo "--- deterministic simulation e2e, ci-3cn-1fe, batch"
seq 1 | timeout 30s parallel MADSIM_TEST_SEED={} $RUNNER './e2e_test/batch/\*\*/\*.slt'

echo "--- deterministic simulation e2e, ci-3cn-2fe, parallel, streaming"
seq 1 | timeout 2m parallel MADSIM_TEST_SEED={} $RUNNER -j 16 './e2e_test/streaming/\*\*/\*.slt'

echo "--- deterministic simulation e2e, ci-3cn-2fe, parallel, batch"
seq 1 | timeout 30s parallel MADSIM_TEST_SEED={} $RUNNER -j 16 './e2e_test/batch/\*\*/\*.slt'

# bugs here!
echo "--- deterministic simulation e2e, ci-3cn-1fe, recovery, streaming"
seq 1 | timeout 2m parallel MADSIM_TEST_SEED={} $RUNNER --kill-node './e2e_test/streaming/\*\*/\*.slt' || true

echo "--- deterministic simulation e2e, ci-3cn-1fe, recovery, batch"
seq 1 | timeout 2m parallel MADSIM_TEST_SEED={} $RUNNER --kill-node './e2e_test/batch/\*\*/\*.slt'
