...
# timeout 2m cargo make sslt --profile ci-release -- --kill-node './e2e_test/streaming/**/*.slt'

echo "--- deterministic simulation e2e, ci-3cn-1fe, recovery, batch"
timeout 2m cargo make sslt --profile ci-release -- --kill-node './e2e_test/batch/**/*.slt'
