...
echo "test parallel ungroup"
seq 10 | parallel --ungroup 'echo {} && sleep {} && echo {}'

echo "--- deterministic simulation e2e, ci-3cn-2fe-1meta, recovery, streaming"
seq "$TEST_NUM" | parallel --ungroup 'set -o pipefail && echo {} && sleep {} && echo {}  && ((MADSIM_TEST_SEED={} ./risingwave_simulation \
--kill \
--kill-rate=${KILL_RATE} \
--background-ddl-rate=${BACKGROUND_DDL_RATE} \
${EXTRA_ARGS:-} \
