...
# export RUST_LOG="info,\
# risingwave_stream::executor::over_window=trace,\
# risingwave_simulation=debug"
...
echo "--- deterministic simulation e2e, ci-3cn-2fe-3meta, recovery, streaming over window"
seq $TEST_NUM | parallel MADSIM_TEST_SEED={} './risingwave_simulation --kill --kill-rate=${KILL_RATE} --background-ddl-rate=${BACKGROUND_DDL_RATE} ./e2e_test/streaming/over_window/main.slt 2> $LOGDIR/recovery-streaming-{}.log && rm $LOGDIR/recovery-streaming-{}.log'
