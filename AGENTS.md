...
3. Always run `./risedev c` before submitting or marking a RisingWave PR ready for review, and report the result.
4. Add only validated missing labels. Do not remove labels that existed before the current agent operation unless explicitly instructed by a maintainer. An agent may remove a label it just added if validation shows it was a mistake, but it must report the correction.
5. In the final PR report, list labels added, labels intentionally not added, local tests run, and CI labels requested.
