...
impl serde::Serialize for SourceSnapshot {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("meta.SourceSnapshot", len)?;
        if !self.sources.is_empty() {
...
impl<'de> serde::Deserialize<'de> for SourceSnapshot {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = SourceSnapshot;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct meta.SourceSnapshot")
            }

            fn visit_map<V>(self, mut map: V) -> std::result::Result<SourceSnapshot, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
        ...
        deserializer.deserialize_struct("meta.SourceSnapshot", FIELDS, GeneratedVisitor)
    }
}
        ...
        S: serde::Serializer,
    {
        ...
        if let Some(v) = self.info.as_ref() {
            match v {
                ...
                subscribe_response::Info::BeSnapshot(v) => {
                    struct_ser.serialize_field("beSnapshot", v)?;
                }
            }
        }
        struct_ser.end()
                ...
                impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
                        ...
                        E: serde::de::Error,
                    {
                        match value {
                            ...
                            "beSnapshot" => Ok(GeneratedField::BeSnapshot),
                            _ => Err(serde::de::Error::unknown_field(value, FIELDS)),
                        }
                    }
                }
                    ...
                    V: serde::de::MapAccess<'de>,
            {
                ...
                while let Some(k) = map.next_key()? {
                    match k {
                        ...
                        GeneratedField::BeSnapshot => {
                            if info.is_some() {
                                return Err(serde::de::Error::duplicate_field("beSnapshot"));
                            }
                            info = Some(subscribe_response::Info::BeSnapshot(map.next_value()?));
                        }
                    }
                }
