...
mod tests {
    ...
    async fn test_delete() {
        ...
        let kv_pairs = vec![
            ...
            (2, 400, HummockValue::Put(test_value_of(1, 2))),
        ];
        let table1 = add_kv_pair(1, kv_pairs, sstable_manager.clone()).await;
        ...
        let k = ui.key();
        let v = ui.value();
        assert_eq!(k, user_key(iterator_test_key_of(1, 2).as_slice()));
        assert_eq!(v, test_value_of(1, 2));
