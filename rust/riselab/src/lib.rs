...
use self::util::{get_program_args, get_program_name};
use self::wait_tcp::wait_tcp;
    ...
    W: std::io::Write,
{
    ...
    pub fn complete_spin(&mut self) {
        self.pb
            .set_style(ProgressStyle::default_spinner().template("✅ {prefix}: {msg}"));
    }
    ...
    pub fn tmux_run(&self, user_cmd: Command) -> anyhow::Result<Command> {
        ...
        cmd.arg("-n").arg(self.id.as_ref().unwrap());
