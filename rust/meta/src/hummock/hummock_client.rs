...
mod tests {
    ...
    async fn test_table_operations() -> Result<()> {
        ...
        epoch += 1;
        let version_id_2 =
            add_tables(&hummock_client, vec![original_tables[1].clone()], epoch).await;
    ...
    async fn test_hummock_transaction() -> Result<()> {
        ...
        let epoch4 = epoch3 + 1;
        {
            ...
            // Get tables after committing epoch4. tables_in_epoch1, tables_in_epoch2,
            // tables_in_epoch4 should be returned.
            let (pinned_version_id, pinned_version) = pin_version(&hummock_client).await;
            assert!(pinned_version.uncommitted_epochs.is_empty());
