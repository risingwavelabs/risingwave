            ...
            let collect_futures = info.node_map.iter().filter_map(|(node_id, node)| {
                ...
                if actor_ids_to_send.is_empty() || actor_ids_to_collect.is_empty() {
                    ...
                    async move {
                        ...
                        // 1. Non-fatal injection errors (like inner-DAG barrier propagation errors) should not panic `BarrierManager`.
                        // 2. If this worker node has not collected this barrier, it might should retry.
                        // 3. BarrierManager should distinguish failures and let `StreamManager` cancel related jobs.
