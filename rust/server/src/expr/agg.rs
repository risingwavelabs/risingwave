...
//     Err(ErrorCode::InternalError("AggExpression shall not be evaluated as normal.".into()).into())
//   }
// }
//
// impl AggExpression {
...
//
//       None => {
//         Err(InternalError("Agg expression can only have exactly one child".to_string()).into())
//       }
//     }
//   }
// }
...
//   //     child: Box::new(InputRefExpression::new(t32, 0)),
//   //   };
//   //
//   //   let a = PrimitiveArray::<Int32Type>::from_slice(&[1, 2, 3])?;
//   //   let chunk = DataChunk::builder().cardinality(3).arrays(vec![a]).build();
//   //   let mut s = e.create_agg_state()?;
//   //   s.update(e.eval_child(&chunk)?)?;
//   //   let mut builder = t64.create_array_builder(1)?;
//   //   s.output(builder.as_mut())?;
//   //   let o = builder.finish()?;
//   //
//   //   let a: &PrimitiveArray<Int64Type> = downcast_ref(&*o)?;
//   //   let s = a.as_slice();
//   //   assert_eq!(s, &[6]);
//   //
//   //   Ok(())
