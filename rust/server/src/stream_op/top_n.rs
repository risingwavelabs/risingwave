...
mod tests {
    ...
    async fn test_top_n_executor() {
        ...
        let chunk2 = StreamChunk {
            ops: vec![Op::Insert, Op::Insert, Op::Insert, Op::Insert],
        ...
        if let Message::Chunk(res) = res {
            let col0 = &res.columns()[0];
            assert_eq!(col0.array().as_int64().value_at(0), Some(10));
            assert_eq!(col0.array().as_int64().value_at(1), Some(9));
            assert_eq!(col0.array().as_int64().value_at(2), Some(8));
            let ops = &res.ops;
        ...
        if let Message::Chunk(res) = res {
            let col0 = &res.columns()[0];
            assert_eq!(col0.array().as_int64().value_at(0), Some(7));
            assert_eq!(col0.array().as_int64().value_at(1), Some(10));
            assert_eq!(col0.array().as_int64().value_at(2), Some(3));
            assert_eq!(col0.array().as_int64().value_at(3), Some(9));
            assert_eq!(col0.array().as_int64().value_at(4), Some(3));
            let ops = &res.ops;
            assert_eq!(ops[0], Op::Insert);
            assert_eq!(ops[1], Op::Delete);
            assert_eq!(ops[2], Op::Insert);
            assert_eq!(ops[3], Op::Delete);
