...
impl Operations {
    pub(crate) async fn write_batch(&mut self, store: &impl StateStore, opts: &Opts) {
        let (prefixes, keys) = Workload::new_random_keys(opts, opts.writes as u64, 3);
        let values = Workload::new_values(opts, opts.writes as u64, 4);
    ...
    pub(crate) async fn delete_random(&mut self, store: &impl StateStore, opts: &Opts) {
        let (prefixes, keys) = Workload::new_random_keys(opts, opts.deletes as u64, 3);
