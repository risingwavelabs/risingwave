...
def test_table_creation(crud_instance, db_connection):
    ...
    cursor.execute("SELECT table_name FROM information_schema.tables WHERE table_name = 'sample_table';")
...
def test_data_updation(crud_instance, db_connection):
    ...
    cursor = db_connection.connect()
    cursor.execute("FLUSH;")
    cursor.execute("SELECT salary FROM sample_table WHERE name='John Doe';")
    result = cursor.fetchone()
    result = result[0]
    cursor.close()
    assert result == 12000
...
def test_data_deletion(crud_instance, db_connection):
    ...
    cursor = db_connection.connect()
    cursor.execute("FLUSH;")
    cursor.execute("SELECT EXISTS (SELECT 1 FROM sample_table WHERE name = 'John Doe');")
    result = cursor.fetchone()
    result = result[0]
    cursor.close()
...
def test_table_drop(crud_instance, db_connection):
    ...
    cursor = db_connection.connect()
    cursor.execute("FLUSH;")
    cursor.execute("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'sample_table');")
    result = cursor.fetchone()
    result = result[0]
    cursor.close()
...
def test_mv_creation(mv_instance,db_connection):
    ...
    cursor = db_connection.connect()
    cursor.execute("FLUSH;")
    cursor.execute("SELECT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'average_salary_view');")
    result = cursor.fetchone()[0]
    cursor.close()
...
def test_mv_updation(db_connection,crud_instance):
    ...
    cursor = db_connection.connect()
    cursor.execute("FLUSH;")
    cursor.execute("SELECT average_salary FROM average_salary_view WHERE age=25;")
    result = cursor.fetchone()[0]
    cursor.close()
    # assert result == 11250
    assert result == 12000
