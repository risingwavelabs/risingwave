...
import static com.risingwave.sql.tree.FrameBound.Type.UNBOUNDED_FOLLOWING;
...
public class UnboundedFollowingFrameBoundTest {
  ...
  public void testEndForFirstFrame() {
    ...
    assertThat(
        "the end boundary should always be the end of "
            + "the partition for the UNBOUNDED FOLLOWING frames",
        end,
        is(3));
  }
  ...
  public void testEndForSecondFrame() {
    ...
    assertThat(
        "the end boundary should always be "
            + "the end of the partition for the UNBOUNDED FOLLOWING frames",
        end,
        is(3));
  }
