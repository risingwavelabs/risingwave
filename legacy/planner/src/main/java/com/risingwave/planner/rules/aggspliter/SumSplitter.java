...
class SumSplitter extends AbstractAggSplitter {
  ...
  protected ImmutableList<AggregateCall> doMakeGlobalAggCall(SplitterArgs args) {
    ...
    // Use `SUM` from `SqlStdOperatorTable` rather than `RisingWaveOverrideOperatorTable` in global
    // phase to avoid promoting the type twice.
    var aggFunction = SqlStdOperatorTable.SUM;
