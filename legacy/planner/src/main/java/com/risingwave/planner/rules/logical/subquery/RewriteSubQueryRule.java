 ...
 * LogicalProject(a=[$0], b=[$1], c=[$2]) +- LogicalJoin(condition=[$3], joinType=[semi]) :-
 * LogicalTableScan(table=[[x, source: [TestTableSource(a, b, c)]]]) +- LogicalProject($f0=[IS NOT
 * NULL($0)]) +- LogicalAggregate(group=[{}], m=[MIN($0)]) +- LogicalProject(i=[true]) +-
...
public class RewriteSubQueryRule extends RelOptRule {
  ...
  public RewriteSubQueryRule() {
    super(
        operandJ(Filter.class, null, RexUtil.SubQueryFinder.FILTER_PREDICATE, any()),
