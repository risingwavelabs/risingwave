    ...
    ($self:expr, $it:ident, { $($l_body:tt)* }, { $($r_body:tt)* }) => {
        match $self {
            ListRef::Indexed { arr, idx } => {
                ...
                // construct `ListRef::Indexed` now.
                let $it = unsafe {
                    let offsets = *arr.offsets.get_unchecked(idx)..*arr.offsets.get_unchecked(idx + 1);
                    offsets.map(|o| arr.value.value_at_unchecked(o as usize))
                };
                $($l_body)*
            }
