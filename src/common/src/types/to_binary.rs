...
use super::{
    DataType, Date, Decimal, Interval, ScalarRefImpl, Serial, Time, Timestamp, TimestampNano,
    Timestamptz, F32, F64,
};
...
implement_using_to_sql! {
    ...
    { TimestampNano, TimestampNano, |x: &TimestampNano| x.0 },
    { Decimal, Decimal, |x| x },
...
impl ToBinary for ScalarRefImpl<'_> {
    fn to_binary_with_type(&self, ty: &DataType) -> Result<Bytes> {
        match self {
            ...
            ScalarRefImpl::TimestampNano(v) => v.to_binary_with_type(ty),
