...
use risingwave_pb::meta::PbActorCountPerParallelism;
...
impl From<WorkerActorCount> for PbWorkerActorCount {
    fn from(worker_actor_count: WorkerActorCount) -> Self {
        PbWorkerActorCount {
            actor_count: worker_actor_count.actor_count,
            parallelism: worker_actor_count.parallelism,
        }
    }
}
...
impl From<PbWorkerActorCount> for WorkerActorCount {
    fn from(pb_worker_actor_count: PbWorkerActorCount) -> Self {
        WorkerActorCount {
            actor_count: pb_worker_actor_count.actor_count,
            parallelism: pb_worker_actor_count.parallelism,
        }
    }
}
    ...
    fn from(actor_count_per_parallelism: ActorCountPerParallelism) -> Self {
        PbActorCountPerParallelism {
            worker_id_to_actor_count: actor_count_per_parallelism
                ...
                .map(|(k, v)| (k, v.into()))
                .collect(),
            hard_limit: actor_count_per_parallelism.hard_limit,
            soft_limit: actor_count_per_parallelism.soft_limit,
        }
    }
}
    ...
    fn from(pb_actor_count_per_parallelism: PbActorCountPerParallelism) -> Self {
        ActorCountPerParallelism {
            worker_id_to_actor_count: pb_actor_count_per_parallelism
                ...
                .map(|(k, v)| (k, v.into()))
                .collect(),
            hard_limit: pb_actor_count_per_parallelism.hard_limit,
            soft_limit: pb_actor_count_per_parallelism.soft_limit,
        }
    }
}
...
impl ActorCountPerParallelism {
    pub fn exceed_hard_limit(&self) -> bool {
        self.worker_id_to_actor_count
            .values()
            .any(|v| v.actor_count.saturating_mul(v.parallelism) > self.hard_limit)
    }

    pub fn exceed_soft_limit(&self) -> bool {
        self.worker_id_to_actor_count
            .values()
            .any(|v| v.actor_count.saturating_mul(v.parallelism) > self.soft_limit)
    }

    pub fn exceed_limit(&self) -> bool {
        self.exceed_hard_limit() || self.exceed_soft_limit()
    }
}
