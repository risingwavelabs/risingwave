...
pub trait SstableDataStream: Send + Sync {
    async fn reset(&mut self) -> ObjectResult<()>;
    async fn read(&mut self, size: usize) -> ObjectResult<&[u8]>;
}

pub struct InMemSstableDataStream {
    data: Bytes,
    offset: usize,
}

impl InMemSstableDataStream {
...
#[async_trait::async_trait]
impl SstableDataStream for InMemSstableDataStream {
    async fn reset(&mut self) -> ObjectResult<()> {
        self.offset = 0;
        Ok(())
    }
    ...
    async fn read(&mut self, size: usize) -> ObjectResult<&[u8]> {
        if self.offset + size > self.data.len() {
            return Err(ObjectError::internal(format!(
                "read in mem sstable data stream out of range. File size: {}, offset: {}, size: {}",
                self.data.len(),
                self.offset,
                size
            )));
...
pub type BoxedObjectUploader = Box<dyn ObjectUploader + Send + Sync>;
...
pub trait ObjectStore: Send + Sync {
    ...
    /// Get a handle for uploading an object to a specific `path`.
...
impl ObjectStoreImpl {
    ...
    pub async fn get_upload_handle(&self, path: &str) -> ObjectResult<BoxedObjectUploader> {
        match self {
            ObjectStoreImpl::Mem(mem) => mem.get_upload_handle(path).await,
            ObjectStoreImpl::S3(s3) => s3.get_upload_handle(path).await,
            ObjectStoreImpl::Disk(disk) => disk.get_upload_handle(path).await,
        }
    }
