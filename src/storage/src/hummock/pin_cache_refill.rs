...
impl PinCacheRefillController {
    ...
    pub(crate) fn replace_pinned_tables(&mut self, pinned_table_ids: HashSet<TableId>) -> bool {
    ...
    pub(crate) fn update_ownership(
        ...
        if changed && let Some(cache) = self.sstable_store.pin_cache() {
    ...
    fn rebuild_desired_objects(&mut self, preserve_versions: bool) {
        ...
        if preserve_versions {
            pin_cache.replace_version_objects(self.version.id(), objects);
        } else {
    ...
    fn apply_desired_object_delta(&mut self, deltas: &[SstDeltaInfo]) -> Option<()> {
        ...
        pin_cache.apply_desired_object_delta(self.version.id(), removed, inserted);
        Some(())
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_initial_empty_pin_snapshot_cleans_recovered_file() {
        let local_store: risingwave_object_store::object::ObjectStoreRef = Arc::new(
            ObjectStoreImpl::InMem(InMemObjectStore::for_test().monitored(
        ...
        sstable_store.set_pin_cache(pin_cache);
        ...
        assert!(controller.replace_pinned_tables(HashSet::new()));
