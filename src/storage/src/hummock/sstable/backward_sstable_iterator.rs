...
mod tests {
    ...
    async fn test_backward_sstable_seek() {
        ...
        for i in (0..TEST_KEYS_COUNT - 500 + 1).rev() {
            let key = sstable_iter.key();
            assert_eq!(key, test_key_of(i).table_key_as_slice(), "key index:{}", i);
            sstable_iter.next().await.unwrap();
        }
        ...
        sstable_iter
            ...
            .unwrap();
        let key = sstable_iter.key();
        assert_eq!(key, test_key_of(TEST_KEYS_COUNT - 1).table_key_as_slice());
        ...
        for idx in (1..TEST_KEYS_COUNT).rev() {
            ...
            let key = sstable_iter.key();
            assert_eq!(key, test_key_of(idx - 1).table_key_as_slice());
            sstable_iter.next().await.unwrap();
        }
        assert!(!sstable_iter.is_valid());
    }
