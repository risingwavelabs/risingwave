    ...
    F: TableBuilderFactory,
{
    ...
    pub fn check_table_and_vnode_change(&mut self, user_key: &UserKey<&[u8]>) -> (bool, bool) {
        ...
        if !self.split_tables.is_empty() && user_key.table_id.table_id != self.last_table_id {
            // table_id change
            self.last_table_id = user_key.table_id.table_id;
            if self.split_tables.contains(&user_key.table_id.table_id)
                || self.split_tables.contains(&self.last_table_id)
            {
                switch_builder = true;
            }
            self.last_vnode = 0;
