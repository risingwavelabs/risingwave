...
impl HummockEventHandler {
    ...
    fn handle_uploaded_ssts_inner(&mut self, ssts: Vec<Arc<StagingSstableInfo>>) {
        match ssts.as_slice() {
            ...
            ssts => {
                ...
                let affected_imms: HashSet<_> = ssts
                    .iter()
                    .flat_map(|sst| {
                    ...
                    .copied()
                    .collect();
                self.for_each_read_version(affected_imms, |instance_id, read_version| {
                    for sst in ssts {
    ...
    fn apply_version_updates(&mut self, events: Vec<CacheRefillerEvent>) {
        let Some(CacheRefillerEvent {
            new_pinned_version: lastest_pinned_version,
            ..
        }) = events.last()
        else {
            if cfg!(debug_assertions) {
        ...
        debug!("update to hummock version: {}", lastest_pinned_version.id(),);

        self.uploader
            .update_pinned_version(lastest_pinned_version.clone());
    }
}
