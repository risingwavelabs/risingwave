...
struct SyncingData {
    ...
    vector_index_adds: HashMap<TableId, Vec<PbVectorIndexAdd>>,
    sync_result_sender: oneshot::Sender<HummockResult<SyncedData>>,
}
...
struct UnsyncVectorIndexData {
    sealed_epoch_data: BTreeMap<HummockEpoch, PbVectorIndexAdd>,
    curr_epoch: u64,
    is_dropped: bool,
}
...
pub(crate) mod tests {
    ...
    use risingwave_pb::hummock::{PbFlatIndexAdd, PbVectorFileInfo};
    use tokio::sync::oneshot;
    ...
    async fn test_uploader_basic() {
        ...
        let vector_info_file = PbVectorFileInfo {
            object_id: 1,
            min_epoch: epoch1,
            max_epoch: epoch1,
            vector_count: 0,
        ...
        uploader.vector_writer_seal_epoch(
            ...
            epoch1.next_epoch(),
            PbVectorIndexAdd {
                add: Some(vector_index_add::Add::Flat(PbFlatIndexAdd {
        ...
        match sync_rx.await {
            Ok(Ok(data)) => {
                ...
                assert_eq!(
                    vector_index_add.add,
                    Some(vector_index_add::Add::Flat(PbFlatIndexAdd {
                        added_vector_files: vec![vector_info_file],
                    }))
                );
            }
