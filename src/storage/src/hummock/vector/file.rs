...
impl VectorFileMeta {
    ...
    fn encode_footer(&self) -> Bytes {
        let encoded_footer_len = self.encode_len() + size_of::<u64>() // for checksum
            + size_of::<u32>() // for version
            + size_of::<u32>(); // for magic
...
impl VectorFileBuilder {
    ...
    pub async fn finish(
        ...
        let ret = if let Some((object_id, mut uploader)) = self.uploader.take() {
            ...
            let encoded_footer = meta.encode_footer();
