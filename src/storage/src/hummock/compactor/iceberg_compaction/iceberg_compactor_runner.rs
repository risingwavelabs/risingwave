...
impl IcebergCompactionPlanRunner {
    ...
    async fn compact_impl(self) -> HummockResult<RewriteFilesStat> {
        let Self {
            task_id,
            plan_index,
            catalog,
...
fn retain_unrewritten_data_files(
    ...
    statistics.missing_rewrite_data_file_count = rewritten_data_file_paths
        .len()
        .saturating_sub(statistics.rewritten_data_file_count);
    statistics
}
...
async fn publish_cow_snapshot_to_main(
    ...
    if pruning_statistics.missing_rewrite_data_file_count > 0 {
        tracing::warn!(
            iceberg_component = "compaction_worker",
            iceberg_operation = "prune_cow_rewrite",
            task_id,
            ...
            ingestion_branch,
            planned_snapshot_id = publish_plan.snapshot_id,
            expected_rewritten_data_file_count = publish_plan.rewritten_data_file_paths.len(),
            matched_rewritten_data_file_count = pruning_statistics.rewritten_data_file_count,
            missing_rewrite_data_file_count = pruning_statistics.missing_rewrite_data_file_count,
...
mod tests {
    ...
    fn test_build_cow_auto_compaction_planning_config() {
        ...
        let CompactionPlanningConfig::Auto(config) =
            build_task_planning_config(compaction_kind, &iceberg_config, &test_runner_config())
                .unwrap()
        else {
            panic!("expected auto planning config");
        };
