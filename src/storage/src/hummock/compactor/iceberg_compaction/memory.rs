...
mod tests {
    ...
    fn scan_task(
        ...
        FileScanTask {
            ...
            schema,
            project_field_ids: matches!(content, DataContentType::EqualityDeletes)
                .then(|| vec![1])
                .unwrap_or_default(),
            predicate: None,
            deletes: vec![],
            sequence_number: 0,
            equality_ids: matches!(content, DataContentType::EqualityDeletes).then(|| vec![1]),
            partition: None,
            partition_spec: None,
            name_mapping: None,
            case_sensitive: true,
