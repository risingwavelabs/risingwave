...
// Wird von compactor benutzt um über SSTs zu iterieren.
// Nutzt intern [SstablePrefetchIterator].
pub struct ConcatSstableStreamIterator {
    key_range: KeyRange,
...
impl ConcatSstableStreamIterator {
    ...
    async fn seek_idx(&mut self, idx: usize, seek_key: Option<&[u8]>) -> HummockResult<()> {
        if idx >= self.tables.len() {
            ...
            let data = self
                .sstable_store
                // table data is loaded here?
                .scan(table.value(), start_index, end_index, &mut self.stats)
                .await?;
            let mut sstable_iter = SstableStreamIterator::new(table, data);
            if let Some(key) = seek_key {
...
impl HummockIterator for ConcatSstableStreamIterator {
...
mod tests {
    ...
    use crate::hummock::compactor::ConcatSstableStreamIterator;
    ...
    async fn test_concat_iterator() {
        ...
        let mut iter = ConcatSstableStreamIterator::new(
            table_infos.clone(),
            kr.clone(),
            compact_store.clone(),
        );
