...
use crate::hummock::utils::{check_subset_preserve_order, filter_single_sst, prune_ssts, range_overlap, search_sst_idx, search_sst_idx_in_level};
use crate::hummock::{
        ...
        for level in committed.levels(read_options.table_id) {
            ...
            if level.level_type == LevelType::Nonoverlapping as i32 {
                ...
                let start_table_idx = match encoded_user_key_range.start_bound() {
                    Included(key) | Excluded(key) => search_sst_idx_in_level(&level.table_infos, key),
                    _ => 0,
                };
                ...
                assert!(start_table_idx < level.table_infos.len() && end_table_idx < level.table_infos.len());

                let mut sstables = vec![];
                for sstable_info in &level.table_infos[start_table_idx..=end_table_idx] {
                    if sstable_info.table_ids.binary_search(&read_options.table_id.table_id).is_err() {
                        continue;
                    }
                    let sstable = self
                        .sstable_store
