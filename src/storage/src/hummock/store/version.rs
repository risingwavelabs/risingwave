...
use risingwave_pb::hummock::vector_index::Variant;
...
impl HummockVersionReader {
    ...
    pub async fn nearest<M: MeasureDistanceBuilder, O>(
        ...
        if target.dimension() != index.dimension as usize {
        ...
        match index.variant.as_ref().unwrap() {
            Variant::Flat(flat) => {
                for vector_file in &flat.vector_files {
                    if vector_file.min_epoch > epoch {
                        continue;
                    }
                    let path = self.sstable_store.get_sst_data_path(vector_file.object_id);
