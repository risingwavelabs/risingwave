...
impl BlockDataStream {
    ...
    pub async fn next_block_impl(&mut self) -> HummockResult<Option<(Bytes, usize)>> {
        let result = self.next_block_impl_inner().await;
        if result.is_err()
    ...
    async fn next_block_impl_inner(&mut self) -> HummockResult<Option<(Bytes, usize)>> {
    ...
    pub async fn next_block(&mut self) -> HummockResult<Option<Box<Block>>> {
        match self.next_block_impl().await? {
            None => Ok(None),
            Some((buf, uncompressed_size)) => Block::decode(buf, uncompressed_size)
                .inspect_err(|_| {
...
mod tests {
    ...
    async fn test_stream_error_invalidates_local_route() {
        ...
        assert!(stream.next_block_impl().await.is_err());
