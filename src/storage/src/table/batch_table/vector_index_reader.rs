...
impl<S: StateStore> VectorIndexReader<S> {
    pub fn new(vector_index_nearest_node: &PbVectorIndexReaderDesc, state_store: S) -> Self {
        ...
        let measure = PbDistanceType::try_from(vector_index_nearest_node.distance_type)
            .unwrap()
            .into();
