...
impl<S: StateStore, RS: RowSerde, const T: AccessType> StorageTableBase<S, RS, T> {
        ...
        let iterators: Vec<_> = try_join_all(vnodes.map(|vnode| {
            ...
            async move {
                let iter = StorageTableIterInner::<S, RS>::new(
                    ...
                    prefix_hint,
                    raw_key_range,
                    self.get_read_option(epoch.get_epoch()),
                    epoch,
                )
                .await?
                .into_stream();
    ...
    pub async fn streaming_iter_with_pk_bounds(
        ...
        self.iter_with_pk_bounds(
            HummockVersionEpoch::NoWait(epoch),
            pk_prefix,
            next_col_bounds,
            true,
        )
    ...
    pub async fn batch_iter(&self, epoch: u64) -> StorageResult<StorageTableIter<S, RS>> {
        self.batch_iter_with_pk_bounds(HummockVersionEpoch::Checkpoint(epoch), Row::empty(), ..)
            .await
    }
    ...
    pub async fn batch_iter_no_checkpoint(
        ...
        self.batch_iter_with_pk_bounds(HummockVersionEpoch::Reading(epoch), Row::empty(), ..)
            .await
    }
...
impl<S: StateStore, RS: RowSerde> StorageTableIterInner<S, RS> {
    ...
    where
        ...
        B: AsRef<[u8]> + Send,
    {
        if !matches!(epoch, HummockVersionEpoch::NoWait(_)) {
