...
impl<S: StateStore, E: Encoding> StorageTableBase<S, E, READ_WRITE> {
    ...
    pub async fn batch_write_rows(
        ...
        // The epoch might not be generated by meta, but we can use this epoch to filter out keys
        // from the current epoch. Conflicts in the current epoch should be checked by conflict
        // detector.
        let sanity_check_epoch = epoch.checked_sub(1);
