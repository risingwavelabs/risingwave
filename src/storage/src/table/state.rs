...
use super::flush_status::HashMapFlushStatus as FlushStatus;

/// `ManagedMViewState` buffers recent mutations. Data will be written
/// to backend storage on calling `flush`.
#[derive(Clone)]
pub struct ManagedCellBasedState<S: StateStore> {
    keyspace: Keyspace<S>,
...
impl<S: StateStore> ManagedCellBasedState<S> {
...
mod tests {
    ...
    async fn test_mview_state() {
        ...
        let mut state = ManagedCellBasedState::new(
            keyspace.clone(),
            vec![0.into(), 1.into()],
