...
use crate::vector::{
    MeasureDistance, MeasureDistanceBuilder, OnNearestItemFn, VectorInner, VectorItem, VectorRef,
};
...
impl<M: MeasureDistanceBuilder, R: Rng> HnswBuilder<M, R> {
    ...
    pub fn nearest<O>(
        &self,
        vec: VectorRef<'_>,
        on_nearest_fn: impl OnNearestItemFn<O>,
        ef_search: usize,
        top_n: usize,
    ) -> (Vec<O>, HnswStats) {
...
mod tests {
    ...
    use crate::vector::tests::{gen_info, gen_vector};
    use crate::vector::{InnerProductDistance, NearestBuilder};
