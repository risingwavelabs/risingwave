...
use risingwave_pb::hummock::{
    PbDistanceType, PbFlatIndex, PbFlatIndexAdd, PbFlatIndexConfig, PbHnswFlatIndex,
    PbHnswFlatIndexAdd, PbHnswFlatIndexConfig, PbHnswGraphFileInfo, PbVectorFileInfo,
...
impl HnswFlatIndex {
    ...
    fn apply_hnsw_flat_index_add(&mut self, add: &HnswFlatIndexAdd) {
        ...
        if self.graph_file.is_some() {
            assert!(
                !self.vector_store.vector_files.is_empty(),
...
impl VectorIndex {
    pub fn get_objects(&self) -> impl Iterator<Item = (HummockObjectId, u64)> + '_ {
        ...
        let vector_files = match &self.inner {
            VectorIndexImpl::Flat(flat) => &flat.vector_store.vector_files,
            VectorIndexImpl::HnswFlat(hnsw_flat) => &hnsw_flat.vector_store.vector_files,
        };
        ...
        match self {
            ...
            VectorIndexDelta::Adds(adds) => Some(adds.iter().flat_map(|add| {
                let vector_store_delta = match add {
                    VectorIndexAdd::Flat(add) => &add.vector_store_delta,
                    VectorIndexAdd::HnswFlat(add) => &add.vector_store_delta,
                };
