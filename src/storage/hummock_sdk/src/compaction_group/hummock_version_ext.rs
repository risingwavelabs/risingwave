        ...
        if !insert_table_infos.is_empty() {
            if insert_sst_level_id == 0 {
                ...
                assert!(
                    ...
                    "group-{}, should find the level to insert into when applying compaction generated delta. sub level idx: {},  removed sst ids: {:?}, sub levels: {:?},",
                    self.group_id, insert_sub_level_id, delete_sst_ids_set, l0.sub_levels.iter().map(|level| level.sub_level_id).collect_vec()
                );
