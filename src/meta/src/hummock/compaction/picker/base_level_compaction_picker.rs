...
use crate::hummock::compaction::picker::intral_sub_level_picker::IntraSubLevelPicker;
...
impl CompactionPicker for LevelCompactionPicker {
    fn pick_compaction(
        ...
        let mut picker = IntraSubLevelPicker::new(self.config.clone());

        if let Some(ret) = picker.pick_compaction(levels, level_handlers, stats) {
            return Some(ret);
        }
