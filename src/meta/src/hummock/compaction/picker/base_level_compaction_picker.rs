...
impl LevelCompactionPicker {
    ...
    fn pick_multi_level_to_base(
        ...
        if l0_size < base_level_size && self.strict_check {
            stats.skip_by_write_amp_limit += 1;
            return None;
        }

        let overlap_strategy = create_overlap_strategy(self.config.compaction_mode());
        let min_compaction_bytes = if self.strict_check {
        ...
        for input in l0_select_tables_vec {
            ...
            if target_level_size > self.config.max_compaction_bytes && self.strict_check {
                continue;
            }
        ...
        if !min_write_amp_meet && self.strict_check {
        ...
        for (idx, level) in l0.sub_levels.iter().enumerate() {
            ...
            for (plan_index, input) in l0_select_tables_vec.into_iter().enumerate() {
                ...
                if self.strict_check {
                    ...
                    let is_write_amp_large = max_level_size
...
pub mod tests {
    ...
    fn create_compaction_picker_for_test() -> LevelCompactionPicker {
        ...
        LevelCompactionPicker::new(1, config, true)
    }
    ...
    fn test_selecting_key_range_overlap() {
        ...
        let mut picker = LevelCompactionPicker::new(1, config, true);
    ...
    fn test_l0_to_l1_compact_conflict() {
        ...
        let config: CompactionConfig = CompactionConfigBuilder::new()
            ...
            .build();
        let mut picker = TierCompactionPicker::new(Arc::new(config), true);
    ...
    fn test_skip_compact_write_amplification_limit() {
        let config: CompactionConfig = CompactionConfigBuilder::new()
            ...
            .build();
        let mut picker = LevelCompactionPicker::new(1, Arc::new(config), true);
    ...
    fn test_l0_to_l1_break_on_pending_sub_level() {
        ...
        let mut picker = LevelCompactionPicker::new(1, config.clone(), true);
        let ret = picker.pick_compaction(&levels, &levels_handler, &mut local_stats);
    ...
    fn test_strict_check() {
...

        {
            ...
            let mut picker = LevelCompactionPicker::new(1, config.clone(), false);
            let ret = picker
                .pick_compaction(&levels, &levels_handler, &mut local_stats)
                .unwrap();
            assert_eq!(2, ret.input_levels.len());
...

        {
            let l0 = generate_l0_nonoverlapping_multi_sublevels(vec![vec![
                generate_table(1, 1, 100, 110, 1),
                generate_table(2, 1, 2000, 4000, 1),
            ]]);
            let levels = Levels {
                l0: Some(l0),
                levels: vec![generate_level(
                    1,
                    vec![generate_table(100, 1, 100, 1000, 1)],
                )],
                member_table_ids: vec![1],
                ..Default::default()
            };
...

            {
                ...
                // skip by write-amp
                let mut local_stats = LocalPickerStatistic::default();
                let mut picker = LevelCompactionPicker::new(1, config, false);
                let ret = picker
                    .pick_compaction(&levels, &levels_handler, &mut local_stats)
                    .unwrap();
                assert!(is_l0_trivial_move(&ret));
...

        {
            ...
            let config = Arc::new(
                CompactionConfigBuilder::new()
                    .level0_tier_compact_file_number(2)
                    .target_file_size_base(30)
                    .level0_sub_level_compact_level_count(4)
                    .build(),
            );

            let l0 = generate_l0_nonoverlapping_multi_sublevels(vec![
                vec![generate_table(1, 1, 100, 200, 1)],
                vec![generate_table(2, 1, 150, 2000, 1)],
                vec![generate_table(3, 1, 150, 200, 1)],
                vec![generate_table(4, 1, 150, 200, 1)],
            ]);
            ...
            let mut levels_handler = vec![LevelHandler::new(0), LevelHandler::new(1)];
            levels_handler[1].add_pending_task(
                1,
                1,
                levels.levels.get(0).unwrap().table_infos.iter(),
            );
            ...
            let ret = picker
                .pick_compaction(&levels, &levels_handler, &mut local_stats)
                .unwrap();
            assert_eq!(4, ret.input_levels.len());
        }
    }
}
