...
impl TierCompactionPicker {
    ...
    fn pick_intra_l0_compaction(
        ...
        for (idx, table) in level0.table_infos.iter().enumerate() {
            ...
            if idx + 1 >= level0.table_infos.len()
                || !self
                    .overlap_strategy
                    .check_overlap(table, &level0.table_infos[idx + 1])
            {
                // If the current table is not overlap with the next table, it means that they may
                // be generated by the previous intra-L0 compaction job.
                continue;
            }
            for other in level0.table_infos[idx + 1..].iter() {
