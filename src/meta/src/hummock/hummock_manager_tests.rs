...
async fn test_hummock_compaction_task() {
    ...
    let original_tables = generate_test_tables(
        epoch,
        hummock_manager.get_new_sst_ids(sst_num).await.unwrap(),
    );
...
async fn test_hummock_table() {
    ...
    let epoch: u64 = 1;
    let original_tables =
        generate_test_tables(epoch, hummock_manager.get_new_sst_ids(2).await.unwrap());
    register_sstable_infos_to_compaction_group(
        hummock_manager.compaction_group_manager_ref_for_test(),
        &original_tables,
        StaticCompactionGroupId::StateDefault.into(),
...
async fn test_hummock_transaction() {
    ...
    let epoch1: u64 = 1;
    {
        ...
        let tables_in_epoch1 =
            generate_test_tables(epoch1, hummock_manager.get_new_sst_ids(2).await.unwrap());
        register_sstable_infos_to_compaction_group(
            hummock_manager.compaction_group_manager_ref_for_test(),
            &tables_in_epoch1,
            StaticCompactionGroupId::StateDefault.into(),
    ...
    let epoch2 = epoch1 + 1;
    {
        ...
        let tables_in_epoch2 =
            generate_test_tables(epoch2, hummock_manager.get_new_sst_ids(2).await.unwrap());
        register_sstable_infos_to_compaction_group(
            hummock_manager.compaction_group_manager_ref_for_test(),
            &tables_in_epoch2,
            StaticCompactionGroupId::StateDefault.into(),
