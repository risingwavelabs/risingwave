                    ...
                    TableAlignRule::SplitToDedicatedCg(partition_vnode_count) => {
                        ...
                        match ret {
                            Ok((new_group_id, table_vnode_partition_count)) => {
                                ...
                                tracing::info!("move state table [{}] from group-{} to group-{} success table_vnode_partition_count {:?}", table_id, group.group_id, new_group_id, table_vnode_partition_count);
                            }
                            Err(e) => {
                                tracing::info!(error = %e.as_report(),"failed to move state table [{}] from group-{}",table_id, group.group_id)
                            }
                        }
                    }
                }
