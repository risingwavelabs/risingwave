...
impl<S: MetaStore> HummockManager<S> {
    ...
    pub async fn purge_stale(&self, table_fragments_list: &[TableFragments]) -> Result<()> {
        {
            ...
            let to_unregister = registered_members
                ...
                .collect_vec();
            guard
                .unregister(&to_unregister, self.env.meta_store(), false)
                .await?;
        }
    ...
    pub async fn unregister_table_ids(&self, table_ids: &[StateTableId]) -> Result<()> {
        self.compaction_group_manager
            .write()
            .await
            .unregister(table_ids, self.env.meta_store(), true)
            .await
    }
...
mod tests {
    ...
    async fn test_inner() {
        ...
        inner
            .write()
            .await
            .unregister(&[2u32], env.meta_store(), true)
            .await
            .unwrap();
