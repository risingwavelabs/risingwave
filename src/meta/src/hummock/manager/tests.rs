...
pub(super) async fn setup_compute_env_with_meta_opts(
    ...
    MetaSrvEnv,
    HummockManagerRef,
    crate::controller::cluster::ClusterControllerRef,
    risingwave_meta_model::WorkerId,
) {
...
fn assert_compaction_groups_non_overlapping(version: &HummockVersion) {
    let ranges = compaction_group_ranges(version);
    assert!(ranges.windows(2).all(|window| window[0].1 < window[1].0));
}

fn has_overlapping_compaction_groups(version: &HummockVersion) -> bool {
    let ranges = compaction_group_ranges(version);
...
async fn test_version_stats() {
    ...
    let ssts_with_table_ids = vec![vec![1.into(), 2.into()], vec![2.into(), 3.into()]];
...
async fn test_merge_compaction_group_task_expired() {
    ...
    let created_tables = HashSet::from_iter(vec![100.into(), 101.into(), 102.into()]);
...
#[tokio::test]
async fn test_normalize_overlapping_compaction_groups_cascading() {
    ...
    let members_64 = current_version
        .state_table_info
        .compaction_group_member_table_ids(cg_64)
        .iter()
        .map(|table_id| table_id.as_raw_id())
        .collect_vec();
    let members_65 = current_version
        .state_table_info
        .compaction_group_member_table_ids(cg_65)
        .iter()
        .map(|table_id| table_id.as_raw_id())
        .collect_vec();
...
#[tokio::test]
async fn test_normalize_overlapping_compaction_groups_with_limit() {
    let (_env, hummock_manager, _, _worker_id) = setup_compute_env(80).await;
    ...
    let split_count = hummock_manager
        .normalize_overlapping_compaction_groups_with_limit(1)
        .await
        .unwrap();
...
#[tokio::test]
async fn test_normalize_overlapping_compaction_groups_respects_disable_auto_group_scheduling() {
    let (_env, hummock_manager, _, _worker_id) = setup_compute_env(80).await;
    ...
    let cg_64 = get_compaction_group_id_by_table_id(hummock_manager.clone(), 64).await;
    hummock_manager
        .update_compaction_config(&[cg_64], &[MutableConfig::DisableAutoGroupScheduling(true)])
        .await
        .unwrap();
    ...
    let current_version = hummock_manager.get_current_version().await;
    let members = current_version
        .state_table_info
        .compaction_group_member_table_ids(cg_64)
        .iter()
        .map(|table_id| table_id.as_raw_id())
        .collect_vec();
    assert_eq!(members, vec![64, 80]);
}

#[tokio::test]
async fn test_normalize_overlapping_compaction_groups_respects_disable_auto_group_scheduling_on_right()
 {
    let (_env, hummock_manager, _, _worker_id) = setup_compute_env(80).await;
    ...
    let cg_20 = get_compaction_group_id_by_table_id(hummock_manager.clone(), 20).await;
    hummock_manager
        .update_compaction_config(&[cg_20], &[MutableConfig::DisableAutoGroupScheduling(true)])
        .await
        .unwrap();
    ...
    assert_eq!(ranges, vec![(10, 40), (20, 50)]);
}

#[tokio::test]
async fn test_normalize_overlapping_compaction_groups_skips_disabled_boundary_but_continues_later_segment()
 {
    let (_env, hummock_manager, _, _worker_id) = setup_compute_env(80).await;
    ...
    let cg_10 = get_compaction_group_id_by_table_id(hummock_manager.clone(), 10).await;
    hummock_manager
        .update_compaction_config(&[cg_10], &[MutableConfig::DisableAutoGroupScheduling(true)])
        .await
        .unwrap();
    ...
    let members = current_version
        .state_table_info
        .compaction_group_member_table_ids(cg_10)
        .iter()
        .map(|table_id| table_id.as_raw_id())
        .collect_vec();
    assert_eq!(members, vec![10, 15]);
    assert_compaction_groups_non_overlapping(&current_version);
}

#[tokio::test]
async fn test_normalize_overlapping_compaction_groups_noop_on_non_overlapping_input() {
    let (_env, hummock_manager, _, _worker_id) = setup_compute_env(80).await;
...
#[tokio::test]
async fn test_schedule_group_split_with_normalize_disabled() {
    let (_env, hummock_manager, _, _worker_id) = setup_compute_env(80).await;
...
async fn test_split_multiple_hot_tables_uses_current_parent() {
        ...
        .unwrap();
    {
        ...
        for table in [100, 101, 102] {
            ...
            stats.record_commit(
                table.into(),
                manager.env.opts.table_high_write_throughput_threshold + 1,
    ...
    tokio::time::advance(Duration::from_secs(1)).await;
    {
        let mut stats = manager.table_write_throughput_statistic_manager.write();
        let now = Instant::now();
        // New hot ingress must not be diluted by the older cold history. The last
        // table's burst is still pending, with no later commit to close its sample.
        for table in [100, 101, 102] {
            if table == 102 {
                stats.record_commit(table.into(), 0, now);
            }
...
async fn test_merge_batch_uses_accumulated_size() {
    ...
    let registry = Registry::new();
    let (env, manager, _, worker_id) = setup_compute_env_with_metric(
        80,
    ...
    tokio::time::advance(Duration::from_secs(cold_interval)).await;
    {
        let mut stats = manager.table_write_throughput_statistic_manager.write();
        for table in [100, 101, 102] {
    ...
    // Current size can also change after the batch snapshot was built.
    {
        ...
        versioning
            .version_stats
            .table_stats
            .get_mut(&TableId::new(100))
            .unwrap()
            .total_value_size = limit as i64;
    }
    let error = manager
        .try_merge_compaction_group(&stale_stats, left, right, &created_tables)
        .await
    ...
    // A newly tightened L0 limit must use current config and levels, even with the
    // original scheduling snapshot. Table 100's actual commit above added an L0 SST.
    let cold_stats = manager
        .table_write_throughput_statistic_manager
        .read()
        .clone();
    manager
        .update_compaction_config(
            ...
            &[MutableConfig::Level0StopWriteThresholdMaxSstCount(0)],
        )
        .await
        .unwrap();
    let error = manager
        .try_merge_compaction_group(&cold_stats, left, right, &created_tables)
        .await
        .err()
        .expect("the current merge policy must reject this stale snapshot");
    ...
    manager
        .update_compaction_config(
            &[left.group_id, right.group_id],
            &[MutableConfig::Level0StopWriteThresholdMaxSstCount(
                left.compaction_group_config
                    .compaction_config
...
async fn test_empty_commits_observe_only_committed_tables() {
        ...
        .unwrap();
    {
        ...
        assert_eq!(stats.latest_table_throughput(100.into()), Some(50));
...
async fn test_vnode_partition_ignores_expired_throughput() {
    ...
    for (timestamp, expected) in [
        (now - Duration::from_secs(3600), 1),
        (now, manager.env.opts.hybrid_partition_node_count),
    ] {
        record_table_throughput(
            &mut manager.table_write_throughput_statistic_manager.write(),
            table,
            u64::MAX / 2,
            timestamp,
        );
