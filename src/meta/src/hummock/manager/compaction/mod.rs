...
use risingwave_pb::hummock::{
    CompactTaskAssignment, CompactTaskProgress, CompactionConfig, PbCompactStatus,
    PbCompactTaskAssignment, SubscribeCompactionEventRequest,
    SubscribeIcebergCompactionEventRequest, TableOption, TableSchema, compact_task,
};
...
use crate::hummock::{
    CompactorManager, CompactorSubscribeResponseEvent, CompactorType, CompactorTypePrimitive,
    HummockManager, commit_multi_var, start_measure_real_process_timer,
};
...
impl HummockManager {
    pub async fn compaction_event_loop(
        ...
        let (event_loop_join_handle, event_loop_shutdown_tx) = event_loop.run().await;
        join_handle_vec.push((event_loop_join_handle, event_loop_shutdown_tx));

        join_handle_vec
    }

    pub async fn iceberg_compaction_event_loop(
        hummock_manager: Arc<Self>,
        compactor_streams_change_rx: UnboundedReceiver<(
            u32,
        ...
        let iceberg_compaction_event_handler =
            IcebergCompactionEventHandler::new(hummock_manager.compactor_manager.clone());
...
pub struct HummockCompactionEventDispatcher {
    ...
    tx: Option<UnboundedSender<(u32, subscribe_compaction_event_request::Event)>>,
}
            ...
            RequestEvent::PullTask(pull_task) => {
                compactor_alive = self
                    ...
                    .handle_pull_task_event(
                        ...
                        &mut init_selectors(),
                        self.hummock_manager.env.opts.max_get_task_probe_times,
                    )
                    .await;
            }
...
impl HummockCompactionEventHandler {
    ...
    async fn handle_heartbeat(&self, context_id: u32, progress: Vec<CompactTaskProgress>) -> bool {
        ...
        match compactor_manager.get_compactor(context_id, CompactorType::Hummock) {
            Some(compactor) => {
    ...
    async fn handle_pull_task_event(
            ...
            .get_compactor(context_id, CompactorType::Hummock)
        {
                    ...
                    match compact_ret {
                        Ok((compact_tasks, unschedule_groups)) => {
                            ...
                            for task in compact_tasks {
                                ...
                                if let Err(e) =
                                    compactor.send_event(CompactorSubscribeResponseEvent::Hummock(
                                        ResponseEvent::CompactTask(task.into()),
                                    ))
                                {
                                    tracing::warn!(
                                        error = %e.as_report(),
                            ...
                            if !failed_tasks.is_empty() {
                                self.hummock_manager
                                    .compactor_manager
                                    .remove_compactor(context_id, CompactorType::Hummock);
                            }
            ...
            // ack to compactor
            if let Err(e) = compactor.send_event(CompactorSubscribeResponseEvent::Hummock(
                ResponseEvent::PullTaskAck(PullTaskAck {}),
            )) {
                tracing::warn!(
                    error = %e.as_report(),
...
pub struct CompactionEventLoop<
    ...
    const T: CompactorTypePrimitive,
> {
...
> CompactionEventLoop<D, E, T>
{
    ...
    pub async fn run(mut self) -> (JoinHandle<()>, Sender<()>) {
...
struct HummockCompactorDedicatedEventLoop {
    hummock_manager: Arc<HummockManager>,
    hummock_compaction_event_handler: HummockCompactionEventHandler,
}
...
impl HummockCompactorDedicatedEventLoop {
    ...
    fn run(
        ...
        JoinHandle<()>,
        UnboundedSender<(u32, subscribe_compaction_event_request::Event)>,
        Sender<()>,
    ) {
...
struct IcebergCompactionEventHandler {
    compactor_manager: Arc<CompactorManager>,
}

impl IcebergCompactionEventHandler {
    pub fn new(compactor_manager: Arc<CompactorManager>) -> Self {
        Self { compactor_manager }
    }
    ...
    fn handle_pull_task_event(&self, context_id: u32, pull_task_count: usize) -> bool {
        ...
        if let Some(compactor) = self
            .compactor_manager
            .get_compactor(context_id, CompactorType::Iceberg)
        {
            let mut compactor_alive = true;

            // TODO: Pull Iceberg Task from iceberg compaction manager

            if let Err(e) = compactor.send_event(CompactorSubscribeResponseEvent::Iceberg(
                IcebergResponseEvent::PullTaskAck(IcebergPullTaskAck {}),
            )) {
                tracing::warn!(
                    error = %e.as_report(),
                    "Failed to send ask to {}",
...
struct IcebergCompactionEventDispatcher {
...
impl IcebergCompactionEventDispatcher {
    fn new(compaction_event_handler: IcebergCompactionEventHandler) -> Self {
        Self {
            compaction_event_handler,
        }
    }
...
type HummockCompactionEventLoop = CompactionEventLoop<
    HummockCompactionEventDispatcher,
    SubscribeCompactionEventRequest,
    { CompactorType::Hummock },
>;

type IcebergCompactionEventLoop = CompactionEventLoop<
