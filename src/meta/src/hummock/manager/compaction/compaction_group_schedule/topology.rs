...
impl HummockManager {
    pub async fn merge_compaction_group(
        ...
        self.merge_compaction_group_impl(group_1, group_2, created_tables)
            .await
    }
    ...
    pub(crate) async fn merge_compaction_group_impl(
        &self,
        group_1: CompactionGroupId,
        group_2: CompactionGroupId,
        created_tables: HashSet<TableId>,
        ...
        let combined_member_table_ids = member_table_ids_1.iter().chain(member_table_ids_2.iter());
        assert!(combined_member_table_ids.clone().is_sorted());
        ...
        // check branched sst on non-overlap level
        {
            ...
            for level_idx in 1..=max_level {
                ...
                let left_last_sst = left_level.table_infos.last().unwrap();
                let right_first_sst = right_level.table_infos.first().unwrap();
        ...
        new_version_delta.with_latest_version(|version, new_version_delta| {
            combined_member_table_ids.for_each(|&table_id| {
                let info = version
                    .state_table_info
                    .info()
                    .get(&table_id)
