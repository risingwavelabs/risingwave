...
impl HummockManager {
    ...
    pub(crate) async fn truncate_time_travel_metadata(
        ...
        for pinned_snapshot_version_id in pinned_snapshot_version_ids {
            ...
            pinned_snapshot_sst_ids.extend(resolved.replay_version.get_sst_ids(true));
            pinned_snapshot_object_ids.extend(resolved.replay_version.get_object_ids(true));
        ...
        let mut retained_snapshot_sst_ids = latest_valid_version.get_sst_ids(true);
...
async fn resolve_time_travel_version(
    ...
    let deltas = hummock_time_travel_delta::Entity::find()
        ...
        .all(conn)
        .await?;
    let replay_version = replay_version.version.to_protobuf();
    Ok(Some(ResolvedTimeTravelVersion {
        replay_version: IncompleteHummockVersion::from_persisted_protobuf(&replay_version),
        deltas: deltas
            .into_iter()
            .map(|delta| {
                let delta = delta.version_delta.to_protobuf();
                IncompleteHummockVersionDelta::from_persisted_protobuf(&delta)
            })
            .collect(),
    }))
}
