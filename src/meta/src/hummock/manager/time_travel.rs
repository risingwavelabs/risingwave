...
impl HummockManager {
    pub(crate) async fn init_time_travel_state(&self) -> Result<()> {
        ...
        guard.last_time_travel_snapshot_sst_ids = version.get_sst_ids_exclude_change_log();
        Ok(())
    }
    ...
    pub(crate) async fn truncate_time_travel_metadata(
        ...
        let (
            ...
            (
                ...
                latest_valid_version.get_sst_ids_exclude_change_log(),
                latest_valid_version.get_object_ids_exclude_change_log(),
            )
        };
        ...
        for delta_id_to_delete in delta_ids_to_delete {
            ...
            let new_sst_ids = delta_to_delete.newly_added_sst_ids_exclude_change_log();
            ...
            let new_object_ids = delta_to_delete.newly_added_object_ids_exclude_change_log();
        ...
        for prev_version_id in version_ids_to_delete {
            ...
            let sst_ids = prev_version.get_sst_ids_exclude_change_log();
            ...
            let new_object_ids = prev_version.get_object_ids_exclude_change_log();
            object_ids_to_delete.extend(&new_object_ids - &latest_valid_version_object_ids);
    ...
    pub async fn epoch_to_version(
        ...
        let mut sst_ids = actual_version
            .get_sst_ids_exclude_change_log()
            .into_iter()
            .collect::<VecDeque<_>>();
    ...
    pub(crate) async fn write_time_travel_metadata(
        ...
        if let Some(version) = version {
            ...
            version_sst_ids = Some(
                version
                    .get_sst_infos_exclude_change_log()
                    .filter_map(|s| {
                        if s.table_ids
                            .iter()
            ...
            write_sstable_infos(
                version.get_sst_infos_exclude_change_log().filter(|s| {
