...
use risingwave_meta_model_v2::hummock_version_delta::{GroupDeltas, SstableObjectIds};
use risingwave_meta_model_v2::{
    compaction_config, compaction_status, compaction_task, hummock_pinned_snapshot,
    hummock_pinned_version, hummock_version_delta,
};
...
impl Transactional<Transaction> for CompactionGroup {
    ...
    async fn delete_in_transaction(&self, trx: &mut Transaction) -> MetadataModelResult<()> {
        compaction_config::Entity::delete_by_id(self.group_id)
            .exec(trx)
            .await?;
        Ok(())
    }
...
impl Transactional<Transaction> for CompactStatus {
    ...
    async fn delete_in_transaction(&self, trx: &mut Transaction) -> MetadataModelResult<()> {
        compaction_status::Entity::delete_by_id(self.compaction_group_id)
            .exec(trx)
            .await?;
        Ok(())
    }
...
impl Transactional<Transaction> for CompactTaskAssignment {
    ...
    async fn delete_in_transaction(&self, trx: &mut Transaction) -> MetadataModelResult<()> {
        compaction_task::Entity::delete_by_id(self.compact_task.as_ref().unwrap().task_id)
            .exec(trx)
            .await?;
        Ok(())
    }
...
impl Transactional<Transaction> for HummockPinnedSnapshot {
    async fn upsert_in_transaction(&self, trx: &mut Transaction) -> MetadataModelResult<()> {
        let m = hummock_pinned_snapshot::ActiveModel {
            context_id: Set(self.context_id),
            min_pinned_snapshot: Set(self.minimal_pinned_snapshot),
        };
    ...
    async fn upsert_in_transaction(&self, trx: &mut Transaction) -> MetadataModelResult<()> {
        let m = hummock_version_delta::ActiveModel {
            ...
            group_deltas: Set(GroupDeltas(
                self.group_deltas
                    .iter()
                    .map(|(k, v)| (*k, v.group_deltas.to_owned()))
                    .collect(),
            )),
            max_committed_epoch: Set(self.max_committed_epoch),
            safe_epoch: Set(self.safe_epoch),
            trivial_move: Set(self.trivial_move),
            gc_object_ids: Set(SstableObjectIds(self.gc_object_ids.to_owned())),
        };
...
impl From<compaction_config::Model> for CompactionGroup {
    fn from(value: compaction_config::Model) -> Self {
        Self::new(value.compaction_group_id, value.config.0)
    }
}
