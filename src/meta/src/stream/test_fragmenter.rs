...
use risingwave_pb::stream_plan::{
    ...
    FilterNode, FragmentTypeFlag, MaterializeNode, PbDispatchOutputMapping, ProjectNode,
    ...
    let exchange_node = StreamNode {
        node_body: Some(NodeBody::Exchange(Box::new(ExchangeNode {
            strategy: Some(DispatchStrategy {
                ...
                output_mapping: Some(PbDispatchOutputMapping::simple(vec![0, 1, 2])),
            }),
        }))),
        fields: vec![
    ...
    vec![
        StreamFragmentEdge {
            dispatch_strategy: Some(DispatchStrategy {
                ...
                output_mapping: None, // how could this be None?
            }),
            link_id: 4,
            upstream_id: 1,
            downstream_id: 0,
