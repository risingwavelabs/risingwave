...
impl Scheduler {
    ...
    pub fn new(
        ...
        // Round-robin the parallel units in each worker node.
        let mut queue: VecDeque<_> = scheduled.into_values().collect();
        let mut scheduled_parallel_units = vec![];
        while let Some(mut v) = queue.pop_front() {
            if let Some(first) = v.pop_first() {
                scheduled_parallel_units.push(first);
                queue.push_back(v)
            }
        }
