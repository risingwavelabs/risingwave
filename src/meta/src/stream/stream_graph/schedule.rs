...
pub(super) struct Scheduler {
    ...
    dynamic_mapping_fn: Box<dyn Fn(usize, bool) -> anyhow::Result<ActorAlignmentMapping>>,
}
...
impl Scheduler {
    ...
    pub fn new(
        ...
        let dynamic_mapping_fn = Box::new(move |limited_count: usize, force_parallelism: bool| {
            let (parallelism, vnode_count) = if force_parallelism {
                ...
                (parallelism.min(limited_count), limited_count)
            };

            let assignment = assigner.assign_hierarchical(
                &worker_weights,
                &(0..parallelism).collect_vec(),
                &(0..vnode_count).collect_vec(),
            )?;

            let mapping = ActorAlignmentMapping::from_assignment(assignment, vnode_count);
            Ok(mapping)
        });
        Ok(Self {
    ...
    pub fn schedule(
        ...
        let mut force_parallelism_fragment_ids: HashSet<_> = HashSet::default();
                ...
                let vnode_count = match node {
                    ...
                    NodeBody::StreamCdcScan(node) => {
                        ...
                        if options.is_parallelized_backfill() {
                            force_parallelism_fragment_ids.insert(id);
                            options.backfill_parallelism as usize
                        } else {
                            return;
                        }
                    }
                ...
                Some(reqs) => {
                    ...
                    match req {
                        ...
                        Req::AnyVnodeCount(vnode_count) => {
                            let force_parallelism = force_parallelism_fragment_ids.contains(&id);
