...
pub async fn list_prometheus_cluster(
    ...
    if let Some(ref client) = srv.prometheus_client {
        ...
        let cpu_query =
            format!("sum(rate(process_cpu_seconds_total{{job=~\"standalone|compute|meta|frontend\", {}}}[60s]) or label_replace(rate(process_cpu_seconds_total{{component=~\"standalone|compute|meta|frontend\", {}}}[60s]), \"job\", \"$1\", \"component\", \"(.*)\")) by (job,instance)", srv.prometheus_selector);
        let result = client
            .query_range(
                cpu_query,
        ...
        let cpu_data = result
            ...
            .collect();
        let memory_query =
            format!("avg(process_resident_memory_bytes{{job=~\"standalone|compute|meta|frontend\", {}}} or label_replace(process_resident_memory_bytes{{component=~\"standalone|compute|meta|frontend\", {}}}, \"job\", \"$1\", \"component\", \"(.*)\")) by (job,instance)", srv.prometheus_selector);
        let result = client
            .query_range(
                memory_query,
                now.duration_since(SystemTime::UNIX_EPOCH)
