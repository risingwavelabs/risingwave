...
impl CatalogController {
    ...
    pub(crate) async fn clean_dirty_sink_downstreams(
        ...
        // For the current sink-into-table implementation, there is no need to clean up the info of `StreamNode`,
        // because the `UpstreamSinkUnion` operator does not persist any data, but relies on refill during recovery. The
        // info inside operator in old version will no longer be used.
