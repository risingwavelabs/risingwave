...
impl From<ObjectModel<table::Model>> for PbTable {
    fn from(value: ObjectModel<table::Model>) -> Self {
        Self {
            ...
            cdc_schema_change_failure_policy: value.0.cdc_schema_change_failure_policy.map(
                |policy| risingwave_pb::catalog::SchemaChangeFailurePolicy::from(policy) as i32,
            ),
        }
    }
}
