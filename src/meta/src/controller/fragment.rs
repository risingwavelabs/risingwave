...
use risingwave_pb::stream_plan::{
    ...
    PbStreamNode, PbStreamScanType, StreamActor, StreamScanType,
};
...
use crate::controller::utils::{
    get_fragment_actor_dispatchers, get_fragment_mappings, rebuild_fragment_mapping_from_actors,
    resolve_no_shuffle_actor_dispatcher, FragmentDesc, PartialActorLocation,
...
impl CatalogController {
    ...
    pub async fn all_node_actors(
        ...
        for (fragment, actors) in fragment_actors {
            let fragment_dispatcher = fragment_actor_dispatcher
                .remove(&fragment.fragment_id)
                .unwrap_or_default();
            let (table_fragments, actor_status, _) =
                Self::compose_fragment(fragment, actors, fragment_dispatcher)?;
    ...
    pub async fn get_root_fragments(
        ...
        for (_, fragment) in root_fragments {
            ...
            root_fragments_pb.insert(
                fragment.job_id,
                Self::compose_fragment(fragment, actors, fragment_dispatcher)?.0,
            );
        }
...
mod tests {
    ...
    use risingwave_meta_model::fragment::DistributionType;
    use risingwave_meta_model::{
        actor, fragment, ActorId, ConnectorSplits, ExprContext, FragmentId, I32Array, ObjectId,
        StreamNode, TableId, VnodeBitmap,
    };
    use risingwave_pb::common::PbActorLocation;
    use risingwave_pb::meta::table_fragments::actor_status::PbActorState;
