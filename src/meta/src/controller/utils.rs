...
/// Reject direct deletion of an internal cross-database subscription.
...
where
    C: ConnectionTrait,
{
    let cross_db_downstream_job_id: Option<JobId> = Subscription::find_by_id(subscription_id)
        .select_only()
        .column(subscription::Column::CrossDbDownstreamJobId)
        .into_tuple()
        .one(txn)
        .await?
        .ok_or_else(|| MetaError::catalog_id_not_found("subscription", subscription_id))?;
    if cross_db_downstream_job_id.is_some() {
        return Err(MetaError::permission_denied(
            "internal cross-database subscriptions cannot be dropped directly".to_owned(),
        ));
    }
