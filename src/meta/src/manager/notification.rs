...
use risingwave_pb::meta::{AlignEpoch, MetaSnapshot, SubscribeResponse, SubscribeType};
    ...
    S: MetaStore,
{
    ...
    async fn notify_with_version(
        ...
        self.notify(target, operation, info, Some(version), align_epoch);
        version
    }
    ...
    pub async fn notify_frontend(
        ...
        self.notify_with_version(SubscribeType::Frontend.into(), operation, info, align_epoch)
            .await
    }

    pub async fn notify_hummock(&self, operation: Operation, info: Info) -> NotificationVersion {
        self.notify_with_version(SubscribeType::Hummock.into(), operation, info, None)
            .await
    }

    pub async fn notify_compactor(&self, operation: Operation, info: Info) -> NotificationVersion {
        self.notify_with_version(SubscribeType::Compactor.into(), operation, info, None)
            .await
    }
