...
impl IcebergCompactionManager {
    ...
    async fn route_pk_index_compaction_report(
        ...
        let output_files = match IcebergCommitResult::try_from(output_metadata) {
            Ok(result) => result.data_files,
            Err(e) => {
                tracing::warn!(
                    sink_id = %sink_id,
                    task_id = %task_id,
    ...
    async fn enqueue_compaction_resolve_job(
        ...
        sink_id: SinkId,
        task_id: IcebergCompactionTaskId,
        output_files: Vec<iceberg::spec::SerializedDataFile>,
        ...
        let command = crate::barrier::Command::CreateCompactionResolveJob {
            database_id,
            sink_id,
            task_id,
            writer_fragment_id,
            pk_index_table_id,
            overwrite: crate::barrier::CompactionResolveOverwrite {
                output_files,
                input_file_paths,
                read_snapshot_id,
            },
