...
impl DatabaseManager {
    pub async fn new(env: MetaSrvEnv) -> MetaResult<Self> {
        let databases = Database::list(env.kv_meta_store_checked()).await?;
        let schemas = Schema::list(env.kv_meta_store_checked()).await?;
        let sources = Source::list(env.kv_meta_store_checked()).await?;
        let sinks = Sink::list(env.kv_meta_store_checked()).await?;
        let tables = Table::list(env.kv_meta_store_checked()).await?;
        let indexes = Index::list(env.kv_meta_store_checked()).await?;
        let views = View::list(env.kv_meta_store_checked()).await?;
        let functions = Function::list(env.kv_meta_store_checked()).await?;
        let connections = Connection::list(env.kv_meta_store_checked()).await?;
        let subscriptions = Subscription::list(env.kv_meta_store_checked()).await?;
