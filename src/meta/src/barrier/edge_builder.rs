...
impl FragmentEdgeBuilder {
    ...
    pub(super) fn replace_upstream(
        ...
        let new_upstream_fragment = &self.fragments[&new_upstream_fragment_id];
        self.result
            .merge_updates
            .entry(fragment_id)
            .or_default()
            .extend(fragment.actors.keys().map(|actor_id| MergeUpdate {
                actor_id: *actor_id,
                upstream_fragment_id: original_upstream_fragment_id,
                new_upstream_fragment_id: Some(new_upstream_fragment_id),
                added_upstream_actor_id: new_upstream_fragment.actors.keys().cloned().collect(),
