        ...
        match self {
            &mut Self::ConsumingSnapshot {
                ...
                if snapshot.apply_progress(create_mview_progress) {
                    let pending_barriers: VecDeque<_> =
                        [snapshot.finish_snapshot_barrier(barrier_interval_ms)]
                            .into_iter()
        ...
        match self {
            CreatingStreamingJobStatus::ConsumingSnapshot {
                ...
                *barrier_interval_ms = barrier_info.barrier_interval_ms;
                ...
                vec![(
                    snapshot
                        .next_fake_barrier(&barrier_info.kind, barrier_info.barrier_interval_ms),
                    mutation,
                )]
            }
            CreatingStreamingJobStatus::ConsumingLogStore {
