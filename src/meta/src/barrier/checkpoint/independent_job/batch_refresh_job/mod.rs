...
impl BatchRefreshJobCheckpointControl {
    pub(crate) fn on_new_upstream_barrier(
        ...
        if is_finished {
            ...
            let final_checkpoint = snapshot.finish_snapshot_barrier(self.barrier_interval_ms);
            ...
            let barrier_to_inject =
                snapshot.next_fake_barrier(&barrier_info.kind, barrier_info.barrier_interval_ms);
            Self::inject_barrier(
                self.control.info.partial_graph_id,
                partial_graph_manager,
                node_actors,
