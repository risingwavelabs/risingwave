...
impl IcebergV3JobCheckpointControl {
                ...
                let barriers_to_inject = match input_phase {
                    IcebergV3InputPhase::Snapshot {
                        ...
                        if available == 0 && mutation.is_none() {
                            ...
                            vec![snapshot.next_fake_barrier(
                                &barrier_info.kind,
            ...
            IcebergV3JobStatus::Running(input_phase) => match input_phase {
                IcebergV3InputPhase::Snapshot {
                    ...
                    if snapshot.apply_progress(progress) {
                        pending_upstream_barriers
                            .push_front(snapshot.finish_snapshot_barrier(self.barrier_interval_ms));
                        *input_phase = IcebergV3InputPhase::LogStore {
    ...
    pub(crate) fn start_apply_compaction(
        ...
        self.barrier_interval_ms = upstream_barrier.barrier_interval_ms;
        ...
        match &mut self.status {
            IcebergV3JobStatus::Running(input_phase) => match input_phase {
                IcebergV3InputPhase::Snapshot {
                    ...
                    let begin = snapshot.next_fake_barrier(
                        &BarrierKind::Checkpoint(vec![]),
                        upstream_barrier.barrier_interval_ms,
                    );
                    let end = snapshot.next_fake_barrier(
                        &BarrierKind::Checkpoint(vec![]),
                        upstream_barrier.barrier_interval_ms,
                    );
                    Ok((begin, end))
                ...
                IcebergV3InputPhase::LogStore { .. } => {
                    ...
                    Ok((
                        BarrierInfo {
                            ...
                            barrier_interval_ms: upstream_barrier.barrier_interval_ms,
                        },
                        BarrierInfo {
