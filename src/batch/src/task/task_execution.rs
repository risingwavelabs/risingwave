        ...
        *self.shutdown_tx.lock() = Some(shutdown_tx);
        ...
        let fut = async move {
            ...
            if let Some(task_metrics) = task_metrics {
                ...
                task_metrics
                    .remove_labels_after_finish(task_metrics.task_first_poll_delay.clone(), labels);
                task_metrics
                    .task_fast_poll_duration
                    .with_label_values(labels)
                    .set(cumulative.total_fast_poll_duration.as_secs_f64());
                task_metrics.remove_labels_after_finish(
                    task_metrics.task_fast_poll_duration.clone(),
                    labels,
                );
                task_metrics
                    .task_idle_duration
                ...
                task_metrics
                    .remove_labels_after_finish(task_metrics.task_idle_duration.clone(), labels);
                task_metrics
                    .task_poll_duration
                    .with_label_values(labels)
                    .set(cumulative.total_poll_duration.as_secs_f64());
                task_metrics
                    .remove_labels_after_finish(task_metrics.task_poll_duration.clone(), labels);
                task_metrics
                    .task_scheduled_duration
                    .with_label_values(labels)
                    .set(cumulative.total_scheduled_duration.as_secs_f64());
                task_metrics.remove_labels_after_finish(
                    task_metrics.task_scheduled_duration.clone(),
                    labels,
                );
                task_metrics
                ...
                task_metrics.remove_labels_after_finish(
                    task_metrics.task_slow_poll_duration.clone(),
                    labels,
                );
