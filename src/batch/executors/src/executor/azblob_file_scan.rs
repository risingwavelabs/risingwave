    ...
    fn execute_push_with_operators<'a>(
        ...
        if context.morsel_parallelism() > 1 {
            return async move {
                ...
                let source = AzblobFileScanMorselSource::new(
                    file_format,
                    file_location,
                    account_name,
                    account_key,
...
struct AzblobFileScanMorselSource {
    file_format: FileFormat,
...
impl AzblobFileScanMorselSource {
    fn new(
        file_format: FileFormat,
        file_location: Vec<String>,
        account_name: String,
    ...
    async fn read_file(
        ...
        let (bucket, file_name) = extract_bucket_and_file_name(&file, &FileScanBackend::Azblob)?;
        let op = new_azblob_operator(endpoint, account_name, account_key, bucket)?;
        let chunk_stream =
...
impl MorselSource for AzblobFileScanMorselSource {
    fn next_morsel<'a>(
        &'a mut self,
