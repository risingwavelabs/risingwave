...
fn tz_step(
    ...
    tz_step_impl_captured(start, stop, step)
}
...
fn tz_step_impl(
    ...
    let time_zone = Timestamptz::lookup_time_zone(time_zone).unwrap();
...
impl CheckedAddWithExtra<Interval, Tz> for Timestamptz {
    ...
    fn checked_add_with_extra(self, rhs: Interval, extra: Tz) -> Option<Self::Output> {
        // placeholder
        let usecs = self.timestamp_micros();
        let usecs = usecs.checked_add(86400_000_000)?;
        let ans = Timestamptz::from_micros(usecs);
        eprintln!("==== {extra}");
        Some(ans)
    }
}
