...
fn jsonb_path_exists(target: JsonbRef<'_>, vars: JsonbRef<'_>, path: &JsonPath) -> Result<bool> {
    let exist = path
        .exists_with_vars::<ValueRef<'_>>(target.into(), vars.into())
        .map_err(eval_error)?;
    Ok(exist)
}
...
fn jsonb_path_match(target: JsonbRef<'_>, vars: JsonbRef<'_>, path: &JsonPath) -> Result<bool> {
    let matched = path
