...
use hex;
use risingwave_common::types::bytea::{parse_bytes_hex, parse_bytes_traditional};
...
pub fn decode(data: &str, format: &str) -> Result<Box<[u8]>> {
    match format {
        ...
        "hex" => Ok(parse_bytes_hex(data).map_err(ExprError::Parse)?.into()),
        "escape" => Ok(parse_bytes_traditional(data)
            .map_err(ExprError::Parse)?
            .into()),
