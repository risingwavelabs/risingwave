            ...
            Box::pin(async move { paste! {
                ...
                Ok(match ($([<val_ $arg:lower>], )*) {
                    ...
                    ($([<val_ $arg:lower>], )*) => {
                        ...
                        let mut output_array = <$OA as Array>::Builder::with_meta(data_chunk.capacity(), self.return_type.clone());
    ...
    for<'a> &'a OA: std::convert::From<&'a ArrayImpl>,
{
    ...
    async fn eval(&self, data_chunk: &DataChunk) -> Result<ArrayRef> {
        ...
        let mut output_array =
            OA::Builder::with_meta(data_chunk.capacity(), self.return_type.clone());
