...
impl FunctionAttr {
    ...
    fn generate_build_scalar_function(
        ...
        let append_output = match user_fn.writer_type_kind {
            ...
            | Some(WriterTypeKind::VectorWrite) => quote! {{
        ...
        let row_output = match user_fn.writer_type_kind {
            ...
            Some(WriterTypeKind::VectorWrite) => quote! {{
                let mut builder = {
                    let DataType::Vector(elem_size) = &self.context.return_type else {
                        panic!("data type must be DataType::Vector");
                    };
                    VectorArrayBuilder::with_type(1, DataType::Vector(*elem_size))
                };
                let committed = {
                    let mut writer = builder.writer();
                    let committed = #output.is_some();
