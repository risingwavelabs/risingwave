...
#[tokio::test]
async fn test_exactly_once_sink_2pc_basic() -> Result<()> {
...
#[tokio::test]
async fn test_exactly_once_iceberg_sink() {
    let err_rate_list = vec![0.1, 0.1, 0.1, 0.0, 0.0];
    let _ = test_exactly_once_iceberg_sink_inner(err_rate_list).await;
}
...
async fn test_exactly_once_iceberg_sink_inner(err_rate_list: Vec<f64>) -> Result<()> {
    let mut cluster = start_sink_test_cluster().await?;

    let source_parallelism = 6;

    let test_sink =
        SimulationTestIcebergExactlyOnceSink::register_with_iceberg_committer_with_err_rate(
            err_rate_list,
        );
    let test_source = SimulationTestSource::register_new(source_parallelism, 0..100000, 0.2, 20);
    let mut session = cluster.start_session();
