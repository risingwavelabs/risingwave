...
use risingwave_common::config::{load_config, RwConfig, StorageConfig, NO_OVERWRITE};
...
pub async fn start_meta_node(listen_addr: String, config_path: String) {
    ...
    let config = load_config(&meta_opts.config_path.clone(), Some(meta_opts.clone()));
    assert!(
...
async fn start_replay(
    ...
    let config = load_config(&opts.config_path_for_meta, NO_OVERWRITE);
    tracing::info!(
