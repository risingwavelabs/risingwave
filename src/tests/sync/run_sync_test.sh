...
RUSTFLAGS='--cfg tokio_unstable --cfg sync_test'
export RUSTFLAGS=${RUSTFLAGS}

cp -R -n "${RW_WORKSPACE}"/e2e_test "${SCRIPT_PATH}"/slt/ || :

# cargo test -p risingwave_sync_test
...
cargo test -p risingwave_sync_test -- --list --format=terse| awk '/: test/{print substr($1, 1, length($1)-1)}' |xargs -I{} cargo test -p risingwave_sync_test {}
