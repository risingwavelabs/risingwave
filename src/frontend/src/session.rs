...
use crate::monitor::{
    CursorMetrics, FrontendMetrics, NimtableMetrics, GLOBAL_FRONTEND_METRICS,
    GLOBAL_NIMTABLE_METRICS,
};
...
impl FrontendEnv {
    ...
    pub async fn init(opts: FrontendOpts) -> Result<(Self, Vec<JoinHandle<()>>, Vec<Sender<()>>)> {
        ...
        let nimtable_metrics = Arc::new(GLOBAL_NIMTABLE_METRICS.clone());
        ...
        let join_handle = tokio::spawn(async move {
            loop {
                if let Err(err) =
                    report_iceberg_metrics(&catalog_reader_for_report, &nimtable_metrics).await
                {
