...
impl Binder {
    ...
    pub(super) fn bind_match_recognize(
        ...
        // WITHIN: lower the bound to `last <= first + bound` over a synthetic [last, first] row
        // and to the deadline `first + bound`, using the leading ORDER BY column's type.
        let (within, within_deadline) = match within {
            Some(e) => {
                ...
                // semantics.)
                let folded_bound = bound.try_fold_const().expect("checked is_const")?;
                let is_positive = match &folded_bound {
                    None => Some(false),
                ...
                // Calendar-month addition can reverse the order of deadlines at month end.
                // The finality gate relies on earlier starts having no later deadline, so reject
                // month-bearing bounds (including years) after folding constant expressions.
                if matches!(&folded_bound, Some(ScalarImpl::Interval(iv)) if iv.months() != 0) {
                    return Err(crate::error::ErrorCode::NotSupported(
                        "MATCH_RECOGNIZE WITHIN intervals with a nonzero month component"
                            .to_owned(),
                        "use an interval with only non-negative days and seconds; calendar months \
                         and years can make deadlines non-monotone in the match start time"
                            .to_owned(),
                    )
                    .into());
                }
