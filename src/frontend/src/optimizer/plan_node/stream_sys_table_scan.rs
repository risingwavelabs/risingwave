...
/// `StreamSysTableScan` is a virtual plan node to represent a stream table scan. It will be converted
/// to stream scan + merge node (for upstream materialize) + batch table scan when converting to `MView`
/// creation request.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct StreamSysTableScan {
...
impl StreamSysTableScan {
    ...
    pub fn to_index_scan(
        ...
        StreamSysTableScan::new_with_stream_scan_type(logical_index_scan, stream_scan_type)
    }
...
impl_plan_tree_node_for_leaf! { StreamSysTableScan }

impl Distill for StreamSysTableScan {
    fn distill<'a>(&self) -> XmlNode<'a> {
        ...
        childless_record("StreamSysTableScan", vec)
    }
}

impl StreamNode for StreamSysTableScan {
...
impl ExprRewritable for StreamSysTableScan {
