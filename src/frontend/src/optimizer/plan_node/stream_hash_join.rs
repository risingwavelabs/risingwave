...
pub struct StreamHashJoin {
    ...
    /// `(conjunction_idx, clean_left_state, clean_right_state, InequalityInputPairV2)`.
    ...
    inequality_pairs: Vec<(usize, bool, bool, InequalityInputPairV2)>,
...
impl StreamHashJoin {
    ...
    pub fn inequality_pairs(&self) -> &Vec<(usize, bool, bool, InequalityInputPairV2)> {
    ...
    fn infer_internal_and_degree_table_catalog(
        ...
        let clean_watermark_indices =
            self.infer_clean_watermark_indices(&join_key_indices, is_left);
        // Infer clean_watermark_indices for degree table state cleaning.
        ...
        degree_table_catalog_builder
            .set_clean_watermark_indices(eq_join_key_clean_watermark_indices);
    ...
    fn infer_clean_watermark_indices(
        ...
        let eq_join_key_clean_watermark_indices =
            self.infer_eq_join_key_clean_watermark_indices(join_key_indices);
        clean_watermark_indices.extend(eq_join_key_clean_watermark_indices);
        ...
        for (_conjunction_idx, clean_left, clean_right, pair) in &self.inequality_pairs {
            if is_left && *clean_left {
                ...
                if !clean_watermark_indices.contains(&col_idx) {
                    clean_watermark_indices.push(col_idx);
                }
