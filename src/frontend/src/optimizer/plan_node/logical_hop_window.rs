...
impl ColPrunable for LogicalHopWindow {
    fn prune_col(&self, required_cols: &[usize]) -> PlanRef {
        ...
        let require_win_start = upstream_required_cols.contains(self.window_start_col_idx());
        let require_win_end = upstream_required_cols.contains(self.window_end_col_idx());

        let upstream_required_cols = {
            let o2i = self.o2i_col_mapping();
            o2i.rewrite_bitset(&upstream_required_cols)
                .ones()
                .collect_vec()
        };
