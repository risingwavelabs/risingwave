...
impl ToStream for LogicalMatchRecognize {
    fn to_stream(&self, ctx: &mut ToStreamContext) -> Result<super::StreamPlanRef> {
        ...
        // Ordered-input planning: hash-shard by the PARTITION BY key, then an EowcSort over
        ...
        let sorted_input: super::StreamPlanRef =
            StreamEowcSort::with_secondary_order(stream_input, time_col, secondary_order).into();
        let core = generic::MatchRecognize {
            input: sorted_input,
