...
use crate::expr::{
    Expr, ExprImpl, ExprRewriter, ExprType, ExprVisitor, FunctionCall, collect_input_refs,
};
...
use crate::optimizer::plan_node::{
    ...
    EqJoinPredicate, ExprRewritable, Logical, LogicalJoin, LogicalPlanRef as PlanRef,
    LogicalProject, LogicalScan, PlanBase, PlanTreeNodeUnary, PredicatePushdown,
            ...
            for index in scan.vector_indexes() {
                ...
                return Ok(if non_covered_table_cols_idx.is_empty() {
                    ...
                    let mut primary_table_cols_idx = Vec::with_capacity(
                        non_covered_table_cols_idx.len() + scan.table_catalog().pk().len(),
                    );
                    primary_table_cols_idx.extend(
                        non_covered_table_cols_idx.iter().cloned().chain(
                            scan.table_catalog()
                                .pk()
                                .iter()
                    ...
                    let on_condition = Condition {
                        conjunctions: index
                            ...
                            .iter()
                            .zip_eq_debug(0..scan.table_catalog().pk().len())
