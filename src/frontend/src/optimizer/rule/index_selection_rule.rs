...
impl IndexCost {
    ...
    /// Returns true if `self` has a strictly lower cost than `other`.
    ///
    /// This is deliberately strict (`<`, not `<=`): ties go to the primary table.
    /// That matters when costs saturate at [`IndexCost::maximum`], where every
    /// candidate lands on the same ceiling — strict comparison keeps the planner
    /// on the base table instead of picking an arbitrary index by catalog order.
