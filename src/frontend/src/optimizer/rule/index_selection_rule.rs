...
/// Ordering is lexicographic over `(cost, primary_lookup)` and is what `min` and
/// `choose_min_cost_path` use; [`IndexCost::strictly_cheaper_than`] intentionally compares `cost`
/// only, so the two orderings are not interchangeable.
...
impl IndexCost {
    ...
    ///
    /// Every call site compares against a running minimum seeded with the primary table's cost and
    /// keeps the incumbent on a tie, so ties deliberately go to the primary table. That matters
    /// when costs saturate: `new` clamps at `maximum` and `Default` is already `maximum`, so on a
    /// wide table (no-predicate multiplier 4000, i.e. row size >= 2500) the primary table and
    /// every index all cost exactly 10,000,000. Strict `<` keeps the primary table there, while
    /// `<=` would degenerate selection into catalog order.
