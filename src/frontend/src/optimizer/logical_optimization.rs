...
impl LogicalOptimizer {
    ...
    pub fn gen_optimized_logical_plan_for_stream(mut plan: PlanRef) -> Result<PlanRef> {
        ...
        let enable_share_plan = ctx
            .session_ctx()
            .running_sql_runtime_parameters(RuntimeParameters::enable_share_plan);
        ...
        if plan
            .ctx()
            .session_ctx()
            .running_sql_runtime_parameters(RuntimeParameters::enable_join_ordering)
        {
            ...
            if plan
                .ctx()
                .session_ctx()
                .running_sql_runtime_parameters(RuntimeParameters::streaming_enable_bushy_join)
            {
        ...
        let force_split_distinct_agg = ctx
            .session_ctx()
            .running_sql_runtime_parameters(RuntimeParameters::force_split_distinct_agg);
