...
impl<C: ConventionMarker> PlanRef<C> {
    async fn optimize_by_rules_inner(
        self,
        ...
        let result = heuristic_optimizer.optimize(self).await;
    ...
    pub(crate) async fn optimize_by_rules(
        self,
        OptimizationStage {
            stage_name,
            rules,
    ...
    pub(crate) async fn optimize_by_rules_until_fix_point(
        mut self,
        OptimizationStage {
            stage_name,
            rules,
        ...
        loop {
            ...
            self = self
                .optimize_by_rules_inner(&mut heuristic_optimizer, stage_name)
                .await?;
...
impl LogicalOptimizer {
    ...
    pub async fn subquery_unnesting(
        ...
        // Simple Unnesting.
        plan = plan.optimize_by_rules(&SIMPLE_UNNESTING).await?;
        ...
        plan = plan.optimize_by_rules(&VALUES_EXTRACT_PROJECT).await?;
        // General Unnesting.
        ...
        plan = plan
            .optimize_by_rules_until_fix_point(&GENERAL_UNNESTING_PUSH_DOWN_APPLY)
            .await?;
    ...
    pub async fn gen_optimized_logical_plan_for_stream(
        mut plan: LogicalPlanRef,
    ) -> Result<LogicalPlanRef> {
        let ctx = plan.ctx();
        ...
        plan = plan.optimize_by_rules(&GROUPING_SETS).await?;
        // Remove nodes with constant output.
        plan = plan.optimize_by_rules(&CONSTANT_OUTPUT_REMOVE).await?;
        // Remove project to make common sub-plan sharing easier.
        plan = plan.optimize_by_rules(&PROJECT_REMOVE).await?;
        ...
        if enable_share_plan {
            ...
            plan = plan.optimize_by_rules(&DAG_TO_TREE).await?;
        ...
        plan = plan.optimize_by_rules(&SET_OPERATION_MERGE).await?;
        plan = plan.optimize_by_rules(&SET_OPERATION_TO_JOIN).await?;
        ...
        plan = plan
            .optimize_by_rules(&STREAM_GENERATE_SERIES_WITH_NOW)
            .await?;
        // In order to unnest a table function, we need to convert it into a `project_set` first.
        plan = plan.optimize_by_rules(&TABLE_FUNCTION_CONVERT).await?;

        plan = plan
            .optimize_by_rules(&CORRELATED_TOP_N_TO_VECTOR_SEARCH_FOR_STREAM)
            .await?;

        plan = Self::subquery_unnesting(plan, enable_share_plan, explain_trace, &ctx).await?;
        ...
        // predicate push down
        plan = plan
            .optimize_by_rules(&LOGICAL_FILTER_EXPRESSION_SIMPLIFY)
            .await?;
        ...
        if plan.ctx().session_ctx().config().enable_join_ordering() {
            ...
            plan = plan.optimize_by_rules(&TO_MULTI_JOIN).await?;
            ...
            {
                plan = plan.optimize_by_rules(&BUSHY_TREE_JOIN_ORDERING).await?;
            } else {
                plan = plan.optimize_by_rules(&LEFT_DEEP_JOIN_ORDERING).await?;
            }
        }
        ...
        plan = plan.optimize_by_rules(&FILTER_WITH_NOW_TO_JOIN).await?;

        // Push down the calculation of inputs of join's condition.
        plan = plan.optimize_by_rules(&PUSH_CALC_OF_JOIN).await?;

        plan = plan.optimize_by_rules(&SPLIT_OVER_WINDOW).await?;
        // Must push down predicates again after split over window so that OverWindow can be
        // optimized to TopN.
        plan = Self::predicate_pushdown(plan, explain_trace, &ctx);
        plan = plan.optimize_by_rules(&CONVERT_OVER_WINDOW).await?;
        plan = plan.optimize_by_rules(&MERGE_OVER_WINDOW).await?;
        ...
        plan = plan.optimize_by_rules(&SIMPLIFY_AGG).await?;

        plan = plan.optimize_by_rules(&JOIN_COMMUTE).await?;
        ...
        plan = plan.optimize_by_rules(&COMMON_SUB_EXPR_EXTRACT).await?;
    ...
    pub async fn gen_optimized_logical_plan_for_batch(
        ...
        plan = plan.optimize_by_rules(&REWRITE_SOURCE_FOR_BATCH).await?;
        plan = plan.optimize_by_rules(&GROUPING_SETS).await?;
        plan = plan.optimize_by_rules(&REWRITE_LIKE_EXPR).await?;
        plan = plan.optimize_by_rules(&SET_OPERATION_MERGE).await?;
        plan = plan.optimize_by_rules(&SET_OPERATION_TO_JOIN).await?;
        plan = plan.optimize_by_rules(&ALWAYS_FALSE_FILTER).await?;
        // Table function should be converted into `file_scan` before `project_set`.
        plan = plan.optimize_by_rules(&TABLE_FUNCTION_TO_FILE_SCAN).await?;
        plan = plan
            .optimize_by_rules(&TABLE_FUNCTION_TO_POSTGRES_QUERY)
            .await?;
        plan = plan
            .optimize_by_rules(&TABLE_FUNCTION_TO_MYSQL_QUERY)
            .await?;
        plan = plan
            .optimize_by_rules(&TABLE_FUNCTION_TO_INTERNAL_BACKFILL_PROGRESS)
            .await?;
        plan = plan
            .optimize_by_rules(&TABLE_FUNCTION_TO_INTERNAL_GET_CHANNEL_DELTA_STATS)
            .await?;
        plan = plan
            .optimize_by_rules(&TABLE_FUNCTION_TO_INTERNAL_SOURCE_BACKFILL_PROGRESS)
            .await?;
        // In order to unnest a table function, we need to convert it into a `project_set` first.
        plan = plan.optimize_by_rules(&TABLE_FUNCTION_CONVERT).await?;

        plan = plan
            .optimize_by_rules(&CORRELATED_TOP_N_TO_VECTOR_SEARCH_FOR_BATCH)
            .await?;

        plan = Self::subquery_unnesting(plan, false, explain_trace, &ctx).await?;
        ...
        // Convert distinct aggregates.
        plan = plan
            .optimize_by_rules(&CONVERT_DISTINCT_AGG_FOR_BATCH)
            .await?;
        ...
        plan = plan.optimize_by_rules(&TOP_N_TO_VECTOR_SEARCH).await?;
        ...
        plan = plan.optimize_by_rules(&PULL_UP_HOP).await?;

        plan = plan.optimize_by_rules(&TOP_N_AGG_ON_INDEX).await?;

        plan = plan.optimize_by_rules(&LIMIT_PUSH_DOWN).await?;
