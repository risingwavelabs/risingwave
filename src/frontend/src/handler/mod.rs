...
pub struct HandlerArgs {
    ...
    pub redacted_sql: Option<String>,
}
...
impl HandlerArgs {
    pub fn new(session: Arc<SessionImpl>, stmt: &Statement, sql: Arc<str>) -> Result<Self> {
        Ok(Self {
            ...
            redacted_sql: try_redact_statement(stmt).as_ref().map(Self::normalize_sql),
        })
    }
...
/// Returns a redacted and normalized SQL, or None if fails.
