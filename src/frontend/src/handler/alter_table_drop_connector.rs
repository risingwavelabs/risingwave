...
use crate::utils::RETENTION_SECONDS;
...
fn fetch_schema_info(
    ...
    let (source_def, _) =
        reader.get_any_source_by_id(db_name, schema_path, &source_id.table_id())?;
...
pub async fn handle_alter_table_drop_connector(
    ...
    catalog_writer
        .replace_table(source, table, graph, col_index_mapping, job_type, true)
        .await?;
