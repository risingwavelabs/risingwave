...
use crate::optimizer::{
    BatchPlanRoot, ExecutionModeDecider, LogicalIcebergScanExt, LogicalPlanToDataFusionExt,
    OptimizerContext, OptimizerContextRef, RelationCollectorVisitor, SysTableVisitor,
};
use crate::planner::Planner;
use crate::scheduler::plan_fragmenter::Query;
use crate::scheduler::{
    BatchPlanFragmenter, DistributedQueryStream, ExecutionContext, ExecutionContextRef,
    LocalQueryExecution, LocalQueryStream, SchedulerError,
};
...
pub async fn handle_query(
    ...
    let fut: Either<_, _> = match gen_batch_plan_by_statement(&session, context.into(), stmt)? {
    ...
    match statement {
        ...
        | Statement::Update { .. } => {
            ...
            let plan_fragmenter_result = {
                let context = OptimizerContext::from_handler_args(handler_args);
                let BatchPlanChoice::Rw(plan_result) =
                    gen_batch_query_plan(&session, context.into(), bound_result)?
                else {
                    bail_not_implemented!(
                        "Executing DataFusion plan is not supported in prepared statements yet"
                    );
                };
            ...
            risingwave_sqlparser::ast::DeclareCursor::Query(_) => {
                ...
                let plan_fragmenter_result = {
                    ...
                    let BatchPlanChoice::Rw(plan_result) =
                        ...
                        bail_not_implemented!(
                            "Executing DataFusion plan is not supported in cursor declaration yet"
                        );
                    };
                    gen_batch_plan_fragmenter(&session, plan_result)?
                };
...
fn gen_batch_query_plan(
    ...
    if session.config().enable_datafusion_engine() && optimized_logical.plan.contains_iceberg_scan()
    {
        // Convert RisingWave logical plan to DataFusion logical plan for Iceberg queries.
        let Some(df_plan) = optimized_logical.plan.to_datafusion_logical_plan() else {
            bail_not_implemented!(
                "Some part of the plan cannot be converted to DataFusion plan for Iceberg query."
            );
        };
...
pub async fn execute_datafusion_plan(
    ...
    let rw_fields = plan
        ...
        .map(|arrow_field| {
            risingwave_common::catalog::Field::new(
                arrow_field.name(),
                IcebergArrowConvert.type_from_field(arrow_field).unwrap(),
            )
        })
        .collect_vec();
    ...
    // FIXME: It will optimize logical plan twice. Once for risingwave and once for datafusion.
