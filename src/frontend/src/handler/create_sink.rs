...
use risingwave_connector::sink::catalog::{SinkCatalog, SinkDesc};
...
pub fn gen_sink_plan(
    ...
    Ok((
        sink_plan,
        sink_desc.into_catalog(
            risingwave_common::catalog::SchemaId::new(sink_schema_id as i32),
            risingwave_common::catalog::DatabaseId::new(sink_database_id as i32),
            session.user_id(),
            vec![],
        ),
    ))
