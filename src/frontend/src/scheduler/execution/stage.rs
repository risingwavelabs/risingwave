...
impl StageExecution {
  ...
  pub async fn start(&self) -> Result<()> {
    ...
    match &*s {
      ...
      _ => {
        // This is possible since we notify stage schedule event to query runner, which may receive
        // multi events and start stage multi times.
        info!("Staged {:?}-{:?} already started, skipping.", &self.stage.query_id, &self.stage.id);
        Ok(())
      },
    }
  }
