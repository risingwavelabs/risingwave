...
#[test]
fn parse_offset() {
    let expect = Some(Expr::Value(number("2")));
    ...
    assert_eq!(ast.offset, Some(Expr::Value(number("1"))));
    let ast = verified_query("SELECT 'foo' OFFSET 1");
    assert_eq!(ast.offset, Some(Expr::Value(number("1"))));

    // OFFSET accepts a general expression, not just a bare number literal.
    let ast = verified_query("SELECT foo FROM bar OFFSET 1 + 1");
    assert_eq!(
        ast.offset,
        Some(Expr::BinaryOp {
            left: Box::new(Expr::Value(number("1"))),
    ...
    // Parameterized LIMIT/OFFSET as generated by Prisma (`LIMIT $1 OFFSET $2`).
    // Regression test for https://github.com/risingwavelabs/risingwave/issues/23345.
    let ast = verified_query("SELECT foo FROM bar LIMIT $1 OFFSET $2");
    assert_eq!(ast.limit, Some(Expr::Parameter { index: 1 }));
    assert_eq!(ast.offset, Some(Expr::Parameter { index: 2 }));
}
