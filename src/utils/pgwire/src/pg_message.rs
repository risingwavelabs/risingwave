...
pub enum FeMessage {
    ...
    HealthCheck,
    // The original message has been rejected due to server throttling. This is a placeholder message.
    ServerThrottle,
}
...
impl FeMessage {
    ...
    pub async fn read_header(stream: &mut (impl AsyncRead + Unpin)) -> Result<FeMessageHeader> {
    ...
    pub async fn read_body(
        ...
        header: FeMessageHeader,
    ) -> Result<FeMessage> {
        let FeMessageHeader { tag, payload_len } = header;
        ...
        match tag {
            ...
            _ => Err(std::io::Error::new(
                ErrorKind::InvalidInput,
                format!("Unsupported tag of regular message: {}", tag),
            )),
        }
    }

    pub async fn skip_body(
        stream: &mut (impl AsyncRead + Unpin),
        header: FeMessageHeader,
    ) -> Result<()> {
        let FeMessageHeader {
        ...
        if payload_len > 0 {
            ...
            let remain = (payload_len as usize) % BUF_SIZE;
            if remain > 0 {
                buf.truncate(remain);
                stream.read_exact(&mut buf).await?;
            }
