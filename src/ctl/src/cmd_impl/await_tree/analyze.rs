...
use crate::cmd_impl::await_tree::utils::{extract_actor_traces, parse_tree_view_from_text};
...
pub async fn bottleneck_detect(context: &CtlContext, path: Option<String>) -> anyhow::Result<()> {
    if let Some(path) = path {
        bottleneck_detect_from_file(path)
    } else {
...
fn bottleneck_detect_from_file(path: String) -> anyhow::Result<()> {
