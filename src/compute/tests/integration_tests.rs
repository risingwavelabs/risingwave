...
async fn test_table_v2_materialize() -> Result<()> {
    ...
    let scan = Box::new(RowSeqScanExecutor::new(
        table.schema().clone(),
        ScanType::TableScan(table.batch_dedup_pk_iter(u64::MAX, &ordered_column_descs).await?),
        1024,
        true,
...
async fn test_row_seq_scan() -> Result<()> {
    ...
    let executor = Box::new(RowSeqScanExecutor::new(
        table.schema().clone(),
        ScanType::TableScan(table.batch_dedup_pk_iter(u64::MAX, &pk_descs).await.unwrap()),
        1,
        true,
        "RowSeqScanExecutor2".to_string(),
        Arc::new(BatchMetrics::unused()),
