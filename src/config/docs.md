...
| periodic_purge_stale_compaction_group_interval_sec | Interval of purging empty dynamic compaction groups from the latest Hummock version, and purging stale compaction-group configs that are not referenced by the version. Defaults to 1 hour. Set to 0 to disable the periodic purge. | 3600 |
