...
| compactor_absolute_max_task_multiplier | Allows the compactor to use more tasks than `max_pull_task_count` if the CPU usage is lower than `compactor_cpu_usage_threshold`. | 9.0 |
| compactor_concurrent_uploading_sst_count | The concurrent uploading number of `SSTables` of builder |  |
| compactor_cpu_usage_threshold |  | 0.800000011920929 |
...
| enforce_secret_on_cloud | Whether to enforce secret on cloud. | false |
