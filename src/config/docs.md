...
| Config | Description | Default |
|--------|-------------|---------|
| blob_index_size | Set the blob index size for each blob. A larger blob index size can hold more blob entries, but it will also increase the io size of each blob part write. NOTE: - The size will be aligned up to a multiplier of 4K. - Modifying this configuration will invalidate all existing file cache data. Default: 16 `KiB` | 16384 |
| capacity_mb |  | 1024 |
| compression |  | "None" |
| dir |  | "" |
