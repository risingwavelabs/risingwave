...
| checkpoint_compression_algorithm | Compression algorithm for hummock version checkpoint. Set to `none` to write checkpoints in the legacy raw protobuf format. | "zstd" |
...
| slow_query_threshold_ms | Threshold in milliseconds for logging slow queries issued to the meta store. | 1000 |
