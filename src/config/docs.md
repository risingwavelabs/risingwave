...
| remote_input_subscription_rate_limit | The maximum rate per second at which a compute node starts remote exchange subscriptions. This smooths actor startup and recovery bursts without limiting the number of in-flight subscriptions, which could deadlock actors with remote input dependencies. As a rule of thumb, keep `rate >= remote exchange inputs on the busiest compute node / target recovery seconds`. Set to 0 to disable the limit. | 2048 |
