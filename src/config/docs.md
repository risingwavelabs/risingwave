...
| data_file_cache_admission_filter | Filter applied before admitting a block into the data file cache. `none` accepts every block; `live-sst` accepts only blocks whose SST is live and belongs to the cache's level partition. | "none" |
| data_file_cache_reinsertion_filter | Filter applied before reinserting a block during data file cache reclaim. `none` accepts every block; `live-sst` accepts only blocks whose SST is live and belongs to the cache's level partition. | "none" |
