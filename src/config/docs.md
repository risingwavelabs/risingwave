...
| Config | Description | Default |
|--------|-------------|---------|
| blocked_xor_filter_kv_count_threshold | KV-count threshold for using blocked SST filters when output filter layout is "auto". When `sstable_filter_layout[level]` is "auto", compaction will build blocked SST filters if the estimated total key count of the task exceeds this threshold. Otherwise it will build a single non-blocked SST filter. Note: shared-buffer flush does not read compaction group config, and always uses the built-in default threshold. | 262144 |
...
| sstable_filter_kind | Compression-style per-level SST filter family for compaction output. Index 0 applies to L0 and levels above the dynamic base level. Index 1 applies to the current dynamic base level. Index 2+ applies to levels below the base level. Shared-buffer flush ignores this config and always uses "binary_fuse16". | ["binary_fuse16", "binary_fuse16", "binary_fuse16", "binary_fuse8", "binary_fuse16", "binary_fuse16", "binary_fuse16"] |
| sstable_filter_layout | Compression-style per-level SST filter layout for compaction output. "auto" uses the kv-count heuristic; "plain"/"normal" forces non-blocked filters and ignores kv-count threshold. Index 0 applies to L0 and levels above the dynamic base level. Index 1 applies to the current dynamic base level. Index 2+ applies to levels below the base level. Shared-buffer flush ignores this config and always uses "auto". | ["auto", "auto", "auto", "auto", "auto", "auto", "auto"] |
