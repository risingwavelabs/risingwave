...
| sstable_filter_kind | Per-level xor filter family for compaction output. Index by LSM level: `0..=max_level`. Note: L0 (index 0) is currently ignored by shared-buffer flush, which always uses "xor16". | ["xor16", "xor16", "xor16", "xor16", "xor16", "xor8", "xor8"] |
...
| skip_inheritance_filter | Skip checking inheritance filter on data refill. This option is suitable for a single compute node or debugging. | false |
...
| cache_refill_policy | Cache refill policy for streaming cache refill feature. Can be `unspecified`, `disabled`, `streaming`, `serving` or `both`. | "enabled" |
