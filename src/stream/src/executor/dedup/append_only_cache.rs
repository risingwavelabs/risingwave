...
/// [`AppendOnlyDedupCache`] is used for key deduplication. Currently, the cache behaves like a set that only
/// accepts a key without a value. This could be refined in the future to support k-v pairs.
pub struct AppendOnlyDedupCache<K: Hash + Eq + EstimateSize> {
    inner: ManagedLruCache<K, ()>,
}

impl<K: Hash + Eq + EstimateSize> AppendOnlyDedupCache<K> {
...
mod tests {
    ...
    use super::AppendOnlyDedupCache;
    use crate::common::metrics::MetricsInfo;

    #[test]
    fn test_dedup_cache() {
        let mut cache =
            AppendOnlyDedupCache::new(Arc::new(AtomicU64::new(10000)), MetricsInfo::for_test());
