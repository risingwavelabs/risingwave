            ...
            'backfill_loop: while let Some(either) = backfill_stream.next().await {
                match either {
                    ...
                    Either::Right(msg) => {
                        ...
                        // Process all rows including invisible ones (for EOF detection).
                        // Invisible rows are generated by heartbeat mechanism when Kafka
                        // partition reaches EOF due to transaction control messages.
                        for (i, row_opt) in chunk.rows_with_holes().enumerate() {
                            match row_opt {
                                Some((_, row)) => {
                                    // Visible row: normal processing
                                ...
                                None => {
                                    // Invisible row (heartbeat/EOF): extract offset for state update
                                    // This handles Kafka EOF signals where control messages
                                    // (COMMIT/ABORT) occupy offsets but are not visible.
                                    let data_chunk = chunk.data_chunk();
                                    let split_id =
                                        data_chunk.column_at(split_idx).as_utf8().value_at(i);
                                    let offset =
                                        data_chunk.column_at(offset_idx).as_utf8().value_at(i);
                                    if let (Some(split_id), Some(offset)) = (split_id, offset) {
                                        // Update state but don't mark as visible
                                        backfill_stage.handle_backfill_row(split_id, offset);
                                    }
                                    // new_vis stays false for invisible rows
                                }
                            }
                        }
