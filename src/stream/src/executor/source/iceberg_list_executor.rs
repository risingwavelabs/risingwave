...
impl<S: StateStore> IcebergListExecutor<S> {
    ...
    async fn into_stream(mut self) {
        ...
        let columns = source_desc_builder.column_catalogs_to_source_column_descs();
        let column_names = columns.iter().map(|c| c.name.clone()).collect::<Vec<_>>();
        ...
        if last_snapshot.is_none() {
            ...
            if let Some(start_snapshot) = table.metadata().current_snapshot() {
                ...
                let snapshot_scan = table
                    ...
                    // FIXME: select by name might be not robust under schema evolution
                    .select(column_names.iter())
                    .build()
