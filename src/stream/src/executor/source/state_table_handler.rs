...
pub(crate) mod tests {
    ...
    async fn test_from_table_catalog() {
        ...
        let init_epoch_num = TestEpoch::new_without_offset(1).as_u64();
        let init_epoch = EpochPair::new_test_epoch(init_epoch_num);
        let next_epoch =
            EpochPair::new_test_epoch(init_epoch_num + TestEpoch::new_without_offset(1).as_u64());
    ...
    async fn test_set_and_get() -> StreamExecutorResult<()> {
        ...
        let epoch_1 = EpochPair::new_test_epoch(TestEpoch::new_without_offset(1).as_u64());
        let epoch_2 = EpochPair::new_test_epoch(TestEpoch::new_without_offset(2).as_u64());
        let epoch_3 = EpochPair::new_test_epoch(TestEpoch::new_without_offset(3).as_u64());
