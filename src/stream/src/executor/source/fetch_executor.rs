...
use risingwave_connector::source::{StreamChunkWithState, SplitMetaData, SplitImpl, SplitId};
...
use risingwave_common::catalog::{Schema, ColumnId, TableId};
use risingwave_connector::ConnectorParams;
use risingwave_connector::source::{SourceContext, BoxSourceWithStateStream, filesystem::FsSplit, SourceCtrlOpts};
use risingwave_source::source_desc::{SourceDesc, SourceDescBuilder};
use crate::common::table::state_table::KeyedRowStream;
use crate::executor::{StreamSourceCore, Mutation, SourceStateTableHandler};
use crate::executor::{
    ActorContextRef, Executor, BoxedMessageStream, PkIndicesRef, PkIndices, Message, StreamExecutorError, 
    BoxedExecutor, expect_first_barrier, stream_reader::StreamReaderWithPause, StreamExecutorResult,
};
...
impl<S: StateStore> FsFetchExecutor<S> {
    ...
    async fn try_replace_with_new_reader<'a, const BIASED: bool>(
        ...
        let fs_split = 
            if let Some(item) = store_iter.next().await {
                ...
                // Find incompleted assignment in state cache.
                state_cache.iter().find(|(_, split)| {
        ...
        if let Some(fs_split) = fs_split {
            stream.replace_data_stream(
                Self::build_stream_source_reader(column_ids, source_ctx, source_desc, fs_split).await?
            );
    ...
    async fn into_stream(mut self) {
        ...
        let source_desc_builder: SourceDescBuilder = core
            .source_desc_builder
            .take()
            .unwrap();
                ...
                Ok(msg) => {
                    match msg {
                        ...
                        Either::Right(StreamChunkWithState {
                            ...
                            let mut cache_entry = match state_cache
                                .entry(split_id.to_owned()) {
