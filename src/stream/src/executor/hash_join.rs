...
impl<K: HashKey, S: StateStore, const T: JoinTypePrimitive> HashJoinExecutor<K, S, T> {
    ...
    pub fn new(
        ...
        let (new_column_n, schema_fields) = match T {
            JoinType::LeftSemi | JoinType::LeftAnti => {
                let schema_fields = input_l.schema().fields.clone();
                let new_column_n = input_l.schema().len();
                (new_column_n, schema_fields)
            }
            _ => {
    ...
    async fn eq_join_oneside<const SIDE: SideTypePrimitive>(
        ...
        let mut check_join_condition = |row_update: &RowRef<'_>,
         ...
         -> Result<bool> {
            ...
            if let Some(ref mut cond) = self.cond {
                ...
                cond_match =
                    Self::bool_from_array_ref(cond.eval(&new_row, &self.output_data_types)?);
            }
            Ok(cond_match)
        };
            ...
            match *op {
                Op::Insert | Op::UpdateInsert => {
                    ...
                    if let Some(matched_rows) = matched_rows {
                        ...
                        if forward_exactly_once(T, SIDE) {
                            if (degree == 0 && is_anti(T)) || (degree > 0 && is_semi(T)) {
                ...
                Op::Delete | Op::UpdateDelete => {
                    ...
                    if let Some(matched_rows) = matched_rows {
                        ...
                        if forward_exactly_once(T, SIDE) {
                            if (!matched && is_anti(T)) || (matched && is_semi(T)) {
                                hashjoin_chunk_builder.forward(*op, &row)?;
                            }
                        }
                    } else {
        ...
        let new_chunk = hashjoin_chunk_builder.finish()?;
