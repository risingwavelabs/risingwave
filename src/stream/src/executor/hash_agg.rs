...
impl<K: HashKey, S: StateStore> HashAggExecutor<K, S> {
        ...
        let futs = group_visibilities
            ...
            .filter_map(|(key, _)| {
                ...
                if vars.agg_group_cache.contains(key) {
                    ...
                    vars.stats.lookup_miss_count.fetch_add(1, Ordering::Relaxed);
                    Some(async {
    ...
    async fn flush_data<'a>(
        ...
        this.metrics
            ...
            .inc_by(vars.stats.lookup_miss_count.swap(0, Ordering::Relaxed));
        this.metrics
            .agg_total_lookup_count
            .with_label_values(&[&actor_id_str])
            .inc_by(vars.stats.total_lookup_count.swap(0, Ordering::Relaxed));
        this.metrics
            .agg_cached_keys
            .with_label_values(&[&actor_id_str])
