...
struct HashAggExecutorExtra<K: HashKey, S: StateStore> {
    ...
    /// How many times have we hit the cache of join executor for each lookup
    ...
    /// StreamChunk
    chunk_lookup_miss_count: AtomicU64,

    chunk_total_lookup_count: AtomicU64,
...
impl<K: HashKey, S: StateStore> HashAggExecutor<K, S> {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            extra: HashAggExecutorExtra {
                ...
                chunk_lookup_miss_count: AtomicU64::new(0),
                chunk_total_lookup_count: AtomicU64::new(0),
                metrics,
                chunk_size,
    ...
    async fn apply_chunk(
        HashAggExecutorExtra::<K, S> {
            ...
            lookup_miss_count,
            total_lookup_count,
            chunk_lookup_miss_count,
            chunk_total_lookup_count,
            ..
        ...
        let mut no_cache_miss_whole_chunk = true;
        let futs = unique_keys
            ...
            .filter_map(|(key, _, _)| {
                ...
                if agg_group_cache.contains(key) {
                    ...
                    no_cache_miss_whole_chunk = false;
                    Some(async {
        ...
        if !no_cache_miss_whole_chunk {
            chunk_lookup_miss_count.fetch_add(1, Ordering::Relaxed);
        }
        chunk_total_lookup_count.fetch_add(1, Ordering::Relaxed);
    ...
    async fn flush_data<'a>(
        &mut HashAggExecutorExtra::<K, S> {
            ...
            ref chunk_lookup_miss_count,
            ref chunk_total_lookup_count,
            ref metrics,
            ref chunk_size,
        ...
        metrics
            ...
            .with_label_values(&[&actor_id_str])
            .inc_by(chunk_lookup_miss_count.swap(0, Ordering::Relaxed));
        metrics
            .agg_chunk_total_lookup_count
            .with_label_values(&[&actor_id_str])
            .inc_by(chunk_total_lookup_count.swap(0, Ordering::Relaxed));
