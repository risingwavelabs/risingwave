...
/// `ReplicateExecutor` replicates upstream's arrangement state.
/// It is used by executors downstream of the arrangement.
/// These executors need the following properties:
/// 1. They can scale independently of the upstream arrangement executor.
///    This means that they could reside in different CNs,
///    and do not have access to the same shared buffer.
...
impl<S: StateStore, SD: ValueRowSerde> ReplicateExecutor<S, SD> {
    /// Create a new `ReplicateExecutor` with distribution specified with `distribution_keys` and
...
impl<S: StateStore, SD: ValueRowSerde> Executor for ReplicateExecutor<S, SD> {
...
impl<S: StateStore, SD: ValueRowSerde> std::fmt::Debug for ReplicateExecutor<S, SD> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ReplicateExecutor")
