...
mod tests {
    ...
    async fn test_managed_top_n_state() {
        ...
        // now ("ab", 4)
        let valid_rows = managed_state.find_range(None, 0, 1, epoch).await.unwrap();
        ...
        managed_state
            ...
            .unwrap();
        let valid_rows = managed_state.find_range(None, 1, 1, epoch).await.unwrap();
        assert_eq!(valid_rows.len(), 1);
        ...
        let valid_rows = managed_state.find_range(None, 1, 2, epoch).await.unwrap();
        ...
        let valid_rows = managed_state.find_range(None, 0, 3, epoch).await.unwrap();
