...
impl<K: HashKey, S: StateStore> JoinHashMap<K, S> {
    ...
    async fn fetch_cached_state(&self, key: &K) -> StreamExecutorResult<JoinEntryState> {
        ...
        if self.need_degree_table {
            let table_iter_fut = self.state.table.iter_key_and_val(
                &key,
                PrefetchOptions {
                    exhaust_iter: false,
                },
            );
            let degree_table_iter_fut = self.degree_state.table.iter_key_and_val(
                &key,
                PrefetchOptions {
                    exhaust_iter: false,
                },
            ...
            let table_iter = self
                .state
                .table
                .iter_with_pk_prefix(&key, PrefetchOptions { exhaust_iter: true })
                .await?;
