...
/// ```
#[derive(Debug)]
pub enum BtreeMapFlushStatus<T> {
...
impl<T: std::fmt::Debug> BtreeMapFlushStatus<T> {
    ...
    pub fn do_insert<K: Ord + std::fmt::Debug>(entry: btree_map::Entry<K, Self>, value: T) {
        match entry {
            btree_map::Entry::<K, Self>::Vacant(e) => {
                // No-op -> Insert
                e.insert(Self::Insert(value));
            }
            btree_map::Entry::<K, Self>::Occupied(mut e) => {
                if e.get().is_delete() {
                    // Delete -> DeleteInsert
    ...
    pub fn do_delete<K: Ord + std::fmt::Debug>(entry: btree_map::Entry<K, Self>) {
        match entry {
            btree_map::Entry::<K, Self>::Vacant(e) => {
                // No-op -> Delete
