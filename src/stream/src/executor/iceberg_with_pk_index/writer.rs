        ...
        while let Some(msg) = input.next().await {
            match msg? {
                ...
                Message::Barrier(barrier) => {
                    ...
                    // The pause half of the target-scoped pair makes this writer relinquish its
                    // state table after forwarding B1.
                    let signal = match barrier.iceberg_pk_index_barrier() {
                        Some(context)
                    ...
                    if let PauseSignal::Pause(task_id) = signal {
                        ...
                        match seal_control {
                            ...
                            IcebergPkIndexWriterControl::SealReady {
                                ...
                                "iceberg pk-index writer {} expected SealReady({}, {}) but received SealReady({}, {})",
                                self.sink_id,
                                task_id,
                                seal_epoch,
                                received_task_id,
                            ...
                            IcebergPkIndexWriterControl::Committed {
                                ...
                                "iceberg pk-index writer {} received Committed({}, {}) before SealReady({}, {})",
                                self.sink_id,
                                received_task_id,
                                received_epoch,
                                task_id,
                        ...
                        let next_message = input.next().await.ok_or_else(|| {
                            StreamExecutorError::channel_closed(
                                "pk-index writer input closed while waiting for B2",
                            )
                        })??;
                        let b2 = match next_message {
                            Message::Barrier(barrier) => barrier,
                            Message::Chunk(_) => bail!(
                                "iceberg pk-index writer {} received chunk between B1 and B2",
                                self.sink_id
                            ),
                            Message::Watermark(_) => bail!(
                                "iceberg pk-index writer {} received watermark between B1 and B2",
                                self.sink_id
                            ),
                        };
                        if !b2.is_checkpoint() || b2.epoch.prev != seal_epoch {
                            bail!(
                                "iceberg pk-index writer {} expected checkpoint B2 starting at {}, got {:?}",
                                self.sink_id,
                                seal_epoch,
                                b2
                            );
                        ...
                        match b2.iceberg_pk_index_barrier() {
                            Some(context)
                                if context.sink_id == self.sink_id
                                    ...
                                    && context.gated_actor_ids.contains(&self.ctx.id) => {}
                            _ => bail!(
                                "iceberg pk-index writer {} expected matching Resume mutation on B2",
                                self.sink_id
                            ),
                        }
                        ...
                        loop {
                            let control = self.writer_control_rx.recv().await.ok_or_else(|| {
                                StreamExecutorError::channel_closed(
