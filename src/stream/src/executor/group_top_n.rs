...
use super::top_n::{generate_executor_pk_indices_info, TopNCache, TopNCacheRowNumber};
...
pub struct InnerGroupTopNExecutorNew<S: StateStore> {
    ...
    caches: HashMap<Vec<Datum>, TopNCacheRowNumber>,
    ...
    async fn apply_chunk(&mut self, chunk: StreamChunk) -> StreamExecutorResult<StreamChunk> {
        ...
        for (op, row_ref) in chunk.rows() {
            ...
            match entry {
                ...
                Vacant(entry) => {
                    let mut topn_cache =
                        TopNCacheRowNumber::new(self.offset, self.limit, self.sort_key_len);
                    self.managed_state
