...
impl<S: StateStore> GapFillExecutor<S> {
            ...
            row_buffer
                .binary_search_by(|extended_row| {
                    if let Some(existing_time) = extended_row.row.datum_at(time_column_index) {
                        if let Some(existing_impl) = existing_time.to_owned_datum() {
    ...
    async fn generate_filled_rows_between_static(
        prev_row: &OwnedRow,
        curr_row: &OwnedRow,
...
impl<S: StateStore> GapFillExecutor<S> {
    ...
    async fn execute_inner(self: Box<Self>) {
        ...
        // Initialize row_buffer with original rows from state
        for row in all_rows_from_state {
        ...
        if let Ok(Some(ScalarImpl::Interval(interval))) = gap_interval.eval_row(&dummy_row).await {
                        ...
                        if is_optimizable_update {
                                ...
                                .position(|r| r.row == old_row && r.row_type == RowType::Original)
                            {
                                ...
                                if is_locf {
                                    ...
                                    if let Some(next) = next_original_info {
                                        let dummy_row = OwnedRow::new(vec![]);
                                        if let Ok(Some(ScalarImpl::Interval(interval))) =
                                            gap_interval.eval_row(&dummy_row).await
                                        {
                                            let filled_rows =
                                        ...
                                        if start_idx < end_idx {
                                            ...
                                            for extended_row in rows_to_delete {
                                                if extended_row.row_type == RowType::Filled {
                                                    state_table.delete(&extended_row.row);
                                                    temp_outputs
