...
pub mod agg_executor {
    ...
    pub async fn create_agg_state_storage<S: StateStore>(
        ...
        match agg_call.kind {
            AggKind::Builtin(PbAggType::Min | PbAggType::Max) if !is_append_only => {
                ...
                add_column(agg_call.args.val_indices()[0], agg_call.args.arg_types()[0].clone(), if matches!(agg_call.kind, AggKind::Builtin(PbAggType::Max)) {
            ...
            AggKind::Builtin(
                PbAggType::Min /* append only */
                | PbAggType::Max /* append only */
                | PbAggType::Sum
                | PbAggType::Sum0
                | PbAggType::Count
                | PbAggType::Avg
