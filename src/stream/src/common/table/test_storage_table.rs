...
#[tokio::test]
async fn test_mem_table_spill_in_batch_mode() {
    const TEST_TABLE_ID: TableId = TableId { table_id: 233 };
    let test_env = prepare_hummock_test_env().await;
    ...
    let column_ids_partial = vec![ColumnId::from(1)];
    let table = StorageTable::new_partial(
        test_env.storage.clone(),
        TEST_TABLE_ID,
        column_descs.clone(),
    ...
    let res = iter1.next_row().await.unwrap();
    assert_eq!(res, None);
    ...
    // Can not read spill data with prev epoch.
    assert_eq!(get_row1_res, None);
    ...
    // Can not read spill data with prev epoch.
    assert_eq!(get_row3_res, None);

    epoch.inc_for_test(10);

    state.commit(epoch).await.unwrap();
    test_env.commit_epoch(epoch.prev).await;

    // Now, spillable data can be read.
    ...
    assert_eq!(
        get_row1_res,
        Some(OwnedRow::new(vec![Some(text1.clone().into()),]))
    );
    ...
    assert_eq!(
        get_row2_res,
        Some(OwnedRow::new(vec![Some(text2.clone().into()),]))
    );
    ...
    assert_eq!(
        get_row3_res,
        Some(OwnedRow::new(vec![Some(text3.clone().into()),]))
    );
    ...
    let res = iter2.next_row().await.unwrap();
    assert_eq!(res, Some(OwnedRow::new(vec![Some(text1.clone().into()),])));

    let res = iter2.next_row().await.unwrap();
    assert_eq!(res, Some(OwnedRow::new(vec![Some(text2.clone().into()),])));

    let res = iter2.next_row().await.unwrap();
    assert_eq!(res, Some(OwnedRow::new(vec![Some(text3.clone().into()),])));
}

#[tokio::test]
async fn test_mem_table_spill_multiple_times_in_batch_mode() {
    const TEST_TABLE_ID: TableId = TableId { table_id: 233 };
    let test_env = prepare_hummock_test_env().await;
