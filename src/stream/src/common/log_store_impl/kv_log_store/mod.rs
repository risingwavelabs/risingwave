...
mod tests {
    ...
    async fn test_update_vnode_recover() {
        ...
        let vnodes1 = build_bitmap((0..VirtualNode::DEFAULT_COUNT).filter(|i| i % 2 == 0));
        let vnodes2 = build_bitmap((0..VirtualNode::DEFAULT_COUNT).filter(|i| i % 2 == 1));
        ...
        let vnodes = build_bitmap(0..VirtualNode::DEFAULT_COUNT);
        let factory = KvLogStoreFactory::new(
            test_env.storage.clone(),
            table.clone(),
            Some(vnodes),
