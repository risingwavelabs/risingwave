...
mod tests {
    ...
    async fn test_basic_inner(max_row_count: usize, pk_info: &'static KvLogStorePkInfo) {
        ...
        let epoch2 = epoch1 + 1;
        writer
            .flush_current_epoch(epoch2, false, false)
            .await
            .unwrap();
        writer.write_chunk(stream_chunk2.clone()).await.unwrap();
        let epoch3 = epoch2 + 1;
        writer
            .flush_current_epoch(epoch3, true, false)
            .await
            .unwrap();
    ...
    async fn test_rewind_on_consuming_persisted_log() {
        ...
        writer.write_chunk(stream_chunk3.clone()).await.unwrap();
        writer
            .flush_current_epoch(u64::MAX, true, false)
            .await
            .unwrap();
        ...
        let epoch5 = epoch4 + 1;
        writer
            .flush_current_epoch(epoch5, true, false)
            .await
            .unwrap();
