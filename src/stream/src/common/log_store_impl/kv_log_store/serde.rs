...
use risingwave_hummock_sdk::key::{next_key, FullKey, TableKey};
...
impl<S: StateStoreReadIterStream> LogStoreRowOpStream<S> {
    async fn next_op(&mut self) -> LogStoreResult<Option<(u64, LogStoreRowOp, usize)>> {
        ...
        {
            let (key, value): (FullKey<_>, Bytes) = result?;
            let read_size = key.user_key.table_key.estimated_size() + value.estimated_size();
...
mod tests {
    ...
    async fn test_log_store_stream_basic() {
        ...
        let stream = new_log_store_item_stream(
            vec![stream],
            serde,
            CHUNK_SIZE,
            KvLogStoreReadMetrics::for_test(),
