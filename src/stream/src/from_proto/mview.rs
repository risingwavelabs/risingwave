...
impl ExecutorBuilder for MaterializeExecutorBuilder {
    ...
    async fn new_boxed_executor(
        ...
        macro_rules! new_refreshable_executor {
            ($SD:ident) => {
                RefreshableMaterializeExecutor::<_, $SD>::new(
                    input,
                    params.info.schema.clone(),
                    store,
                    order_key,
        ...
        let exec = if refreshable {
            ...
            if versioned {
                new_refreshable_executor!(ColumnAwareSerde)
            } else {
                new_refreshable_executor!(BasicSerde)
            }
        } else {
