...
impl CdcTableBackfillState {
    pub fn to_pb(self, actor_id: ActorId, epoch: u64) -> PbCdcTableBackfillProgress {
        must_match!(self, CdcTableBackfillState::Finish(split_id_start_inclusive, split_id_end_inclusive, generation) => {
...
impl CdcProgressReporter {
    ...
    pub fn finish(
        ...
        self.barrier_manager.update_cdc_backfill_progress(
            actor_id,
            epoch,
            CdcTableBackfillState::Finish(split_id_range.0, split_id_range.1, generation),
        );
    }
}
