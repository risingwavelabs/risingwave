...
impl IcebergPkIndexWriterControl {
    pub fn task_id(self) -> IcebergCompactionTaskId {
        match self {
            Self::SealReady { task_id, .. } | Self::Committed { task_id, .. } => task_id,
        }
    }

    pub fn epoch(self) -> u64 {
        match self {
            Self::SealReady { epoch, .. } | Self::Committed { epoch, .. } => epoch,
        }
    }
}
