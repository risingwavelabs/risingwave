...
use risingwave_connector::parser::json_parser::JsonAccessBuilder;
use risingwave_connector::parser::unified::Access;
...
fn create_large_payloads(batch_size: usize) -> (Vec<Vec<u8>>, Vec<u8>) {
    ...
    for i in 0..batch_size {
        let single = format!(r#"{{"id": {}, "name": "item_{}", "value": {}}}"#, i, i, i as f64 * 1.5);
        individual_payloads.push(single.as_bytes().to_vec());
        
        let array_obj = format!(r#"{{"id": {}, "name": "item_{}", "value": {}}}"#, i, i, i as f64 * 1.5);
...
fn bench_individual_parsing(c: &mut Criterion) {
    ...
    group.bench_function("individual_parsing", |b| {
        b.iter(|| {
            let access = builder.generate_accessor(payload.clone(), &()).await.unwrap();
            if let AccessImpl::Json(json_access) = access {
                let _ = json_access.access(&["id"], &DataType::Int32).unwrap();
                let _ = json_access.access(&["name"], &DataType::Varchar).unwrap();
                let _ = json_access.access(&["value"], &DataType::Float64).unwrap();
            }
        })
    });
            ...
            |b, &_| {
                b.iter(|| {
                    for payload in &individual_payloads {
                        let access = individual_builder.generate_accessor(payload.clone(), &()).await.unwrap();
                        if let AccessImpl::Json(json_access) = access {
                            let _ = json_access.access(&["id"], &DataType::Int32).unwrap();
                        }
                    }
