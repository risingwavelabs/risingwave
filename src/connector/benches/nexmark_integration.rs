...
use risingwave_connector::parser::{
    ByteStreamSourceParser, EncodingProperties, JsonParser, JsonProperties, ProtocolProperties,
    SourceParserIntoStreamExt, SpecificParserConfig,
};
...
fn make_data_stream() -> BoxSourceStream {
    let mut generator = nexmark::EventGenerator::default()
        ...
        .map(|e| match e {
            nexmark::event::Event::Bid(bid) => bid,
            _ => unreachable!(),
        })
        .enumerate();
