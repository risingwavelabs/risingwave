...
// Batch PosixFs for refreshable tables - only lists files on demand, not continuously
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct OpendalBatchPosixFs;

impl OpendalSource for OpendalBatchPosixFs {
    type Properties = BatchPosixFsProperties;

    fn new_enumerator(properties: Self::Properties) -> ConnectorResult<OpendalEnumerator<Self>> {
        OpendalEnumerator::new_batch_posix_fs_source(properties)
    }
}
...
impl SourceProperties for BatchPosixFsProperties {
    type Split = OpendalFsSplit<OpendalBatchPosixFs>;
    type SplitEnumerator = OpendalEnumerator<OpendalBatchPosixFs>;
    type SplitReader = OpendalReader<OpendalBatchPosixFs>;
