...
impl SplitMetaData for SplitImpl {
    ...
    fn update_offset(&mut self, last_recorded_offset: String) -> Result<()> {
        dispatch_split_impl!(
            self,
            inner,
            IgnoreType,
            inner.update_offset(last_recorded_offset)
        )
    }
}
...
impl SplitImpl {
    ...
    pub fn update_in_place(&mut self, last_recorded_offset: String) -> Result<()> {
...
pub trait SplitMetaData: Sized {
    ...
    fn encode_to_json(&self) -> JsonbVal;
    fn restore_from_json(value: JsonbVal) -> Result<Self>;
    fn update_offset(&mut self, last_recorded_offset: String) -> crate::error::ConnectorResult<()>;
}
