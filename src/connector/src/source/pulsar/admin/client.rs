...
mod test {
    ...
    #[tokio::test]
    async fn test() {
        let client = PulsarAdminClient::new(
            "https://pulsar-gcp-useast1.api.streaming.datastax.com".to_string(),
            None,
        );
        let topic = parse_topic("persistent://meetup/default/twitter").unwrap();
