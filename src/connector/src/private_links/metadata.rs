...
async fn print_metadata(brokers: &str, topic: Option<&str>, timeout: Duration, fetch_offsets: bool) {
    ...
    let metadata = consumer
        .fetch_metadata(topic, timeout).await
        ...
        for partition in topic.partitions() {
            ...
            if fetch_offsets {
                let (low, high) = consumer
                    .fetch_watermarks(topic.name(), partition.id(), Duration::from_secs(1)).await
                    .unwrap_or((-1, -1));
                println!(
