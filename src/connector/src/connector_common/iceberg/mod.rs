...
impl IcebergCommon {
    pub async fn sync_table_comments(
        ...
        let catalog_config = self.resolve_catalog_config(java_catalog_props.clone())?;
        match catalog_config.build_plan()? {
            CatalogBuildPlan::Storage(config) => {
                let catalog = storage_catalog::StorageCatalog::new(config)?;
            ...
            CatalogBuildPlan::NativeRest(_) | CatalogBuildPlan::NativeGlue(_) => {
