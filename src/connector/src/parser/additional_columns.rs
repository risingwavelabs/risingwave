...
pub static COMPATIBLE_ADDITIONAL_COLUMNS: LazyLock<HashMap<&'static str, HashSet<&'static str>>> =
    LazyLock::new(|| {
        HashMap::from([
            ...
            (
                NATS_CONNECTOR,
                HashSet::from(["partition", "offset", "payload", "subject", "header"]),
            ),
            (
...
mod test {
    ...
    #[test]
    fn test_build_nats_header_inner_additional_column() {
        let col = build_additional_column_desc(
            ColumnId::new(1),
            NATS_CONNECTOR,
            "header",
            Some("nats_header".to_owned()),
            Some("trace-id"),
            Some(&DataType::Varchar),
            true,
            false,
        ...
        assert_eq!(col.name, "nats_header");
        assert_eq!(col.data_type, DataType::Varchar);
        assert_matches::assert_matches!(
            col.additional_column.column_type,
            Some(AdditionalColumnType::HeaderInner(ref header))
                if header.inner_field == "trace-id"
        );
    }

    #[test]
    fn test_build_nats_headers_list_additional_column() {
        let col = build_additional_column_desc(
            ColumnId::new(2),
            NATS_CONNECTOR,
            "header",
    ...
    #[test]
    fn test_build_nats_subject_additional_column() {
        let col = build_additional_column_desc(
            ColumnId::new(3),
            NATS_CONNECTOR,
            "subject",
            Some("nats_subject".to_owned()),
            None,
            None,
            true,
            false,
        ...
        assert_eq!(col.name, "nats_subject");
        assert_eq!(col.data_type, DataType::Varchar);
        assert_matches::assert_matches!(
            col.additional_column.column_type,
            Some(AdditionalColumnType::Subject(_))
        );
    }

    #[test]
    fn test_build_nats_subject_rejected_for_kafka_when_strict() {
