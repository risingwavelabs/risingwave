...
impl MessageMeta<'_> {
    ...
    fn value_for_column(self, desc: &SourceColumnDesc) -> Option<Datum> {
        match desc.column_type {
            ...
            SourceColumnType::Offset => Datum::Some(self.meta.get_offset().into()).into(),
        ...
        let mut wrapped_f = |desc: &SourceColumnDesc| {
            match (&desc.column_type, &desc.additional_column.column_type) {
                ...
                (_, &Some(AdditionalColumnType::Filename(_))) => {
                    ...
                    return Ok(A::output_for(self.row_meta.as_ref().map(|ele| {
                        ScalarImpl::Utf8(ele.meta.get_split_id().to_string().into())
                    })));
                }
                (_, _) => {
                        ...
                        Err(error) => {
                            ...
                            if let Ok(suppressed_count) = LOG_SUPPERSSER.check() {
                                tracing::warn!(
                                    ...
                                    split_id = self.row_meta.as_ref().map(|m| m.meta.get_split_id().as_ref()),
                                    offset = self.row_meta.as_ref().map(|m| m.meta.get_offset()),
                                    column = desc.name,
                                    suppressed_count,
        ...
        for (i, msg) in batch.into_iter().enumerate() {
            ...
            match parser
                .parse_one_with_txn(
                    ...
                    builder
                        .row_writer()
                        .with_meta(MessageMeta { meta: &msg.meta }),
                )
                .await
            {
                ...
                res @ (Ok(ParseResult::Rows) | Err(_)) => {
                    ...
                    if let Err(error) = res {
                        ...
                        if let Ok(suppressed_count) = LOG_SUPPERSSER.check() {
                            tracing::error!(
                                error = %error.as_report(),
                                split_id = msg.meta.get_split_id().as_ref(),
                                offset = msg.meta.get_offset(),
                                suppressed_count,
