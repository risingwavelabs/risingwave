...
use crate::schema::pulsar_schema::{
    PulsarSchemaClient, PulsarSchemaClientConfig, PulsarSchemaInfo,
};

#[derive(Debug)]
pub struct PulsarSchemaCache {
    schema_cache: Cache<i64, Arc<Schema>>,
    schema_client: PulsarSchemaClient,
    topic: String,
}

impl PulsarSchemaCache {
    pub fn new(config: PulsarSchemaClientConfig, topic: String) -> ConnectorResult<Self> {
        Ok(Self {
            schema_cache: Cache::new(u64::MAX),
            schema_client: PulsarSchemaClient::new(config)?,
            topic,
        })
    }
    ...
    pub async fn get_latest(&self) -> ConnectorResult<Arc<Schema>> {
        self.parse_and_cache_schema(self.schema_client.get_schema(&self.topic, None).await?)
            .await
    }
