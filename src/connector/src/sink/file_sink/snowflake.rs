...
pub struct SnowflakeConfig {
    ...
    #[serde(rename = "snowflake.post")]
    pub snowflake_post: Option<String>,

    #[serde(rename = "snowflake.token")]
...
impl Sink for SnowflakeSink {
    type Coordinator = SnowflakeSinkCommitCoordinator;
    ...
    async fn new_coordinator(
        ...
        let coordinator = SnowflakeSinkCommitCoordinator::new(
            self.config.clone(),
            &self.schema,
            &self.pk_indices,
        )?;
...
impl SnowflakeSinkCommitCoordinator {
    pub fn new(
        ...
        let post = properties
            .snowflake_post
            .ok_or(SinkError::Config(anyhow!("snowflake.post is required")))?;
        let token = properties
            .snowflake_token
            .ok_or(SinkError::Config(anyhow!("snowflake.token is required")))?;
        Ok(Self {
            task_name,
            cdc_table_name,
            target_table_name,
