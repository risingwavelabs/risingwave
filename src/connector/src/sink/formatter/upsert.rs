...
impl<'a, KE, VE> UpsertFormatter<'a, KE, VE> {
    pub fn new(
        key_encoder: KE,
        val_encoder: VE,
        schema: &'a Schema,
...
impl<'a, KE: RowEncoder, VE: RowEncoder> SinkFormatter for UpsertFormatter<'a, KE, VE> {
    ...
    fn format_row(&mut self, op: Op, row: RowRef<'_>) -> Result<FormattedRow<Self::K, Self::V>> {
        ...
        let event_object = match op {
            Op::Insert | Op::UpdateInsert => {
                Some(self.val_encoder.encode_all(row, &self.schema.fields)?)
            }
            // TODO(xiangjinwu): What is `serde_json::Value::Null.to_string()` in proto / avro?
            Op::Delete => None,
            Op::UpdateDelete => {
