...
use iceberg::spec::{DataFile, Operation, Schema, SerializedDataFile};
...
impl IcebergSinkCommitter {
    ...
    fn build_drop_column_schema_updates(
        &self,
        dropped_column_names: &[String],
    ) -> Result<(Vec<TableRequirement>, Vec<TableUpdate>)> {
        let dropped_column_names: HashSet<_> =
            dropped_column_names.iter().map(String::as_str).collect();
        let current_schema = self.table.metadata().current_schema();
        let retained_fields = current_schema
            .as_struct()
            .fields()
            .iter()
        ...
        if retained_fields.len() == current_schema.as_struct().fields().len() {
            return Err(SinkError::Iceberg(anyhow!(
                "None of the dropped columns {:?} exist in the current iceberg schema",
                dropped_column_names
            )));
        }

        let retained_identifier_field_ids =
            current_schema.identifier_field_ids().filter(|field_id| {
                current_schema
                    .field_by_id(*field_id)
                    .is_some_and(|field| !dropped_column_names.contains(field.name.as_str()))
            });

        let new_schema_id = self
            .table
            .metadata()
            .schemas_iter()
        ...
        let new_schema = Schema::builder()
            ...
            .with_fields(retained_fields)
            .with_identifier_field_ids(retained_identifier_field_ids)
            .build()
            .map_err(|err| SinkError::Iceberg(anyhow!(err)))?;

        let requirements = vec![
            TableRequirement::LastAssignedFieldIdMatch {
                last_assigned_field_id: self.table.metadata().last_column_id(),
            },
        ...
        let action = match schema_change.op.as_ref() {
            ...
            Some(SinkSchemaChangeOp::DropColumns(drop_columns_op)) => {
                ...
                let (requirements, updates) =
                    self.build_drop_column_schema_updates(&drop_columns_op.column_names)?;
                let updated_table = self
                    .config
                    .common
                    .update_table_with_updates(
                        self.table.identifier(),
