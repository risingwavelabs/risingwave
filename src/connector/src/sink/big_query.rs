...
impl BigQuerySink {
    ...
    fn get_string_and_check_support_from_datatype(rw_data_type: &DataType) -> Result<String> {
        match rw_data_type {
            ...
            DataType::TimestampNano => Ok("DATETIME".to_owned()),
    ...
    fn map_field(rw_field: &Field) -> Result<TableFieldSchema> {
        let tfs = match &rw_field.data_type {
            ...
            DataType::TimestampNano => TableFieldSchema::date_time(&rw_field.name),
...
fn build_protobuf_field(
    ...
    match data_type {
        ...
        DataType::TimestampNano => field.r#type = Some(field_descriptor_proto::Type::String.into()),
