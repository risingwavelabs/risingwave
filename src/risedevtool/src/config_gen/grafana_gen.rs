...
datasources:
  - name: risedev-prometheus
    ...
    pub fn gen_jaeger_datasource_yml(&self, config: &GrafanaConfig) -> Result<String> {
        let provide_jaeger = config.provide_jaeger.as_ref().unwrap();
        if provide_jaeger.len() != 1 {
            return Err(anyhow!(
                "expect 1 jaeger nodes, found {}",
                provide_jaeger.len()
            ));
        }
        let jaeger = &provide_jaeger[0];
        let jaeger_host = &jaeger.dashboard_address;
        let jaeger_port = &jaeger.dashboard_port;
