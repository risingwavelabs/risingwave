...
"#
    ...
    pub fn gen_kraft_server_properties(
        &self,
        config: &KafkaConfig,
        kafka_log_dirs: &str,
    ) -> String {
        ...
        let kafka_advertise_host = &config.address;
        let kafka_port = &config.port;
        let kafka_port_next = kafka_port + 1;
        // let zookeeper_hosts = config
        //     .provide_zookeeper
        //     .as_ref()
        //     .unwrap()
...
controller.quorum.voters=1@localhost:{kafka_port_next}
...
#   EXAMPLE:
#     listeners = PLAINTEXT://your.host.name:9092
listeners=PLAINTEXT://:{kafka_port},CONTROLLER://:{kafka_port_next}
