...
impl DockerServiceConfig for MqttConfig {
    fn id(&self) -> String {
        self.id.clone()
    }
    ...
    fn ports(&self) -> Vec<(String, String)> {
        vec![(self.port.to_string(), "1883".to_owned())]
    }

    fn data_path(&self) -> Option<String> {
        self.persist_data.then(|| "/mosquitto/data".to_owned())
    }
}

/// Docker-backed MQTT service.
pub type MqttService = DockerService<MqttConfig>;
