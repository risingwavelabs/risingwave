...
use crate::wait::wait;
use crate::{STARROCKS_CONTAINER_HOST, STARROCKS_CONTAINER_QUERY_PORT, StarrocksConfig};

pub struct StarrocksReadyCheckTask {
    config: StarrocksConfig,
}

impl StarrocksReadyCheckTask {
    pub fn new(config: StarrocksConfig) -> Self {
        Self { config }
    }
}
...
impl Task for StarrocksReadyCheckTask {
    fn execute(&mut self, ctx: &mut ExecuteContext<impl Write>) -> Result<()> {
        let Some(id) = ctx.id.clone() else {
            panic!("Service should be set before executing StarrocksReadyCheckTask");
        };
            ...
            || {
                let mut command = if self.config.user_managed {
                    ...
                    command
                        ...
                        .arg("mysql")
                        .arg("-h")
                        .arg(STARROCKS_CONTAINER_HOST)
                        .arg("-P")
                        .arg(STARROCKS_CONTAINER_QUERY_PORT.to_string())
                        .arg("-u")
                        .arg(&self.config.admin_user);
                    command
                };
                ...
                let output = command
                    ...
                    .output()
                    .context("failed to query StarRocks backends")?;
                ensure!(
                    output.status.success(),
                    "failed to query StarRocks backends: {}",
                    String::from_utf8_lossy(&output.stderr).trim()
                );
                ensure!(
                    backend_is_alive(&String::from_utf8_lossy(&output.stdout)),
                    "StarRocks BE is not alive yet"
                );
                Ok(())
            },
            &mut ctx.log,
