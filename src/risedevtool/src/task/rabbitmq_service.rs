...
impl DockerServiceConfig for RabbitMqConfig {
    ...
    fn envs(&self) -> Vec<(String, String)> {
        vec![
            ("RABBITMQ_DEFAULT_USER".to_owned(), self.user.clone()),
            ("RABBITMQ_DEFAULT_PASS".to_owned(), self.password.clone()),
        ]
    }

    fn ports(&self) -> Vec<(String, String)> {
        vec![
            (self.port.to_string(), "5672".to_owned()),
            (self.management_port.to_string(), "15672".to_owned()),
        ]
    }

    fn data_path(&self) -> Option<String> {
        self.persist_data.then(|| "/var/lib/rabbitmq".to_owned())
    }
}

/// Docker-backed RabbitMQ service.
