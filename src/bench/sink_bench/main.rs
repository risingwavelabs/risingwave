...
impl MockDatagenSource{
    pub async fn new() -> MockDatagenSource{
        ...
        let reader = DatagenSplitReader::new(properties.clone(), splits, parser_config.clone(), Default::default(), Some(mock_datum.clone())).await.unwrap();

        MockDatagenSource{ reader }
    }
