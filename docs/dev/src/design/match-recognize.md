...
StreamExchange HASH(partition keys)   -- exact key order (see below)
  └─ StreamEowcSort                   -- full ORDER BY, releases rows strictly below the watermark
       └─ StreamMatchRecognize        -- same fragment: no exchange in between, order preserved
```

`StreamEowcSort` is a watermark-driven sorter shared by emit-on-window-close operators and
`MATCH_RECOGNIZE`. It buffers rows in a state table and, when the watermark on its sort column
advances, emits every buffered row strictly below it in `(sort column, remaining PK)` order, then
forwards the watermark. `MATCH_RECOGNIZE` extends it with **secondary order columns** —
...
- **Logical plan** (`logical_match_recognize.rs`): `LogicalMatchRecognize` with the standard trait
  ...
  exact-key exchange, `StreamEowcSort` with the secondary order columns, then the matcher.
...
re-feeding, and `DEFINE`/`MEASURES` are evaluated at match time. (The upstream `EowcSort` has
...
choose their order: `StreamEowcSort` has already emitted them using its physical table key
...
Interval bounds must have a zero month component after constant folding. Calendar months and
calendar years are currently rejected: month-end adjustment can give an earlier match start a
later deadline, violating the finality gate's ordering assumption. Positive bounds expressed in
days and seconds remain supported, subject to the existing non-negative-component checks.
...
Rows tying on the full query `ORDER BY` are ordered by `StreamEowcSort`'s remaining physical
