...
`SHOW CREATE TABLE` 暴露。**RisingWave Secret 名称**留空时，connector 会为每张表创建并
管理一个 Secret；也可以填写配置 schema 中已有且值与 **Webhook 密钥**一致的 Secret
名称。自动管理需要 Secret Management 功能和 `CREATE SECRET` 权限。
